...
@Component({
  selector: 'app-project-workflow-details',
  templateUrl: './project-workflow-details.component.html',
  styleUrls: ['./project-workflow-details.component.scss']
})
export class ProjectWorkflowDetailsComponent implements OnInit {
  ...
  auditWorkflowCommonSubscription: Subscription
  nextLevelId: number;
  workflowType: string;
  ...
  ngOnInit(): void {
    ...
    this._route.params.subscribe(params => {
      ...
      ProjectMonitoringWorkflowStore.workflowId = id;
      this.workflowId = id;
    ...
    this.auditWorkflowCommonSubscription = this._eventEmitterService.auditWorkflowCommonAddModal.subscribe(res => {
      this.closeModal(6)
    });
  ...
  deleteWorkflow(status: boolean) {
    if (status && this.deleteObject.id) {
      this._projectWorkflowService.deleteWorkflowSections(this.deleteObject.id, this.workflowId).subscribe(resp => {
        setTimeout(() => {
  ...
  openPopupSection(type: string, level: number) {
    let levelId = level + 1;
    ProjectMonitoringWorkflowStore.workflowPopupEnabled = true;
    this.workFlowSourceData.type = "Add"
    this.workFlowSourceData.values = {
      workflowId: this.workflowId,
      level: levelId,
      module: 'Project_Monitoring_Module',
      moduleGroupId: 3700
    }
  ...
  workflowPopupsSections(type: string, level?) {
    ...
    this.workFlowSourceData.values = {
      workflowId: this.workflowId,
      level: ProjectMonitoringWorkflowStore.ProjectWorkflowDetails.workflow_items.length + 1,
      module: 'Project_Monitoring_Module',
      moduleGroupId: 3700
    }
    switch (type) {
  ...
  closeModal(popUp: number) {
    if (!ProjectMonitoringWorkflowStore.workflowPopupEnabled) {
  ...
  getStrategyWorkflow(id) {
    this._projectWorkflowService.getItem(id).subscribe(res => {
      this._utilityService.detectChanges(this._cdr);
  ...
  ngOnDestroy() {
    ProjectMonitoringWorkflowStore.workflowId = null;
    if (this.reactionDisposer) this.reactionDisposer();
    SubMenuItemStore.makeEmpty();
    ProjectMonitoringWorkflowStore.unsetIndividualWorkflow()
    ...
    this.auditWorkflowCommonSubscription.unsubscribe();
