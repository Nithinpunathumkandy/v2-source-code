...
@Component({
  selector: 'app-change-request-dashboard',
  templateUrl: './change-request-dashboard.component.html',
  styleUrls: ['./change-request-dashboard.component.scss']
})
export class ChangeRequestDashboardComponent implements OnInit,OnDestroy {
  ...
  ngOnInit(): void {
    ProjectDashboardStore.changeRequestLoaded = false;
    ...
    this.filterSubscription = this._eventEmitterService.sidebarFilterChanged.subscribe(filter => {
      this.ProjectDashboardStore.dashboardLoaded = false;
       this.getChangeRequestByStatus();
       this.getChangeRequestByDepartments();
      setTimeout(() => {
    ...
    setTimeout(() => {
      ProjectDashboardStore.changeRequestLoaded = true;
    }, 1000);
  }
  ...
  ngAfterViewInit(): void {
    ...
    setTimeout(() => {
      ...
      // setTimeout(() => {
      RightSidebarLayoutStore.filterPageTag = 'project_change_request_dashbord';
  ...
  getChangeRequestByDepartments(){
    this._projectDashboardService.getChangeRequestByDepartments().subscribe(res => {
      setTimeout(() => {
   
          this.createBarChartChangeRequestByDepartments();
  ...
  getChangeRequestByStatus() {
    this._projectDashboardService.getChangeRequestByStatus().subscribe(res => {
      setTimeout(() => {
        if (res.length != 0) {
          res.forEach(element => {
            this.count+=element.count
          });
          this.pieChartForChangeRequestByStatus(res);
        }else
        {
          this.count = null;
          this.pieChartForChangeRequestByStatus(res);
  ...
  pieChartForChangeRequestByStatus(chartCount){
    am4core.addLicense("CH199714744");
    // Create chart instance
    if(!this.checkDataIsPresent(ProjectDashboardStore.ChangeRequestByStatus,'count')){
      this.showNoDataMap=true;
      return
    }else{
      this.showNoDataMap=false;
    }
    let chart = am4core.create("chartChangeRequestByStatus", am4charts.PieChart);
    
    chart.data = ProjectDashboardStore.ChangeRequestByStatus;
  ...
  createBarChartChangeRequestByDepartments(){
    am4core.addLicense("CH199714744");
    // Create chart instance
    if(ProjectDashboardStore.ChangeRequestByDepartments.length==0){
      this.showNoDataMap=true;
      return
    }else{
      this.showNoDataMap=false;
    ...
    let chart = am4core.create('barChartChangeRequestByDepartments', am4charts.XYChart);
    chart.data = ProjectDashboardStore.ChangeRequestByDepartments;
    chart.rtl = AuthStore.user.language.is_rtl ? true : false;
