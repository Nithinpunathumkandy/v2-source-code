...
@Component({
  selector: 'app-project-monitoring-overview',
  templateUrl: './project-monitoring-overview.component.html',
  styleUrls: ['./project-monitoring-overview.component.scss']
})
export class ProjectMonitoringOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(3700)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=3700&module_ids='+id).subscribe(res=>{
            this.loaded=true
