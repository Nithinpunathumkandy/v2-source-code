...
@Component({
  selector: 'app-project-issue-ca',
  templateUrl: './project-issue-ca.component.html'
})
export class ProjectIssueCaComponent implements OnInit, OnDestroy {
   ...
   *
   * @memberof ProjectIssueCaComponent
   */
  ngOnInit(): void {
    ...
    this.addCASubscriptionEvent = this._eventEmitterService.projectIssueCaModal.subscribe(res => {
      this.closeNewCa();
      this.pageChange();
    })
  ...
  pageChange(){
    let params = `&project_issue_ids=${ProjectIssueStore.IssueId}`
    this._projectIssueCaService.getItems(false, params).subscribe(res=>{
      console.log(res);
      if (res.data)
  ...
  getCorrectiveActionDetails(id) {
    CaStore.ProjectIssueCaId = id;
    this._projectIssueCaService.getCa(id).subscribe(() => {
      CaStore.setSelected(id)
      this._utilityService.detectChanges(this._cdr);
    })
  }
  ...
  closeUpdateModal() {
    ...
    this.pageChange();
    this.getCorrectiveActionDetails(CaStore.ProjectIssueCaId);
  }
  ...
  *// Closes from preview
  ...
  gotoIncident() {
    this._router.navigate([`project-monitoring/projects/${ProjectMonitoringStore.selectedProjectId}/issue`]);
  }
