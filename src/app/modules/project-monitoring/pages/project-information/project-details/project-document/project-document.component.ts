...
import { ProjectDocumentService } from 'src/app/core/services/project-monitoring/project-document/project-document.service';
...
export class ProjectDocumentComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private _projectDocumentService: ProjectDocumentService,
  ...
  ngOnInit(): void {
    ...
    this.reactionDisposer = autorun(() => {  
      ...
      if(ProjectMonitoringStore.individualDetails?.project_monitoring_status?.type =='closed')
      {
        ...
        subMenuItems.push({activityName: 'PROJECT_MONITORING_DOCUMENT_LIST', submenuItem: {type: 'search'}}),
        subMenuItems.push({activityName:null, submenuItem: {type: 'refresh'}}),
        subMenuItems.push({activityName: 'CREATE_PROJECT_MONITORING_DOCUMENT', submenuItem: {type: 'new_modal'}})
        subMenuItems.push({activityName: null, submenuItem: { type: 'close', path: '../' }})  
      }
  ...
  editDocument(id){
    ...
    this._projectDocumentService.getItem(ProjectMonitoringStore.selectedProjectId,id).subscribe(res=>{
      if(res){
        let documentArray = [];
        let project = {
  ...
  deleteDocument(status: boolean) {
    if (status && this.popupObject.id) {
      this._projectDocumentService.delete(ProjectMonitoringStore.selectedProjectId,this.popupObject.id).subscribe(resp => {
        setTimeout(() => {
          this._utilityService.detectChanges(this._cdr);
        }, 500);
        this.pageChange(1)
