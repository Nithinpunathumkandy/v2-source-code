...
@Component({
  selector: 'app-project-closure-workflow-popup',
  templateUrl: './project-closure-workflow-popup.component.html'
})
export class ProjectClosureWorkflowPopupComponent implements OnInit {
  ...
  isUser(){
    for(let i of ProjectClosureWorkflowStore?.workflowDetails){
   ...
   checkStatus(workflow) {
    ...
    if (!workflow.workflow_status) {
      ...
      if (workflow.level == ProjectClosureStore?.indivitualProjectClosure?.next_review_user_level)
  ...
  closeWorkflowModal(){
    this._eventEmitterService.dismissprojectClosureWorkflowModal();
  }
