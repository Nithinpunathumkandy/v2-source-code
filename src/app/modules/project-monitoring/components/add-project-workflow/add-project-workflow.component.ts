...
@Component({
  selector: 'app-add-project-workflow',
  templateUrl: './add-project-workflow.component.html',
  styleUrls: ['./add-project-workflow.component.scss']
})
export class AddProjectWorkflowComponent implements OnInit {
  ...
  getModuleData() {
    this._projectWorkflowService.getModuleItems('?module_group_ids=' + this.moduleGroupId + '&is_workflow=true').subscribe((res) => {
  ...
  save(close: boolean = false) {
    ...
    if (this.form.valid) {
      ...
      if (this.form.value.id) {
        save = this._projectWorkflowService.updateItem(this.form.value.id, this.processSaveData());
      } else {
        delete this.form.value.id
        save = this._projectWorkflowService.saveItem(this.processSaveData());
      }
  ...
  cancel() {
    ...
    this.resetForm();
    this._eventEmitterService.dismissProjectWorkflowAddModal();
  }
  ngOnDestroy(){
