  ...
  save(close: boolean = false) {
    ...
    if (this.form.value) {
      ...
      if (this.form.value.id) {
        ...
        save = this._projectDocumentService.updateItem(ProjectMonitoringStore.selectedProjectId,this.form.value.id, updateParam);
      } else {
        let saveParam = {
          ...this.form.value,
          document: {...this._helperService.sortFileuploadData(fileUploadPopupStore.displayFiles, 'save')[0]}
        }
        save = this._projectDocumentService.saveProjectDocument(ProjectMonitoringStore.selectedProjectId, saveParam);
      }
