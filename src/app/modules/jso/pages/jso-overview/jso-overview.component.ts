...
@Component({
  selector: 'app-jso-overview',
  templateUrl: './jso-overview.component.html',
  styleUrls: ['./jso-overview.component.scss']
})
export class JsoOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(2000)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=2000&module_ids='+id).subscribe(res=>{
            this.loaded=true
