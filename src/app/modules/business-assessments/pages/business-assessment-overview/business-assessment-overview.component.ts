...
@Component({
  selector: 'app-business-assessment-overview',
  templateUrl: './business-assessment-overview.component.html',
  styleUrls: ['./business-assessment-overview.component.scss']
})
export class BusinessAssessmentOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(1800)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=1800&module_ids='+id).subscribe(res=>{
            this.loaded=true
