...
declare var $: any;
@Component({
  selector: 'app-control-assessment-action-plan',
  templateUrl: './control-assessment-action-plan.component.html',
  styleUrls: ['./control-assessment-action-plan.component.scss']
})
export class ControlAssessmentActionPlanComponent implements OnInit,OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.reactionDisposer = autorun(() => {
      var subMenuItems = [
        ...
        { activityName: '', submenuItem: { type: 'close',path:'/business-assessments/control-assessments/'+ControlAssessmentStore?.docversionId+'/assessments/'+ControlAssessmentDetailsStore?.assessmentId } },
      ]

      this._helperService.checkSubMenuItemPermissions(1800, subMenuItems);

      NoDataItemStore.setNoDataItems({title: "control_assessment_action_plan_title", buttonText: 'add_control_assessment_action_plan_modal'});
  ...
  addNewActionPlan(){
    ...
    this.actionPlanModalObject.controlId=ControlAssessmentDetailsStore.controlId;
    this._utilityService.detectChanges(this._cdr);
    this.openFormModal();
  }
  searchActionPlan() {
  ...
  getDetails(row)
  {
    this._router.navigateByUrl('/business-assessments/control-assessment-action-plans/'+row?.id);
  }
