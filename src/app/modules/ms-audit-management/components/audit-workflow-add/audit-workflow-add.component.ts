...
@Component({
  selector: 'app-audit-workflow-add',
  templateUrl: './audit-workflow-add.component.html',
  styleUrls: ['./audit-workflow-add.component.scss']
})
export class AuditWorkflowAddComponent implements OnInit {
  @Input('source') auditWorkflowSource: any;
  ...
  MsAuditWorkflowStore = MsAuditWorkflowStore;
  ...
  constructor(private _renderer2: Renderer2,
    ...
    private _auditWorkflowService: MsAuditWorkflowService,) { }
  ...
  ngOnInit(): void {
    ...
    if (this.auditWorkflowSource) {
      if (this.auditWorkflowSource.module_id) {
        this.moduleGroupId = this.auditWorkflowSource.module_id
      }
      if (this.auditWorkflowSource.type == 'Add')
  ...
  getModuleData() {
    this._auditWorkflowService.getModuleItems('?module_group_ids=' + this.moduleGroupId + '&is_workflow=true').subscribe((res) => {
  ...
  setFormValues() {
    if (this.auditWorkflowSource.values) {
      this.getModuleData()
      this.form.patchValue({
        'organization_ids': this.auditWorkflowSource.values.organizations,
        'division_ids': this.auditWorkflowSource.values.divisions,
        'department_ids': this.auditWorkflowSource.values.departments,
        'section_ids': this.auditWorkflowSource.values.sections,
        'sub_section_ids': this.auditWorkflowSource.values.sub_sections,
        'id': this.auditWorkflowSource.values.id,
        'title': this.auditWorkflowSource.values.title,
        'description': this.auditWorkflowSource.values.description,
        'module_id': this.auditWorkflowSource.values.module.id
      });      
      this._utilityService.detectChanges(this._cdr);
    }
  }
  ...
  save(close: boolean = false) {
    ...
    if (this.form.valid) {
      ...
      if (this.form.value.id) {
        save = this._auditWorkflowService.updateItem(this.form.value.id, this.processSaveData());
      } else {
        delete this.form.value.id
        save = this._auditWorkflowService.saveItem(this.processSaveData());
      }
  ...
  cancel() {
    ...
    this.resetForm();
    this._eventEmitterService.dismissMsAuditWorkflowAddModal();
  }
  ngOnDestroy(){
