...
@Component({
  selector: 'app-audit-plan-workflow-popup',
  templateUrl: './audit-plan-workflow-popup.component.html',
  styleUrls: ['./audit-plan-workflow-popup.component.scss']
})
export class AuditPlanWorkflowPopupComponent implements OnInit {
  ...
  isUser(){
    for(let i of AuditPlanWorkflowStore?.workflowDetails){
   ...
   checkStatus(workflow) {
    ...
    if (!workflow.workflow_status) {
      ...
      if (workflow.level == MsAuditPlansStore?.individualMsAuditPlansDetails?.next_review_user_level)
  ...
  closeWorkflowModal(){
    this._eventEmitterService.dismissMsAuditHistoryModal();
  }
