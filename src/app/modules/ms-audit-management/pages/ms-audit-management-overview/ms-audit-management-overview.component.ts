...
@Component({
  selector: 'app-ms-audit-management-overview',
  templateUrl: './ms-audit-management-overview.component.html',
  styleUrls: ['./ms-audit-management-overview.component.scss']
})
export class MsAuditManagementOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(4100)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=4100&module_ids='+id).subscribe(res=>{
            this.loaded=true
