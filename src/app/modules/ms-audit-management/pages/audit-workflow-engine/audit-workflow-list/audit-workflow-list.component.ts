...
export class AuditWorkflowListComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      if (SubMenuItemStore.clikedSubMenuItem) {
        switch (SubMenuItemStore.clikedSubMenuItem.type) {
          ...
          case "export_to_excel":
            ...
            break;
          case "search":
            MsAuditWorkflowStore.searchText = SubMenuItemStore.searchText;
            this.pageChange(1)
            break;
          case "refresh":
            MsAuditWorkflowStore.loaded = false;
            this.pageChange(1)
            break;
          default:
            break;
    ...
    this.workflowAddModalSubscription = this._eventEmitterService.msAuditWorkflowAddModal.subscribe(res => {
       this.closeFormModal();
    });
  ...
  pageChange(newPage: number = null) {
    if (newPage) MsAuditWorkflowStore.setCurrentPage(newPage);
    this._auditWorkflowService.getItems(false,'&module_group_ids=4100').pipe(takeUntil(this.strategyWorkflow$)).subscribe(() => setTimeout(() => this._utilityService.detectChanges(this._cdr), 100));
  }
  ...
  gotToAuditWorkflowDetails(id: number) {
     this._router.navigateByUrl('/ms-audit-management/ms-audit-workflows/' + id);
  }
  ...
  deleteConfirm(id: number, status) {
    ...
    this.popupObject.subtitle = 'ms_audit_workflow_delete_message';
    this._utilityService.detectChanges(this._cdr);
  ...
  ngOnDestroy() {
    ...
    MsAuditWorkflowStore.searchText = null
    this.idleTimeoutSubscription.unsubscribe()
    MsAuditWorkflowStore.unsetProjectWorkflow()
