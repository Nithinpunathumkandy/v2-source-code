...
export class AuditWorkflowDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._route.params.subscribe(params => {
      ...
      MsAuditWorkflowStore.workflowId = id;
      this.workflowId = id;
  ...
  deleteWorkflow(status: boolean) {
    if (status && this.deleteObject.id) {
      this._auditWorkflowService.deleteWorkflowSections(this.deleteObject.id, this.workflowId).subscribe(resp => {
        setTimeout(() => {
  ...
  openPopupSection(type: string, level: number) {
    let levelId = level + 1;
    MsAuditWorkflowStore.workflowPopupEnabled = true;
  ...
  workflowPopupsSections(type: string, level?) {
    ...
    this.workFlowSourceData.values = {
      workflowId: this.workflowId,
      level: MsAuditWorkflowStore.auditWorkflowDetails.workflow_items.length + 1,
      module: 'Ms_Audit',
      moduleGroupId: 4100
    }
    switch (type) {
  ...
  closeModal(popUp: number) {
    if (!MsAuditWorkflowStore.workflowPopupEnabled) {
    ...
    this.getStrategyWorkflow(this.workflowId);
  }
  ...
  ngOnDestroy() {
    MsAuditWorkflowStore.workflowId = null;
    if (this.reactionDisposer) this.reactionDisposer();
    SubMenuItemStore.makeEmpty();
    MsAuditWorkflowStore.unsetIndividualWorkflow()
