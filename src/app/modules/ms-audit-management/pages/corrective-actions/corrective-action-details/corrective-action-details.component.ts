...
export class CorrectiveActionDetailsComponent implements OnInit,OnDestroy {
  ...
  ngOnInit(): void {
      ...
      if (SubMenuItemStore.clikedSubMenuItem) {
        switch (SubMenuItemStore.clikedSubMenuItem.type) {
          case "edit_modal":
            ...
            break;
          case "delete":
            this.deleteCa(AuditCorrectiveActionStore.correctiveActionDetails.id);
            break;
          case "update_modal":
    ...
    BreadCrumbMenuItemStore.addBreadCrumbMenu({
      ...
      path:'/ms-audit-management/corrective-actions'
    });
  ...
  setSubmenu(res) {
    if (res.ms_audit_finding_corrective_action_status?.type != "closed") {
      if (AuditCorrectiveActionStore.correctiveActionDetails?.findings?.audit_id) {

        var subMenuItems = [
         // { activityName: '', submenuItem: { type: 'go_to_audit' } },
          //{ activityName: '', submenuItem: { type: 'go_to_findings' } },
          //{ activityName: '', submenuItem: { type: 'edit_modal' } },
          //{ activityName: '', submenuItem: { type: 'delete' } },
          
          { activityName: '', submenuItem: { type: 'history' } },
          { activityName: null, submenuItem: { type: 'close', path: '/ms-audit-management/corrective-actions' } },
        ]
        ...
        var subMenuItems = [
          ...
          //{ activityName: '', submenuItem: { type: 'edit_modal' } },
          // { activityName: '', submenuItem: { type: 'delete' } },
          //{ activityName: '', submenuItem: { type: 'update_modal' } },
          { activityName: '', submenuItem: { type: 'history' } },
          { activityName: null, submenuItem: { type: 'close', path: '/ms-audit-management/corrective-actions' } },
        ]
        if(AuthStore.isRoleChecking('auditee') || AuthStore.isRoleChecking('super-admin') && res.ms_audit_finding_corrective_action_status?.type != "closed")
        {
          subMenuItems.unshift({ activityName: '', submenuItem: { type: 'edit_modal' } });
          subMenuItems.unshift({ activityName: '', submenuItem: { type: 'delete' } });
        }
        if(this.isUser() || AuthStore.isRoleChecking('super-admin')&& res.ms_audit_finding_corrective_action_status?.type != "closed")
        {
          subMenuItems.unshift({ activityName: '', submenuItem: { type: 'update_modal' } })
        }
      }
  ...
  closeUpdateModal() {
    ...
    this.caUpdateObject.type = null;
    // this.pageChange();
    this.getActionPlan(AuditCorrectiveActionStore.correctiveActionDetails.id);
  }
  ...
  gotoEditPage() {
    ...
    AuditCorrectiveActionStore.auditFindingId = this.findings_id;
    AuditCorrectiveActionStore.clearDocumentDetails();
    ...
    this.correctiveActionObject.values = {
      ...
      findings: corrective_action?.ms_audit_findings,
  ...
  viewAttachments(type, document, khDocuments?) {
    switch (type) {
      case "corrective-action":
        this._fileServiceService.getFilePreview(type, AuditCorrectiveActionStore.correctiveActionDetails.ms_audit_finding_id, document.id,document.ms_audit_finding_corrective_action_id).subscribe(res => {
  ...
  openPreviewModal(type, filePreview, itemDetails, document) {
    ...
    this.previewObject.fid = AuditCorrectiveActionStore.correctiveActionDetails.ms_audit_finding_id;
  ...
  downloadDocument(type, document, docs?) {
    switch (type) {
      case "corrective-action":
        this._fileServiceService.downloadFile('corrective-action', AuditCorrectiveActionStore.correctiveActionDetails.ms_audit_finding_id, document.id,document.ms_audit_finding_corrective_action_id,document.title, document);
        break;
      case "document-version":
  ...
  ngOnDestroy() {
    ...
    AuditCorrectiveActionStore.unsetCorrectiveActionHistory();
    AuditCorrectiveActionStore.unsetDocumentDetails();
