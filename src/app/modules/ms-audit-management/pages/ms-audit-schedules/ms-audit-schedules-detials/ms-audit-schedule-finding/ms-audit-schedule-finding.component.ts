...
declare var $:any;
@Component({
  selector: 'app-ms-audit-schedule-finding',
  templateUrl: './ms-audit-schedule-finding.component.html',
  styleUrls: ['./ms-audit-schedule-finding.component.scss']
})
export class MsAuditScheduleFindingComponent implements OnInit {
  ...
  setSubmenu(){
    ...
    let subMenuItems=[];
     ...
     if(this.isAuditors()||this.isAuditLeader() || AuthStore.isRoleChecking('super-admin')){
        ...
        if(MsAuditSchedulesStore.individualMsAuditSchedulesDetails?.schedule_status?.type=='audited')
          {
  ...
  isAuditors(){
    if(MsAuditSchedulesStore.individualMsAuditSchedulesDetails?.auditors?.length>0){
      return MsAuditSchedulesStore.individualMsAuditSchedulesDetails?.auditors?.find(element=>element?.user_id==AuthStore.user?.id)
    }else{
      return false;
    }
  }
