...
@Component({
  selector: 'app-cyber-incident-workflow',
  templateUrl: './cyber-incident-workflow.component.html',
  styleUrls: ['./cyber-incident-workflow.component.scss']
})
export class CyberIncidentWorkflowComponent implements OnInit {
  ...
  isUser(){
    for(let i of CyberIncidentStore?.workflowDetails){
   ...
   checkStatus(workflow) {
    ...
    if (!workflow.workflow_status) {
      ...
      if (workflow.level == CyberIncidentStore?.cyberIncidentDetails?.next_review_user_level)
  ...
  closeWorkflowModal(){
    this._eventEmitterService.dismissCyberIncidentWorkflowModal();
  }
