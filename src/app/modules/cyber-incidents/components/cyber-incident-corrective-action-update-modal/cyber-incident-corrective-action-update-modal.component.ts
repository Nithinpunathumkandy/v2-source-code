...
@Component({
  selector: 'app-cyber-incident-corrective-action-update-modal',
  templateUrl: './cyber-incident-corrective-action-update-modal.component.html',
  styleUrls: ['./cyber-incident-corrective-action-update-modal.component.scss']
})
export class CyberIncidentCorrectiveActionUpdateModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.updateForm = this._formBuilder.group({
      ...
      cyber_incident_corrective_action_status_id: [null, Validators.required],
  ...
  saveData() {
    let saveData = {
      cyber_incident_corrective_action_status_id: this.updateForm.value.cyber_incident_corrective_action_status_id,
      ...
      amount_used: this.updateForm.value.amount_used
    }
    return saveData;
  }
  ...
  getStatus() {
    this.statuses = [];
    this._cyberIncidentCorrectiveActionStatusService.getItems().subscribe(res => {
  ...
  searchStatus(event) {
    this.statuses = [];
    this._cyberIncidentCorrectiveActionStatusService.getItems(false, '?q=' + event.term).subscribe(res => {
      this._utilityService.detectChanges(this._cdr);
    })
  }
  ...
  updateCA(close: boolean = false) {
    this.updateForm.patchValue({
      documents: this._cyberIncidentCorrectiveActionService.getDocuments()
    })
    this.formErrors = null;
    AppStore.enableLoading();
    this._cyberIncidentCorrectiveActionService.updateCorrectiveAction(CyberIncidentCorrectiveActionStore?.correctiveActionDetails.id, this.saveData()).subscribe(res => {
      AppStore.disableLoading();
      this._cyberIncidentCorrectiveActionService.getItem(CyberIncidentCorrectiveActionStore?.correctiveActionDetails.id).subscribe(response => {
        this._utilityService.detectChanges(this._cdr);
      })
  ...
  closeUpdateModal() {
    ...
    this.CyberIncidentCorrectiveActionStore.clearUpdateDocumentDetails();
    this.updateForm.reset();
    this._eventEmitterService.dismissCyberIncidentCaUpdateModal();
  }
    ...
    if (selectedFiles.length > 0) {
      ...
      Array.prototype.forEach.call(temporaryFiles, elem => {
        ...
        if (this._imageService.validateFile(file, type)) {
          ...
          CyberIncidentCorrectiveActionStore.update_document_preview_available = true;
            ...
            switch (uploadEvent.type) {
              ...
              case HttpEventType.Response:
                ...
                this._imageService.getPreviewUrl(temp.thumbnail_url).subscribe(prew => {
                  ...
                  CyberIncidentCorrectiveActionStore.update_document_preview_available = false;
  ...
  createImageFromBlob(image: Blob, imageDetails, type) {
    ...
    reader.addEventListener("load", () => {
      ...
      if (imageDetails != null)
        this._cyberIncidentCorrectiveActionService.setImageDetails(imageDetails, logo_url, type);
      else
        this._cyberIncidentCorrectiveActionService.setSelectedImageDetails(logo_url, type);
      // this.checkForFileUploadsScrollbar();
      this._utilityService.detectChanges(this._cdr);
    }, false);
  ...
  removeDocument(token) {
    CyberIncidentCorrectiveActionStore.unsetProductImageDetails('support-file', token);
    // this.checkForFileUploadsScrollbar();
    this._utilityService.detectChanges(this._cdr);
  }
  ...
  getUpdateModalDetails() {
    this.CyberIncidentCorrectiveActionStore.clearUpdateDocumentDetails();
    this.getStatus();
    this._cyberIncidentCorrectiveActionService.getItem(this.updateObject.values.ca_id).subscribe(() => setTimeout(() => this._utilityService.detectChanges(this._cdr), 100));
    this.updateForm.patchValue({
      percentage: CyberIncidentCorrectiveActionStore.correctiveActionDetails?.percentage ? CyberIncidentCorrectiveActionStore.correctiveActionDetails?.percentage : 0,
      cyber_incident_corrective_action_status_id: CyberIncidentCorrectiveActionStore?.correctiveActionDetails?.cyber_incident_corrective_action_status?.id ? CyberIncidentCorrectiveActionStore?.correctiveActionDetails?.cyber_incident_corrective_action_status?.id : null,
    })
    this._utilityService.detectChanges(this._cdr);
  }
