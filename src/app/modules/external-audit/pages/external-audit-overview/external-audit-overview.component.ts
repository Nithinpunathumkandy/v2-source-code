...
@Component({
  selector: 'app-external-audit-overview',
  templateUrl: './external-audit-overview.component.html',
  styleUrls: ['./external-audit-overview.component.scss']
})
export class ExternalAuditOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(1700)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=1700&module_ids='+id).subscribe(res=>{
            this.loaded=true
