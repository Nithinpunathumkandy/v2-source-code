...
export class CorrectiveActionUpdateModalComponent implements OnInit {
  ...
  constructor(private _helperService: HelperServiceService,
    ...
    private _formBuilder: FormBuilder, private _externalCorrectiveActionService: ExternalAuditCorrectiveActionsService) { }
  ...
  updateCA(close: boolean = false) {
    this.updateForm.patchValue({
      documents: this._externalCorrectiveActionService.getDocuments()
    ...
    this._correctiveActionService.updateCorrectiveAction(ExternalAuditCorrectiveActionStore?.correctiveActionDetails.id, this.saveData()).subscribe(res => {
      // this._findingCorrectiveActionService.getItem(this.updateObject?.id).subscribe();
      AppStore.disableLoading();
      this._correctiveActionService.getItem(ExternalAuditCorrectiveActionStore?.correctiveActionDetails.finding_id, ExternalAuditCorrectiveActionStore?.correctiveActionDetails.id).subscribe(response => {
        this._utilityService.detectChanges(this._cdr);
      })
  ...
  closeUpdateModal() {
    ...
    this.ExternalAuditCorrectiveActionStore.clearUpdateDocumentDetails();
    ...
    if (selectedFiles.length > 0) {
      ...
      Array.prototype.forEach.call(temporaryFiles, elem => {
        ...
        if (this._imageService.validateFile(file, type)) {
          ...
          ExternalAuditCorrectiveActionStore.update_document_preview_available = true;
            ...
            switch (uploadEvent.type) {
              ...
              case HttpEventType.Response:
                ...
                this._imageService.getPreviewUrl(temp.thumbnail_url).subscribe(prew => {
                  ...
                  ExternalAuditCorrectiveActionStore.update_document_preview_available = false;
  ...
  createImageFromBlob(image: Blob, imageDetails, type) {
    ...
    reader.addEventListener("load", () => {
      ...
      if (imageDetails != null)
        this._externalCorrectiveActionService.setImageDetails(imageDetails, logo_url, type);
      else
        this._externalCorrectiveActionService.setSelectedImageDetails(logo_url, type);
      // this.checkForFileUploadsScrollbar();
      this._utilityService.detectChanges(this._cdr);
    }, false);
  ...
  removeDocument(token) {
    CorrectiveActionMasterStore.unsetProductImageDetails('support-file', token);
    // this.checkForFileUploadsScrollbar();
    this._utilityService.detectChanges(this._cdr);
  }
  ...
  getUpdateModalDetails() {
    ...
    this._correctiveActionService.getItem(ExternalAuditCorrectiveActionStore.correctiveActionDetails.finding_id, this.updateObject.values.ca_id).subscribe(() => setTimeout(() => this._utilityService.detectChanges(this._cdr), 100));
    this.updateForm.patchValue({
      percentage: ExternalAuditCorrectiveActionStore?.correctiveActionDetails?.finding_corrective_action_status_updates.slice(-1)[0]?.percentage ? ExternalAuditCorrectiveActionStore?.correctiveActionDetails?.finding_corrective_action_status_updates.slice(-1)[0]?.percentage : 0,
      finding_corrective_action_status_id: ExternalAuditCorrectiveActionStore?.correctiveActionDetails?.finding_corrective_action_status_updates.slice(-1)[0]?.finding_corrective_action_status_id ? ExternalAuditCorrectiveActionStore?.correctiveActionDetails?.finding_corrective_action_status_updates.slice(-1)[0]?.finding_corrective_action_status_id : null,
    })
    this._utilityService.detectChanges(this._cdr);
  }
