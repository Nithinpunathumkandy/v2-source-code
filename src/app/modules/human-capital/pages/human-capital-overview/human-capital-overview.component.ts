...
@Component({
  selector: 'app-human-capital-overview',
  templateUrl: './human-capital-overview.component.html',
  styleUrls: ['./human-capital-overview.component.scss']
})
export class HumanCapitalOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(200)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=200&module_ids='+id).subscribe(res=>{
            this.loaded=true
