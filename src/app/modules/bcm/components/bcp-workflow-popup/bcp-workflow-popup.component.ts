...
@Component({
  selector: 'app-bcp-workflow-popup',
  templateUrl: './bcp-workflow-popup.component.html',
  styleUrls: ['./bcp-workflow-popup.component.scss']
})
export class BcpWorkflowPopupComponent implements OnInit {
  ...
  isUser(){
    for(let i of BcpStore?.bcpWorkflow){
      if(i.level == BcpStore.bcpDetails.next_review_user_level){
       var pos=i.business_continuity_plan_workflow_item_users.findIndex(e=>e.id==AuthStore.user.id)
       if(pos!=-1)
         return true;
       else
        return false
   ...
   checkStatus(workflow) {
    ...
    if (!workflow.workflow_status) {
      ...
      if (workflow.level == BcpStore.bcpDetails?.next_review_user_level)
