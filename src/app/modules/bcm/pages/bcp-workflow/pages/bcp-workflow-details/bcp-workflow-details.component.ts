...
declare var $: any;
@Component({
  selector: 'app-bcp-workflow-details',
  templateUrl: './bcp-workflow-details.component.html',
  styleUrls: ['./bcp-workflow-details.component.scss']
})
export class BcpWorkflowDetailsComponent implements OnInit {
   ...
   *
   * @memberof BcpWorkflowDetailsComponent
   */
  ngOnInit(): void {
    let id: number;
    this.route.params.subscribe(params => {
      id = +params['id']; // (+) converts string 'id' to a number
      BcpWorkflowStore.workflowId = id;
      this.workflowId = id;
  ...
  deleteWorkflow(status: boolean) {
    if (status && this.deleteObject.id) {
      this._bcpWorkflowService.deleteWorkflowSections(this.deleteObject.id,this.workflowId).subscribe(resp => {
        setTimeout(() => {
  ...
  openPopupSection(type:string,level:number){
    let levelId = level+1;
    BcpWorkflowStore.workflowPopupEnabled = true;
  ...
  workflowPopupsSections(type: string,level?) {
    ...
    this.workFlowSourceData.values = {
      workflowId: this.workflowId,
      level: BcpWorkflowStore.BcpWorkflowDetails.workflow_items.length+1,
      module:'BCM',
      moduleGroupId:2800
    }
    switch (type) {
  ...
  closeModal(popUp: number) {
    if (!BcpWorkflowStore.workflowPopupEnabled) {
   ...
   */
  ngOnDestroy() {
    BcpWorkflowStore.workflowId=null;
    ...
    BcpWorkflowStore.unsetIndividualRiskTemplate();
  }
