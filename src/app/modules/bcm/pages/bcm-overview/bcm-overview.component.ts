...
@Component({
  selector: 'app-bcm-overview',
  templateUrl: './bcm-overview.component.html',
  styleUrls: ['./bcm-overview.component.scss']
})
export class BcmOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(2800)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=2800&module_ids='+id).subscribe(res=>{
            this.loaded=true
