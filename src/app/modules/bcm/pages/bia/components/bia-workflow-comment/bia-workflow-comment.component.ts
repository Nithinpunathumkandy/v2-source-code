...
@Component({
  selector: 'app-bia-workflow-comment',
  templateUrl: './bia-workflow-comment.component.html'
})
export class BiaWorkflowCommentComponent implements OnInit {
  ...
  setTitle() {
    ...
    this.title = BiaWorkflowStore.type;
  }
  ...
  save(close: boolean = false) {
    ...
    switch (BiaWorkflowStore.type) {

      case 'approve':
        let comment = {
          comment: this.comments
        }
        save = this._biaWorkflowService.approveBia(BiaStore.selectedId,comment);
        break;
        case 'submit':
          let submitComment = {
            comment: this.comments
          }
          save = this._biaWorkflowService.approveBia(BiaStore.selectedId,submitComment);
          break;
      ...
      case 'revert':
        ...
        save = this._biaWorkflowService.revertBia(BiaStore.selectedId,data);
        break;
  ...
  closeFormModal() {
    ...
    this.level=null;
    this.comments = null;
    this._eventEmitterService.dismissBiaCommentModal()
  }
