...
@Component({
  selector: 'app-bia-workflow-popup',
  templateUrl: './bia-workflow-popup.component.html'
})
export class BiaWorkflowPopupComponent implements OnInit {
  ...
  isUser(){
    for(let i of BiaWorkflowStore?.workflowDetails){
   ...
   checkStatus(workflow) {
    ...
    if (!workflow.workflow_status) {
      ...
      if (workflow.level == BiaStore?.ImpactResult?.next_review_user_level)
  ...
  closeWorkflowModal(){
    this._eventEmitterService.dismissBiaWorkflowModal();
  }
