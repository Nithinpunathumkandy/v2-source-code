...
declare var $: any;
@Component({
  selector: 'app-bcm-risk-treatment',
  templateUrl: './bcm-risk-treatment.component.html',
  styleUrls: ['./bcm-risk-treatment.component.scss']
})
export class BcmRiskTreatmentComponent implements OnInit {
  ...
  bcmTreatmentModalSubscription: any;
  bcmTreatmentUpdateModalSubscription: any;
  ...
  constructor(
    ...
    private _bcmRiskTreatmentService: BcmRiskTreatmentService,
  ...
  ngOnInit(): void {
    ...
    BcmRiskTreatmentStore.setCurrentPage(1)
    AppStore.showDiscussion = false;

    if (!(BcmRiskAssessmentStore.bcmRiskDetails) && BcmRiskAssessmentStore.selectedId) {
    ...
    this.reactionDisposer = autorun(() => {
      ...
      if (BcmRiskAssessmentStore.bcmRiskDetails) {
        ...
        if (NoDataItemStore.clikedNoDataItem) {
          BcmRiskTreatmentStore.unsetTreatmentDetails()
          this.openFormModal();
    ...
    this.bcmTreatmentModalSubscription = this._eventEmitterService.addBcmRiskTreatmentModal.subscribe(res=>{
      this.closeFormModal();
      this._bcmRiskAssessmentService.getItem(BcmRiskAssessmentStore.selectedId).subscribe(res=>{
        this.pageChange(BcmRiskTreatmentStore.currentPage,true)
        this._utilityService.detectChanges(this._cdr);
      })
      
    });

    this.bcmTreatmentUpdateModalSubscription = this._eventEmitterService.bcmRiskTreatmentUpdateModal.subscribe(res=>{
    ...
    if (BcmRiskAssessmentStore.detailsLoaded && !BcmRiskAssessmentStore?.bcmRiskDetails?.is_analysis_performed) {
    ...
    if(BcmRiskTreatmentStore.riskTreatmentDetails&&BcmRiskTreatmentStore.riskTreatmentDetails.id){
      this.getDiscussions();
    }
    this.pageChange(1)
    setTimeout(() => {
      if (BcmRiskAssessmentStore.bcmRiskDetails?.is_analysis_performed) {
        this.getSummary()
        this._utilityService.detectChanges(this._cdr);
      }
      this.pageChange(1)
...
viewDocument(type, documents, documentFile, history_id:number=null) {
  switch (type) {
    case "risk-treatment-documents":
      this._bcmFileService
        .getFilePreview(type, BcmRiskTreatmentStore.riskTreatmentDetails.id, history_id, documentFile.id)
        .subscribe((res) => {
          var resp: any = this._utilityService.getDownLoadLink(
            res,
            documents.title
  ...
  setSubmenuitems(){
    ...
    if (BcmRiskAssessmentStore.bcmRiskDetails?.is_analysis_performed==1 && BcmRiskAssessmentStore.bcmRiskDetails?.risk_status?.id != 2) {
        if (BcmRiskAssessmentStore.isProperEditUser()&&BcmRiskTreatmentStore.riskTreatmentDetails) {
        ...
        else if(BcmRiskTreatmentStore.riskTreatmentDetails) {
          subMenuItems = [
      ...
      console.log("below",BcmRiskAssessmentStore.bcmRiskDetails)
      if (BcmRiskAssessmentStore.bcmRiskDetails?.is_corporate) {
  ...
  getSummary(){
    this._bcmRiskTreatmentService.getSummary('?risk_id=' + BcmRiskAssessmentStore.selectedId).subscribe(res => {
      this._utilityService.detectChanges(this._cdr);
    })
  }
  ...
  closeFormModal() {
    this._utilityService.detectChanges(this._cdr);
    this.getSummary()
    BcmRiskTreatmentStore.loaded = false
    this.pageChange(BcmRiskTreatmentStore.currentPage,true)
    setTimeout(() => {
  ...
  setDiscussionComment() {
    DiscussionBotStore.setDiscussionAPI(BcmRiskTreatmentStore.riskTreatmentDetails?.id + '/comments')
  }
  ...
  downloadDiscussionThumbnial() {
    DiscussionBotStore.setThumbnailDownloadAPI(BcmRiskTreatmentStore.riskTreatmentDetails?.id + '/comments/')
  }
  showThumbnailImage() {
    DiscussionBotStore.setShowThumbnailAPI(BcmRiskTreatmentStore.riskTreatmentDetails?.id + '/comments/')
  }
  getImagePrivew() {
  ...
  pageChange(newPage: number = null,refresh:boolean=false) {
    ...
    if (newPage) BcmRiskTreatmentStore.setCurrentPage(newPage);
    this._bcmRiskTreatmentService.getItems().subscribe(res => {
      this._utilityService.detectChanges(this._cdr);
      if (res['data']?.length > 0) {
        ...
        if (BcmRiskAssessmentStore.bcmRiskDetails?.is_analysis_performed)
          {
            NoDataItemStore.setNoDataItems({ title: "common_nodata_title", subtitle: "common_nodata_subtitle", buttonText: "new_treatment" });
          this._utilityService.detectChanges(this._cdr);
          }
        else
          {
  ...
  saveControlPlan(close: boolean = false) {
    ...
    this._bcmRiskTreatmentService.saveControlPlan(saveData).subscribe(res => {
      this._utilityService.detectChanges(this._cdr);
      ...
      this._risksService.getItem(BcmRiskAssessmentStore.selectedId).subscribe(response => {
  ...
  updateTreatment(close: boolean = false) {
    this.updateForm.patchValue({
      documents: this._bcmRiskTreatmentService.getDocuments(),
      amount_used: this.updateForm.value.amount_used ? parseInt(this.updateForm.value.amount_used).toFixed(2) : '',
    })

    this.formErrors = null;
    AppStore.enableLoading();
    BcmRiskTreatmentStore.setHistoryCurrentPage(1);
    this._bcmRiskTreatmentService.getUpdateData(BcmRiskTreatmentStore.riskTreatmentDetails.id).subscribe(res => {

      this._utilityService.detectChanges(this._cdr);

    })

    this._bcmRiskTreatmentService.updateTreatmentStatus(BcmRiskTreatmentStore.riskTreatmentDetails.id, this.saveData()).subscribe(res => {
      // this._bcmRiskTreatmentService.getItems().subscribe();
      // this._bcmRiskTreatmentService.getItem(BcmRiskTreatmentStore.riskTreatmentDetails.id, '?risk_id=' + BcmRiskAssessmentStore.selectedId).subscribe();
      AppStore.disableLoading();
      this._risksService.getItem(BcmRiskAssessmentStore.selectedId).subscribe(response => {
        this._utilityService.detectChanges(this._cdr);
      })
  ...
  getremainingDateChart() {
    ...
    chart.data = [
      {
        category: "Days",
        value1: BcmRiskTreatmentStore.riskTreatmentDetails?.total_days - BcmRiskTreatmentStore.riskTreatmentDetails?.days_remaining,
        value2: BcmRiskTreatmentStore.riskTreatmentDetails?.days_remaining,
      }
    ];
  ...
  getRiskTreatment(id,refresh:boolean=false) {
    if (this.activeTreatment != id || refresh) {
      ...
      this._bcmRiskTreatmentService.getItem(id, '?risk_id=' + BcmRiskAssessmentStore.selectedId).subscribe(res => {
  ...
  openUpdateModal() {
    this.BcmRiskTreatmentStore.clearDocumentDetails();
    this.getStatus();
    this.historyPageChange(1);
    this.updateForm.patchValue({
      percentage: BcmRiskTreatmentStore.riskTreatmentDetails.percentage ? BcmRiskTreatmentStore.riskTreatmentDetails.percentage : 0,
      risk_treatment_status_id: BcmRiskTreatmentStore.riskTreatmentDetails.risk_treatment_status.id,
      amount_used: BcmRiskTreatmentStore.riskTreatmentDetails.amount_used ? BcmRiskTreatmentStore.riskTreatmentDetails.amount_used : '0.00',
      comment: BcmRiskTreatmentStore.riskTreatmentDetails.comment ? BcmRiskTreatmentStore.riskTreatmentDetails.comment : '',
      id:BcmRiskTreatmentStore.riskTreatmentDetails.id?BcmRiskTreatmentStore.riskTreatmentDetails.id:''
    })
    this.updateObject.type = "Add"
    this.sliderValue = BcmRiskTreatmentStore.riskTreatmentDetails.amount_used;
    this._utilityService.detectChanges(this._cdr);
  ...
  historyPageChange(newPage: number = null) {
    if (newPage) BcmRiskTreatmentStore.setHistoryCurrentPage(newPage);
    this._bcmRiskTreatmentService.getUpdateData(BcmRiskTreatmentStore.riskTreatmentDetails?.id).subscribe(res => {
      this._utilityService.detectChanges(this._cdr);
 ...
 */
  removeDocument(token) {
    BcmRiskTreatmentStore.unsetProductImageDetails('support-file', token);
  ...
  changeControlPlan() {
    ...
    // BcmRiskAssessmentStore.bcmRiskDetails.risk_control_plan = null;
  }
 ...
 */
  ...
  createImageFromBlob(image: Blob, imageDetails, type) {
    ...
    reader.addEventListener("load", () => {
      ...
      if (imageDetails != null)
        this._bcmRiskTreatmentService.setImageDetails(imageDetails, logo_url, type);
      else
        this._bcmRiskTreatmentService.setSelectedImageDetails(logo_url, type);
      this.checkForFileUploadsScrollbar();
      this._utilityService.detectChanges(this._cdr);
    }, false);
  ...
  downloadDocument(id, filename, file_id?, doc?) {
    ...
    if (file_id) {
      this._riskManagementService.downloadFile('risk-treatment-documents', BcmRiskTreatmentStore.riskTreatmentDetails.id, id, filename, file_id, doc);
    }
  ...
  checkForFileUploadsScrollbar() {
    if (BcmRiskTreatmentStore.getDocumentDetails.length >= 5 || this.fileUploadsArray.length > 5) {
    ...
    if (selectedFiles.length > 0) {
      ...
      Array.prototype.forEach.call(temporaryFiles, elem => {
        ...
        if (this._imageService.validateFile(file, type)) {
          ...
          BcmRiskTreatmentStore.document_preview_available = true;
            ...
            switch (uploadEvent.type) {
              ...
              case HttpEventType.Response:
                ...
                this._imageService.getPreviewUrl(temp.thumbnail_url).subscribe(prew => {
                  ...
                  BcmRiskTreatmentStore.document_preview_available = false;
  ...
  */
  delete(status) {
    if (status && this.deleteObject.id) {
      ...
      if (this.deleteObject.type == 'are_you_sure_confirm' && this.deleteObject.title == 'treatment') {
        this._bcmRiskTreatmentService.closeTreatment(this.deleteObject.id).subscribe(res => {
          this._utilityService.detectChanges(this._cdr);
          this._bcmRiskTreatmentService.getItems().subscribe();
          this.getSummary()
          // this._bcmRiskTreatmentService.getItem(this.deleteObject.id);
          this.clearDeleteObject();
        })
      }
      else if (this.deleteObject.type == 'are_you_sure_confirm' && this.deleteObject.title == 'risk') {
        this._bcmRiskTreatmentService.closeRisk(this.deleteObject.id).subscribe(res => {
          this._utilityService.detectChanges(this._cdr);
        ...
        this._bcmRiskTreatmentService.delete(this.deleteObject.id).subscribe(resp => {
          setTimeout(() => {

            if (BcmRiskTreatmentStore.currentPage > 1) {
              BcmRiskTreatmentStore.currentPage = Math.ceil(BcmRiskTreatmentStore.totalItems / 15);
              this.pageChange(BcmRiskTreatmentStore.currentPage);
              this._utilityService.detectChanges(this._cdr);
            }
            else {
              this.pageChange(1);
  ...
  closeRisk() {
    this.deleteObject.id = BcmRiskAssessmentStore.selectedId;
    this.deleteObject.type = 'are_you_sure_confirm';
  ...
  getCreatedPopupDetails(user) {
    ...
    if (user) {
      ...
      this.createdDetailObject.created_at = BcmRiskTreatmentStore.riskTreatmentDetails.created_at;
  ...
  getRiskControlPlan(index?) {
    this._riskControlPlanService.getItems().subscribe(res => {
      if (index) {
        if (BcmRiskAssessmentStore.bcmRiskDetails?.risk_control_plan) {
          this.controlPlan = BcmRiskAssessmentStore.bcmRiskDetails?.risk_control_plan.id;
          this.controlData = BcmRiskAssessmentStore.bcmRiskDetails?.risk_control_plan;
          this.strategy = BcmRiskAssessmentStore.bcmRiskDetails?.risk_treatment_plan;
        }
        else {
  ...
  editTreatment() {
    BcmRiskTreatmentStore.setEditFlag();
  ...
  getResponsiblePresent(id) {
    ...
    var pos = BcmRiskAssessmentStore.bcmRiskDetails?.responsible_users?.findIndex(e => e.id == id);
    if (pos != -1)
      return true;
    else
      if (BcmRiskAssessmentStore.bcmRiskDetails?.risk_owner?.id == id)
        return true;
      else
        if (BcmRiskTreatmentStore.riskTreatmentDetails?.responsible_user?.id == id)
          return true
        else {
          var pos2 = BcmRiskTreatmentStore.riskTreatmentDetails?.watchers?.findIndex(v => v.id == id);

          if (pos2 != -1) {
            return true;
          }
          else {
            if (BcmRiskTreatmentStore.riskTreatmentDetails?.created_by?.id == id)
              return true
            else {
              if (BcmRiskAssessmentStore.bcmRiskDetails.created_by.id == id)
                return true;
              else
                return false;
            }
  ...
  ngOnDestroy() {
    ...
    this.bcmTreatmentModalSubscription.unsubscribe()
    this.bcmTreatmentUpdateModalSubscription.unsubscribe()
    // AppStore.showDiscussion = false;
