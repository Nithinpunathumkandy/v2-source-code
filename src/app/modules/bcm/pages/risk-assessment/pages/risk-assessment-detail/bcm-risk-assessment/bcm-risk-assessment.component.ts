...
declare var $: any;
@Component({
  selector: 'app-bcm-risk-assessment',
  templateUrl: './bcm-risk-assessment.component.html',
  styleUrls: ['./bcm-risk-assessment.component.scss']
})
export class BcmRiskAssessmentComponent implements OnInit {
...
downloadPdf(file){
  ...
  getAssessment(newPage: number = null) {
    var subMenuItems
    BcmRiskAssessmentStore.assessmentLoaded = false
    this._bcmRiskAssessmentService.getRiskAssessment(BcmRiskAssessmentStore.selectedId).subscribe(res=>{
      if(res.risk_analysis&&BcmRiskAssessmentStore.bcmRiskDetails?.risk_status?.id==2){
        subMenuItems = []
         subMenuItems = [
