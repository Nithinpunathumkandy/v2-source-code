...
declare var $: any;
@Component({
  selector: 'app-bcm-residual-risk',
  templateUrl: './bcm-residual-risk.component.html',
  styleUrls: ['./bcm-residual-risk.component.scss']
})
export class BcmResidualRiskComponent implements OnInit {
  ...
  constructor(private _helperService: HelperServiceService,
    ...
    private _riskAssessmentService: BcmRiskAssessmentService,
    ...
    private _riskTreatmentService:BcmRiskTreatmentService,
  ...
  ngOnInit(): void {
    ...
    if(BcmRiskAssessmentStore.detailsLoaded && BcmRiskAssessmentStore.bcmRiskDetails?.risk_status?.type=='closed'){
      NoDataItemStore.setNoDataItems({title: "risk_closed"});
      this._utilityService.detectChanges(this._cdr);
    }
    else if(BcmRiskAssessmentStore.detailsLoaded && !BcmRiskAssessmentStore.bcmRiskDetails.is_analysis_performed){
      NoDataItemStore.setNoDataItems({title: "assessment_empty_message"});
      this._utilityService.detectChanges(this._cdr);
    }
    else if(BcmRiskAssessmentStore.detailsLoaded && BcmRiskAssessmentStore.bcmRiskDetails.is_analysis_performed && !BcmRiskAssessmentStore.bcmRiskDetails?.is_residual_analysis_performed){
    ...
    this._riskManagementSettingsService.getItems().subscribe(res=>{
      ...
      if(BcmRiskAssessmentStore?.bcmRiskDetails?.is_analysis_performed==1){
        this._riskAssessmentService.getRiskAssessment(BcmRiskAssessmentStore.selectedId).subscribe(res => {
          this._utilityService.detectChanges(this._cdr);
        })
      }
    ...
    if(BcmRiskAssessmentStore.detailsLoaded && BcmRiskAssessmentStore.bcmRiskDetails.is_residual_analysis_performed){
 ...
 getScoreClass(rating) {
  ...
  getRiskVariation() {
    ...
    chart.data = [{
      "country": "",
      "value1": 0,
      "value2": BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis?.score > BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score ? BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis.score ? BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual.risk_analysis?.score : 0 : BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score,
      "value3": BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score < BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis?.score ? BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis.score ? BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual.residual_analysis?.score : 0 : BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis?.score ? BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis?.score : 0
    }];
    ...
    series2.tooltipText = BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis?.score > BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score ? "{valueY.value}(Risk Analysis)" : "{valueY.value}(Residual Analysis)";
    series2.columns.template.strokeOpacity = 0;
    series2.clustered = false;
    series2.fill = am4core.color(BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis?.score > BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score ? BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis?.color_code : BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.color_code);
    ...
    series3.tooltipText = BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score < BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis?.score ? "{valueY.value}(Residual analysis)" : "{valueY.value}(Risk analysis)";
    series3.columns.template.strokeOpacity = 0;
    series3.clustered = false;
    series3.fill = am4core.color(BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score < BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis?.score ? BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.color_code : BcmResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis?.color_code);
  ...
  getCompletedPercentageChart(){
    ...
    setInterval(function() {
      let value = BcmResidualRiskStore?.chartDetails?.mitigation_cycle_time;// set store values
