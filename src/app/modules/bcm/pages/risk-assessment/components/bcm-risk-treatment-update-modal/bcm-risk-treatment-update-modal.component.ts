...
declare var $: any;
@Component({
  selector: 'app-bcm-risk-treatment-update-modal',
  templateUrl: './bcm-risk-treatment-update-modal.component.html',
  styleUrls: ['./bcm-risk-treatment-update-modal.component.scss']
})
export class BcmRiskTreatmentUpdateModalComponent implements OnInit {
...
function applyFill(slider) {
		...
		documents.forEach(element => {
			...
			if (element.document_id) {
				...
				if (element && element.token) {
					var purl = this._bcmFileService.getThumbnailPreview('risk-treatment-documents', element.token)
					var lDetails = {
						name: element.title,
						ext: element.ext,
						size: element.size,
  ...
  updateTreatment(close:boolean=false){
    ...
    this._riskTreatmentService.updateTreatmentStatus(BcmRiskTreatmentStore.riskTreatmentDetails.id,this.saveData()).subscribe(res=>{
      this._riskTreatmentService.getItem(this.updateObject?.id,'?risk_id='+BcmRiskAssessmentStore.selectedId).subscribe();
  ...
  getUpdateModalDetails() {
    ...
    this._riskTreatmentService.getItem(this.updateObject.id,'?risk_id='+BcmRiskAssessmentStore.selectedId).subscribe(() => setTimeout(() => this._utilityService.detectChanges(this._cdr), 100));
    this.updateForm.patchValue({
      percentage: BcmRiskTreatmentStore?.riskTreatmentDetails?.percentage ? BcmRiskTreatmentStore?.riskTreatmentDetails?.percentage : 0,
      risk_treatment_status_id: BcmRiskTreatmentStore?.riskTreatmentDetails?.risk_treatment_status.id ? BcmRiskTreatmentStore?.riskTreatmentDetails?.risk_treatment_status.id : null,
  ...
  historyPageChange(newPage: number = null) {
    ...
    this._riskTreatmentService.getUpdateData(BcmRiskTreatmentStore.riskTreatmentDetails.id).subscribe(res => {
      this._utilityService.detectChanges(this._cdr);
