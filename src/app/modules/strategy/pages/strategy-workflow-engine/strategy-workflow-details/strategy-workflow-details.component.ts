...
declare var $: any;
@Component({
  selector: 'app-strategy-workflow-details',
  templateUrl: './strategy-workflow-details.component.html',
  styleUrls: ['./strategy-workflow-details.component.scss']
})
export class StrategyWorkflowDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._route.params.subscribe(params => {
      ...
      StrategyWorkflowStore.workflowId = id;
      this.workflowId = id;
  ...
  deleteWorkflow(status: boolean) {
    if (status && this.deleteObject.id) {
      this._strategyWorkflowService.deleteWorkflowSections(this.deleteObject.id, this.workflowId).subscribe(resp => {
        setTimeout(() => {
  ...
  openPopupSection(type: string, level: number) {
    let levelId = level + 1;
    StrategyWorkflowStore.workflowPopupEnabled = true;
  ...
  workflowPopupsSections(type: string, level?) {
    ...
    this.workFlowSourceData.values = {
      workflowId: this.workflowId,
      level: StrategyWorkflowStore.StrategyWorkflowDetails.workflow_items.length + 1,
      module: 'Strategy_Module',
      moduleGroupId: 3200
    }
    switch (type) {
  ...
  closeModal(popUp: number) {
    if (!StrategyWorkflowStore.workflowPopupEnabled) {
  ...
  ngOnDestroy() {
    StrategyWorkflowStore.workflowId = null;
    if (this.reactionDisposer) this.reactionDisposer();
    SubMenuItemStore.makeEmpty();
    StrategyWorkflowStore.unsetIndividualWorkflow()
