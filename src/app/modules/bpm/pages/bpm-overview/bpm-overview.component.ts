...
@Component({
  selector: 'app-bpm-overview',
  templateUrl: './bpm-overview.component.html',
  styleUrls: ['./bpm-overview.component.scss']
})
export class BpmOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(600)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=600&module_ids='+id).subscribe(res=>{
            this.loaded=true
