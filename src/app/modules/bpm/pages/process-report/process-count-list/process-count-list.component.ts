...
@Component({
	selector: 'app-process-count-list',
	templateUrl: './process-count-list.component.html',
	styleUrls: ['./process-count-list.component.scss'],
	providers: [DatePipe]
})
export class ProcessCountListComponent implements OnInit {
		...
		if (!ProcessReportStore.selectedReportObject)
			...
			this.reactionDisposer = autorun(() => {
				var subMenuItems = [
					...
					{ activityName: this.ProcessReportStore.selectedReportObject.listPermission, submenuItem: { type: 'export_to_excel' } },
					{ activityName: null, submenuItem: { type: 'close', path: `/bpm/reports/${this.riskcountType}` } },
				]
					...
					switch (SubMenuItemStore.clikedSubMenuItem.type) {
						case "export_to_excel":
							...
							if (this.filterDateObject.startDate) {
								params = `?${this.ProcessReportStore.selectedReportObject.riskItemId}=${this.id}&from=${this.filterDateObject.startDate}&to=${this.filterDateObject.endDate}`;
							}
							this._processReportDetailsService.exportToExcelList(this.ProcessReportStore.selectedReportObject, params);
							break;
						default:
							break;
					}
	...
	processDateFilterSelected(dateType: any): void {
		if (dateType === "custom") {
			...
			ProcessReportStore.processRisktlistmakeEmpty();
			this.pageChange(1);
		}
	}
	...
	pageChange(newPage: number = null) {
		...
		if (newPage) ProcessReportStore.setCurrentPage(newPage);
		this._processReportDetailsService.getProcessItemsDetails(this.id, this.ProcessReportStore.selectedReportObject, params).subscribe(() => setTimeout(() => this._utilityService.detectChanges(this._cdr), 100));
	}
