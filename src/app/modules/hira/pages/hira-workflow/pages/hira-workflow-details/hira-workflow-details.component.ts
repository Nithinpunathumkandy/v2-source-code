...
declare var $: any;
@Component({
  selector: 'app-hira-workflow-details',
  templateUrl: './hira-workflow-details.component.html',
  styleUrls: ['./hira-workflow-details.component.scss']
})
export class HiraWorkflowDetailsComponent implements OnInit {
  ...
  constructor(private _utilityService: UtilityService,
    ...
    ngOnInit(): void {
      ...
      this.route.params.subscribe(params => {
        ...
        HiraWorkflowStore.workflowId = id;
        this.workflowId = id;
    ...
    deleteWorkflow(status: boolean) {
      if (status && this.deleteObject.id) {
        this._hiraWorkflowService.deleteWorkflowSections(this.deleteObject.id,this.workflowId).subscribe(resp => {
          setTimeout(() => {
    ...
    openPopupSection(type:string,level:number){
      let levelId = level+1;
      HiraWorkflowStore.workflowPopupEnabled = true;
    ...
    workflowPopupsSections(type: string,level?) {
      ...
      this.workFlowSourceData.values = {
        workflowId: this.workflowId,
        level: HiraWorkflowStore.hiraWorkflowDetails.workflow_items.length+1,
        module:'RM',
        moduleGroupId:900
      }
      switch (type) {
    ...
    closeModal(popUp: number) {
      if (!HiraWorkflowStore.workflowPopupEnabled) {
    ...
    ngOnDestroy() {
      HiraWorkflowStore.workflowId=null;
      ...
      HiraWorkflowStore.unsetIndividualHiraTemplate();
    }
