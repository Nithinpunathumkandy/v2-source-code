...
@Component({
  selector: 'app-heat-map-division',
  templateUrl: './heat-map-division.component.html',
  styleUrls: ['./heat-map-division.component.scss']
})
export class HeatMapDivisionComponent implements OnInit {
  ...
  constructor(private _hiraHeatMapService:HiraHeatMapService,
    ...
    ngOnInit(): void {
      ...
      this.filterSubscription = this._eventEmitterService.sidebarFilterChanged.subscribe(filter => {
        this.HiraHeatMapStore.divisionLoaded = false;
      ...
      this._hiraHeatMapService.getItemByDivision().subscribe(res=>{
  ...
  pageChange(newPage: number = null,params?,num?,column?) {
    ...
    if(!params){
      params = 'inherent_risk_score='+HiraHeatMapStore.heatMapByDivisionDetails[0]?.risk_score[0]?.score+'&is_inherent=true &risk_source_ids='+HiraHeatMapStore.heatMapByDivisionDetails[0]?.risk_source_id;
      num=0;
      column=0;
    }
