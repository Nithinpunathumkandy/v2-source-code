...
@Component({
  selector: 'app-heat-map-dept',
  templateUrl: './heat-map-dept.component.html',
  styleUrls: ['./heat-map-dept.component.scss']
})
export class HeatMapDeptComponent implements OnInit {
  ...
  constructor(private _hiraHeatMapService:HiraHeatMapService,
    ...
    ngOnInit(): void {
      ...
      this.filterSubscription = this._eventEmitterService.sidebarFilterChanged.subscribe(filter => {
        this.HiraHeatMapStore.departmentLoaded = false;
      ...
      this._hiraHeatMapService.getItemByDepartment().subscribe(res=>{
    ...
    pageChange(newPage: number = null,params?,num?,column?) {
      ...
      if(!params){
        if(HiraHeatMapStore.heatMapByDepartmentDetails[0]?.risk_score[0]?.score>0)
        params = 'inherent_risk_score='+HiraHeatMapStore.heatMapByDepartmentDetails[0]?.risk_score[0]?.score+'&is_inherent=true &department_ids='+HiraHeatMapStore.heatMapByDepartmentDetails[0]?.department_id;
        num=0;
        column=0;
      }
