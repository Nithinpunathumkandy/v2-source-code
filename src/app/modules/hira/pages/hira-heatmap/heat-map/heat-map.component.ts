...
export class HeatMapComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.filterSubscription = this._eventEmitterService.sidebarFilterChanged.subscribe(filter => {
      this.HiraHeatMapStore.loaded = false;
  ...
  pageChange(newPage: number = null,params?,num?,column?,score=null) {
    ...
    if(!params){
      params = 'inherent_risk_score='+HiraHeatMapStore.HiraHeatMapDetails[0][0]?.risk_score+'&is_inherent=true'; 
      num=0;
      column=0;
    }
  ...
  getHeatMap() {
    this._hiraHeatMapService.getItem().subscribe(res=>{
