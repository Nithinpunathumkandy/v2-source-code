...
declare var $: any
@Component({
  selector: 'app-risk-treatment-details',
  templateUrl: './risk-treatment-details.component.html',
  styleUrls: ['./risk-treatment-details.component.scss']
})
export class RiskTreatmentDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._route.params.subscribe(params => {
      this.Id = params.id;
      RiskTreatmentStore.treatment_id = params.id;
      this.getDetails();
    });
    ...
    this.updateEventSubscription = this._eventEmitterService.riskTreatmentUpdateModal.subscribe(item => {
      this.closeFormModal();
    })
    ...
    // RiskTreatmentStore.treatment_id = this.Id;
...
getButtonText(text) {
  ...
  closeRiskTreatment() {
    SubMenuItemStore.closeTreatmentClicked = true;
    this.deleteObject.id = RiskTreatmentStore.riskTreatmentDetails?.id;
          ...
          if (pos2 != -1) {
            ...
            if (RiskTreatmentStore.riskTreatmentDetails?.created_by?.id == id)
              return true
                ...
                if(RiskTreatmentStore.riskTreatmentDetails?.risk?.created_by.id==id)
                return true;
                else
                return false;
              }
  ...
  getTreatmentDetails(){
    this._riskTreatmentService.getItem(this.Id, '?risk_id=' + RisksStore.riskId).subscribe(res => {
      this.setSubmenu(res);
      this._utilityService.detectChanges(this._cdr);
      ...
      if(RisksStore.isProperEditUser()){
        ...
        if(res['risk_treatment_status']?.type != 'closed'){
          if(this.getResponsiblePresent(AuthStore.user.id)){
            let subMenuItems = [
              { activityName: 'UPDATE_RISK_TREATMENT', submenuItem: { type: 'edit_modal' } },
              { activityName: 'CREATE_RISK_TREATMENT_UPDATE', submenuItem: { type: 'update_modal' } },
              { activityName: 'CLOSE_RISK_TREATMENT', submenuItem: { type: 'close_treatment' } },
          ...
          if(RisksStore.isProperEditUser()){
            if(RisksStore.individualRiskDetails.is_residual_analysis_performed!=1){
              let subMenuItems = [
                { activityName: 'CREATE_RISK_TREATMENT_UPDATE', submenuItem: { type: 'update_modal' } },
                { activityName: 'UPDATE_RISK_TREATMENT', submenuItem: { type: 'edit_modal' } },
                { activityName: 'DELETE_RISK_TREATMENT', submenuItem: { type: 'delete' } },
                // { activityName: null, submenuItem: { type: 'export_to_excel' } },
                { activityName: null, submenuItem: { type: 'close', path: "/risk-management/risk-treatments" } },
  ...
  getPopupDetails(userDetails?) {
    ...
    if (userDetails != null)
      ...
      user = RiskTreatmentStore?.riskTreatmentDetails?.responsible_user;
 ...
 */
  ...
  getCompletedChartAttendance() {
    ...
    setInterval(function () {
      let value = RiskTreatmentStore?.riskTreatmentDetails?.percentage;// set store values
  ...
  getremainingDateChartAttendance() {
    ...
    const startDate: any = new Date(RiskTreatmentStore?.riskTreatmentDetails?.start_date);
    const targetDate: any = new Date(RiskTreatmentStore?.riskTreatmentDetails?.target_date);
  ...
  openUpdateModal() {
    ...
    this._utilityService.detectChanges(this._cdr);
    this._riskTreatmentService.getItem(this.Id, '?risk_id=' + RiskTreatmentStore.riskTreatmentDetails.risk.id).subscribe(res => {
      if (RiskTreatmentStore.individualTreatmentLoaded) {
  ...
  editRiskTreatment() {
    this._riskTreatmentService.getItem(this.Id, '?risk_id=' + RiskTreatmentStore.riskTreatmentDetails.risk.id).subscribe(() => {
    ...
    if (status && this.deleteObject.id) {
      ...
      if(this.deleteObject.type=='Confirm' && this.deleteObject.title=='treatment'){
        this._riskTreatmentService.closeTreatment(this.deleteObject.id).subscribe(res => {
          SubMenuItemStore.closeTreatmentClicked = false;
          this._utilityService.detectChanges(this._cdr);
        ...
        this._riskTreatmentService.delete(this.deleteObject.id).subscribe(resp => {
          ...
          this.clearDeleteObject();
          this.router.navigateByUrl('/risk-management/risk-treatments');
  ...
  ngOnDestroy() {
    ...
    // RiskTreatmentStore.unsetIndividualTreatmentDetails();
    // RiskTreatmentStore.unsetUpdateData();
    RiskTreatmentStore.treatment_id = null;
    RiskTreatmentStore.individualTreatmentLoaded = false;
    SubMenuItemStore.closeTreatmentClicked = false;
  }
