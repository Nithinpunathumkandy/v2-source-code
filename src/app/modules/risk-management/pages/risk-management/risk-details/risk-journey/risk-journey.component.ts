...
declare var $: any;
@Component({
  selector: 'app-risk-journey',
  templateUrl: './risk-journey.component.html',
  styleUrls: ['./risk-journey.component.scss']
})
export class RiskJourneyComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.reactionDisposer = autorun(() => {
      console.log('RisksStore.componentFrom',RisksStore.componentFrom)
      let closeRiskList = '/risk-management/risks';
      let closeCorporateRisks = '/risk-management/corporate-risks';
      if (RisksStore.componentFrom == 'strategy_risk') {
         closeRiskList = '/strategy-management/risk-list';
         closeCorporateRisks = '/strategy-management/risk-list';
      }
      var subMenuItems = [];
      if (RisksStore.individualRiskDetails?.is_corporate) {
        if ((!RiskJourneyStore.individualRiskJourney?.journey_submitted_by || RiskJourneyStore.individualRiskJourney?.journey_submitted_by == null) && RiskJourneyStore.individualRiskJourney?.journey_workflow_items?.length > 0 && RisksStore.isProperUser() && RisksStore.individualRiskDetails?.is_analysis_performed == 1) {
          subMenuItems = [
            ...
            { activityName: null, submenuItem: { type: 'close', path: closeCorporateRisks } },
          ]
        }
        else {
        ...
        if ((!RiskJourneyStore.individualRiskJourney?.journey_submitted_by || RiskJourneyStore.individualRiskJourney?.journey_submitted_by == null) && RiskJourneyStore.individualRiskJourney?.journey_workflow_items?.length > 0 && RisksStore.isProperUser() && RisksStore.individualRiskDetails?.is_analysis_performed == 1) {
          ...
          subMenuItems = [
            ...
            { activityName: 'SUBMIT_RISK_JOURNEY', submenuItem: { type: 'submit' } },
            // { activityName: null, submenuItem: { type: 'export_to_excel' } },
            { activityName: null, submenuItem: { type: 'close', path: closeRiskList } },
          ]
        }
        else {
          if (this.isUser() && RiskJourneyStore.individualRiskJourney?.journey_submitted_by != null) {
  ...
  approveRisk(type?){
    ...
    //   this._risksService.getItem(RiskJourneyStore.riskId).subscribe(()=>this._utilityService.detectChanges(this._cdr))
  ...
  submitForReview() {
    this._riskJourneyWorkflowService.submitRisk(RisksStore.riskId).subscribe(res => {
      ...
      // this._utilityService.detectChanges(this._cdr);
      this._riskJourneyService.getItem(RisksStore.riskId).subscribe(res => {
