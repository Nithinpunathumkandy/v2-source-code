...
declare var $: any;
@Component({
  selector: 'app-risk-workflow-details',
  templateUrl: './risk-workflow-details.component.html',
  styleUrls: ['./risk-workflow-details.component.scss']
})
export class RiskWorkflowDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      ...
      RiskWorkflowStore.workflowId = id;
      this.workflowId = id;
  ...
  getStatusColorKey(){
    var label_color = RiskWorkflowStore.riskWorkflowDetails?.status?.label.split('-');
  ...
  openPopupSection(type:string,level:number){
    let levelId = level+1;
    RiskWorkflowStore.workflowPopupEnabled = true;
  ...
  workflowPopupsSections(type: string,level?) {
    ...
    this.workFlowSourceData.values = {
      workflowId: this.workflowId,
      level: RiskWorkflowStore.riskWorkflowDetails.workflow_items.length+1,
      module:'RM',
      moduleGroupId:900
    }
    switch (type) {
  ...
  closeModal(popUp: number) {
    if (!RiskWorkflowStore.workflowPopupEnabled) {
  ...
  ngOnDestroy() {
    RiskWorkflowStore.workflowId=null;
    ...
    RiskWorkflowStore.unsetIndividualRiskTemplate();
  }
