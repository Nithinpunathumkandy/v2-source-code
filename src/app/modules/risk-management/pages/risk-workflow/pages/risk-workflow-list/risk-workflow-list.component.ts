...
declare var $: any;
@Component({
  selector: 'app-risk-workflow-list',
  templateUrl: './risk-workflow-list.component.html',
  styleUrls: ['./risk-workflow-list.component.scss']
})
export class RiskWorkflowListComponent implements OnInit {
  ...
  RiskWorkflowStore = RiskWorkflowStore;
      ...
      if (SubMenuItemStore.clikedSubMenuItem) {
        switch (SubMenuItemStore.clikedSubMenuItem.type) {
          ...
          case "export_to_excel":
            ...
            break;
          case "search":
            RiskWorkflowStore.searchText = SubMenuItemStore.searchText;
            this.pageChange(1)
            break;
          case "refresh":
            RiskWorkflowStore.loaded = false;
            this.pageChange(1)
            break;
          default:
            break;
  ...
  pageChange(newPage:number=null){
    if (newPage) RiskWorkflowStore.setCurrentPage(newPage);
    var module_id = RiskWorkflowStore.moduleGroupId?RiskWorkflowStore.moduleGroupId:this.moduleGroupId;
    this._riskWorkflowService.getAllItems('?module_group_ids=900').subscribe(() => setTimeout(() => this._utilityService.detectChanges(this._cdr), 100));
  }
  ...
  edit(id) {
    ...
    this._riskWorkflowService.getItem(id).subscribe(res => {
      ...
      RiskWorkflowStore.addFlag=false;
      this._utilityService.detectChanges(this._cdr);
      this.openFormModal()
  ...
  gotToRiskWorkflowDetails(id: number){
    this._router.navigateByUrl('/risk-management/workflow/'+id);
  }
  ...
  ngOnDestroy() {
    ...
    RiskWorkflowStore.searchText = null;
