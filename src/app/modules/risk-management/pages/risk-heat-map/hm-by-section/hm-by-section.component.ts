...
@Component({
  selector: 'app-hm-by-section',
  templateUrl: './hm-by-section.component.html',
  styleUrls: ['./hm-by-section.component.scss']
})
export class HmBySectionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.filterSubscription = this._eventEmitterService.sidebarFilterChanged.subscribe(filter => {
      this.RiskHeatMapStore.sectionLoaded = false;
  ...
  pageChange(newPage: number = null,params?,num?,column?) {
    ...
    if(!params){
      params = 'inherent_risk_score='+RiskHeatMapStore.heatMapBySectionDetails[0]?.risk_score[0]?.score+'&is_inherent=true &section_ids='+RiskHeatMapStore.heatMapBySectionDetails[0]?.section_id;
      num=0;
      column=0;
    }
