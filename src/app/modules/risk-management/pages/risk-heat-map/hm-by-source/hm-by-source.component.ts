...
@Component({
  selector: 'app-hm-by-source',
  templateUrl: './hm-by-source.component.html',
  styleUrls: ['./hm-by-source.component.scss']
})
export class HmBySourceComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.filterSubscription = this._eventEmitterService.sidebarFilterChanged.subscribe(filter => {
      this.RiskHeatMapStore.sourceLoaded = false;
  ...
  pageChange(newPage: number = null,params?,num?,column?,selected:boolean=false) {
    ...
    if(!params){
      params = 'inherent_risk_score='+RiskHeatMapStore.heatMapBySourceDetails[0]?.risk_score[0]?.score+'&is_inherent=true &risk_source_ids='+RiskHeatMapStore.heatMapBySourceDetails[0]?.risk_source_id;
      num=0;
      column=0;
    }
