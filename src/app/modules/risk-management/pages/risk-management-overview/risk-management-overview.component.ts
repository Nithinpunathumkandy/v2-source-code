...
@Component({
  selector: 'app-risk-management-overview',
  templateUrl: './risk-management-overview.component.html',
  styleUrls: ['./risk-management-overview.component.scss']
})
export class RiskManagementOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(900)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=900&module_ids='+id).subscribe(res=>{
            this.loaded=true
