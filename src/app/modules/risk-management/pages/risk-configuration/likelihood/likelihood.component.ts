...
export class LikelihoodComponent implements OnInit {
  ...
  constructor(private _likelihoodService:LikelihoodService,
  ...
  ngOnInit(): void {
    this.reactionDisposer = autorun(() => {
      var subMenuItems = [
        { activityName: 'CREATE_RISK_MATRIX_LIKELIHOOD', submenuItem: { type: 'new_modal' } },
        { activityName: 'GENERATE_RISK_MATRIX_LIKELIHOOD_TEMPLATE', submenuItem: { type: 'template' } },
        { activityName: 'EXPORT_RISK_MATRIX_LIKELIHOOD', submenuItem: { type: 'export_to_excel' } },
        { activityName: null, submenuItem: { type: 'close', path: '/risk-management/risk-matrix' } },
      ]

      this._helperService.checkSubMenuItemPermissions(900, subMenuItems);
      NoDataItemStore.setNoDataItems({title: "likelihood_title_nodata", subtitle: 'likelihhod_subtitle_nodata',buttonText: 'add_likelihood'});
      ...
      if (SubMenuItemStore.clikedSubMenuItem) {
        switch (SubMenuItemStore.clikedSubMenuItem.type) {
          ...
          case "export_to_excel":
            ...
            break;
            case "search":
              LikelihoodStore.searchText = SubMenuItemStore.searchText;
  ...
  pageChange(newPage: number = null) {
    ...
    if (newPage) LikelihoodStore.setCurrentPage(newPage);
...
createSaveData(){
  ...
  for(let i of formData){
    ...
    delete i.error;
    if(!LikelihoodStore.individualLikelihoodDetails)
      delete i.id;
  }
...
setEditValue(){
  if(LikelihoodStore.individual_likelihood_loaded && LikelihoodStore.individualLikelihoodDetails.languages.length>0){
    for(let i of this.formNgModal){
...
getValuesForEdit(id: number){
  let languageValues = LikelihoodStore.individualLikelihoodDetails.languages.find(e=> e.id == id);
...
checkFormValid(){
  ...
  searchLikelihoodList(){
    LikelihoodStore.setCurrentPage(1);
 ...
 delete(status) {
  if (status && this.deleteObject.id) {
    ...
    this._likelihoodService.delete(this.deleteObject.id).subscribe(resp => {
      setTimeout(() => {
        this._utilityService.detectChanges(this._cdr);
        if (LikelihoodStore.currentPage > 1) {
          LikelihoodStore.currentPage = Math.ceil(LikelihoodStore.totalItems / 15);
          this.pageChange(LikelihoodStore.currentPage);
        }
      }, 500);
...
closeFormModal(){
  ...
  LikelihoodStore.unsetIndiviudalLikelihoodDetails();
  setTimeout(() => {
...
editLikelihood(id){
  ...
  ngOnDestroy(){
    ...
    LikelihoodStore.searchText=null;
