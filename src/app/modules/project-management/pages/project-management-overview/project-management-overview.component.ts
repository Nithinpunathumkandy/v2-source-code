...
@Component({
  selector: 'app-project-management-overview',
  templateUrl: './project-management-overview.component.html',
  styleUrls: ['./project-management-overview.component.scss']
})
export class ProjectManagementOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(400)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=400&module_ids='+id).subscribe(res=>{
            this.loaded=true
