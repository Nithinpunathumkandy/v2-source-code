...
@Component({
  selector: 'app-am-audit-field-work-findings',
  templateUrl: './am-audit-field-work-findings.component.html',
  styleUrls: ['./am-audit-field-work-findings.component.scss']
})
export class AmAuditFieldWorkFindingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    AmAuditFindingStore.findingComponent = 'fieldwork';

    this.filterSubscription = this._eventEmitterService.sidebarFilterChanged.subscribe(filter => {
      this.AmAuditFindingStore.loaded = false;
      this.pageChange(1);
    });
    if(AmAuditFieldWorkStore.individualAuditFieldWorkDetails?.priliminary_report_workflow_status?.workflow_status?.type=='approved'){
      var subMenuItems = [
      ...
      var subMenuItems = [
        { activityName: 'AM_AUDIT_FINDING_LIST', submenuItem: { type: 'refresh' } },
        { activityName: 'AM_AUDIT_FINDING_LIST', submenuItem: { type: 'search' } },
        { activityName: 'CREATE_AM_AUDIT_FINDING', submenuItem: { type: 'new_modal' } },
        { activityName: 'EXPORT_AM_AUDIT_FINDING', submenuItem: { type: 'export_to_excel' } },
        { activityName: null, submenuItem: { type: 'close', path: '/audit-management/am-audit-field-works' } },
    ...
    this.reactionDisposer = autorun(() => {
      ...
      NoDataItemStore.setNoDataItems({ title: "common_nodata_title", subtitle: 'common_nodata_subtitle', buttonText: 'add_audit_finding' });
      if (AuthStore.userPermissionsLoaded && !AuthStore.getActivityPermission(3900, 'CREATE_AM_AUDIT_FINDING')) {
      ...
      if (SubMenuItemStore.clikedSubMenuItem) {
        switch (SubMenuItemStore.clikedSubMenuItem.type) {
          ...
          case 'refresh':
            ...
            this._auditFindingService.exportToExcel('am_audit_ids='+AmAuditFieldWorkStore.auditFieldWorkId);
            break;
    ...
    RightSidebarLayoutStore.filterPageTag = 'am_audit_fieldwork_finding';
  ...
  pageChange(newPage: number = null) {
    ...
    this._auditFindingService.getItems(false, 'status=all&am_audit_ids=' + AmAuditFieldWorkStore.auditFieldWorkId).subscribe(res => {
      this._utilityService.detectChanges(this._cdr);
    })
  }

  gotoDetails(id) {
    this._router.navigateByUrl('/audit-management/am-audit-field-works/' + AmAuditFieldWorkStore.auditFieldWorkId + '/am-audit-findings/' + id)
  }
