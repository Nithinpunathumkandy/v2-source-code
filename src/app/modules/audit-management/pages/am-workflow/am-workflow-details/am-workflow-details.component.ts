...
@Component({
  selector: 'app-am-workflow-details',
  templateUrl: './am-workflow-details.component.html',
  styleUrls: ['./am-workflow-details.component.scss']
})
export class AmWorkflowDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    NoDataItemStore.setNoDataItems({title: "no_am_workflow_data", subtitle: 'no_am_workflow_sub_data', buttonText: 'add_workflow'});
  ...
  deleteWorkflow(status: boolean) {
    if (status && this.deleteObject.id) {
      this._amWorkflowService.deleteWorkflowSections(this.deleteObject.id,this.workflowId).subscribe(resp => {
        setTimeout(() => {
  ...
  openPopupSection(type:string,level:number){
    let levelId = level+1;
    AmWorkflowStore.workflowPopupEnabled = true;
  ...
  workflowPopupsSections(type: string,level?) {
    ...
    this.workFlowSourceData.values = {
      workflowId: this.workflowId,
      level: AmWorkflowStore.amWorkflowDetails.workflow_items.length+1,
      module:'AM',
      moduleGroupId:3900
    }
    switch (type) {
  ...
  closeModal(popUp: number) {
    if (!AmWorkflowStore.workflowPopupEnabled) {
  ...
  ngOnDestroy() {
    AmWorkflowStore.workflowId=null;
    ...
    AmWorkflowStore.unsetIndividualAmTemplate();
