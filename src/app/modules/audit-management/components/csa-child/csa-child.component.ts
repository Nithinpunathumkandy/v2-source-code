...
// import { CSAAssessmentsStore } from 'src/app/stores/business-assessments/assessments/assessments.store';
...
@Component({
  selector: 'app-csa-child',
  templateUrl: './csa-child.component.html',
  styleUrls: ['./csa-child.component.scss']
})
export class CsaChildComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.checklistIndex[CSAAssessmentsStore.currentIndex] = [];
  ...
  getChecklist(docData) {
    ...
    this._assessmentsService.getChecklist(CSAAssessmentsStore.assessmentId, docData.document_version_content_id).subscribe(res => {
...
*/
  removeDocument(token) {
    CSAAssessmentsStore.unsetChecklistImageDetails('support-file', token);
 ...
 */
  ...
  saveChecklist(checklist, docData, checklistIndex,docIndex) {
    ...
    this._assessmentsService.updateChecklist(this.form.value, checklist.id).subscribe((res: any) => {
      ...
      setTimeout(() => {
        ...
        this._assessmentsService.getChecklist(CSAAssessmentsStore.assessmentId, docData.document_version_content_id).subscribe(response => {
          this._utilityService.detectChanges(this._cdr);
  ...
  getDocArray(res, clause_number, id, save: boolean = false) {
    if (res?.documents && res?.documents.length > 0) {
      ...
      for (let i = 0; i < CSAAssessmentsStore.getDocumentDetails.length; i++) {
        if (save) {
          if ((CSAAssessmentsStore.getDocumentDetails[i].is_deleted) && CSAAssessmentsStore.getDocumentDetails[i]['clause_number'] == clause_number && CSAAssessmentsStore.getDocumentDetails[i]['checklist_id'] == id) {
            CSAAssessmentsStore.getDocumentDetails.splice(i, 1);
          }
        }
        else {
          if ((CSAAssessmentsStore.getDocumentDetails[i].is_new || CSAAssessmentsStore.getDocumentDetails[i].is_deleted) && CSAAssessmentsStore.getDocumentDetails[i]['clause_number'] == clause_number && CSAAssessmentsStore.getDocumentDetails[i]['checklist_id'] == id) {
            CSAAssessmentsStore.getDocumentDetails.splice(i, 1);
          }
        }
      }
    ...
    if (selectedFiles.length > 0) {
      ...
      Array.prototype.forEach.call(temporaryFiles, elem => {
        ...
        if (this._imageService.validateFile(file, type)) {
          ...
          CSAAssessmentsStore.document_preview_available = true;
            ...
            switch (uploadEvent.type) {
              ...
              case HttpEventType.Response:
                ...
                this._imageService.getPreviewUrl(temp.thumbnail_url).subscribe(prew => {
                  CSAAssessmentsStore.document_preview_available = false;
  ...
  */
  ...
  // Handling add and edit separately to compare avoid duplicatio when editing.
    ...
    if(BAActionPlanStore.formType=='add'){
      ...
      actionsArray.map(item=>{
        item.responsible_user_id=item.responsible_user.id,
  ...
  editActionPlan(actionPlanData){
    ...
    this.actionPlanData.values={
        ...
        responsible_user_id:actionPlanData.responsible_user
    }
