...
@Component({
  selector: 'app-auditable-item-for-findings',
  templateUrl: './auditable-item-for-findings.component.html',
  styleUrls: ['./auditable-item-for-findings.component.scss']
})
export class AuditableItemForFindingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.CommonAuditableItemSource.type == 'auditPlan') {
      ...
      this.auditableItemArray = [];
      this.auditableItemArray = JSON.parse(JSON.stringify(AuditFindingsStore.auditableItemToDisplay));
    } else {
      this.auditableItemArray = [];
    }
  ...
  pageChange(newPage: number = null) {
    ...
    this._auditFindingsService.getAuditableItem(this.CommonAuditableItemSource.values.scheduleId).subscribe(res=>{
      this._utilityService.detectChanges(this._cdr);
      if (AuditFindingsStore.auditLoaded) {
        if (this.auditableItemArray.length > 0) {
          AuditFindingsStore.auditableItems.forEach(element => {
  ...
  searchAuditableItem() {
    ...
    params='&auditable_item=true';
    if (this.searchTerm) {
      this._auditFindingsService.getItemsForAuditProgam(this.CommonAuditableItemSource.values.scheduleId,false, `&q=${this.searchTerm}` + params).subscribe(res => {
        if(res.length==0){
    ...
    if (this.auditableItemArray.length == 0) {
      ...
      if (this.auditableItemArray.length > 0) {
        ...
        AppStore.enableLoading();
          this._auditFindingsService.selectRequiredAuditableItem(this.auditableItemArray);
  ...
  sortAuditableItems() {
    ...
    if (this.auditable_item_type_id == null && this.auditable_item_category_id == null && this.riskRatingId == null) {
      ...
      this._auditFindingsService.getItemsForAuditProgam(this.CommonAuditableItemSource.values.scheduleId,false, params).subscribe(res => {
        this._utilityService.detectChanges(this._cdr);
      });
    }
  }
  ...
  checkAll(event) {
    if (event.target.checked) {
      ...
      for(let i of AuditFindingsStore.auditableItems){
        var pos = this.auditableItemArray.findIndex(e => e.id == i.id);
        if (pos == -1){
          this.auditableItemArray.push(i);}          
      }
