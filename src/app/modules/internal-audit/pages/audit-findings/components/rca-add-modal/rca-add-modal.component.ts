...
declare var $: any;
@Component({
  selector: 'app-rca-add-modal',
  templateUrl: './rca-add-modal.component.html',
  styleUrls: ['./rca-add-modal.component.scss']
})
export class RcaAddModalComponent implements OnInit , OnDestroy {
  ...
  if(this.RCASource.type=='Edit'){
    ...
    this.whyValue = AuditFindingsStore.rcaDataLength + 1;
  }
  ...
  save(close: boolean = false) {
   if(!this.rcaForm.value.id)
    ...
    if (this.rcaForm.value && this.whyValue <= 5) {
      ...
      if(this.rcaForm.value.id){
        save = this._rcaService.updateItem(AuditFindingsStore.auditFindingId, this.rcaForm.value.id, this.rcaForm.value);
      } else {
      save = this._rcaService.saveItem(AuditFindingsStore.auditFindingId, AuditFindingsStore.auditFindingId, this.rcaForm.value);}
