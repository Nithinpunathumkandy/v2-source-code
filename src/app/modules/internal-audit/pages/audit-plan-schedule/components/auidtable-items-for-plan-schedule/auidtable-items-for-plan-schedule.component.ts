...
@Component({
  selector: 'app-auidtable-items-for-plan-schedule',
  templateUrl: './auidtable-items-for-plan-schedule.component.html',
  styleUrls: ['./auidtable-items-for-plan-schedule.component.scss']
})
export class AuidtableItemsForPlanScheduleComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.CommonAuditableItemSource.type == 'auditPlan') {
      this.auditableItemArray = [];
      this.auditableItemArray = JSON.parse(JSON.stringify(AuditPlanScheduleMasterStore.auditableItemToDisplay));
    } else if(this.CommonAuditableItemSource.type == 'auditFindings') {
      this.auditableItemArray = [];
      this.auditableItemArray = JSON.parse(JSON.stringify(AuditPlanScheduleMasterStore.auditableItemToDisplay));
    } else {
      this.auditableItemArray = [];
    }
    if(this.CommonAuditableItemSource.values){
      this.auditLeaderId = this.CommonAuditableItemSource?.values?.auditLeaderId;
      this.auditPlanId = this.CommonAuditableItemSource?.values?.auditSchedules?.id?this.CommonAuditableItemSource?.values?.auditSchedules?.id:this.CommonAuditableItemSource?.values?.auditSchedules;
      }
  ...
  pageChange(newPage: number = null) {
    ...
    this._auditPlanScheduleService.getAuditableItem(this.auditPlanId,this.auditLeaderId).subscribe(res=>{
      this._utilityService.detectChanges(this._cdr);
      if (AuditPlanScheduleMasterStore.auditLoaded) {
        if (this.auditableItemArray.length > 0) {
          AuditPlanScheduleMasterStore.auditableItems.forEach(element => {
  ...
  searchAuditableItem() {
    ...
    params='&auditable_item=true';
    if (this.searchTerm) {
      this._auditPlanScheduleService.getItemsForAuditProgam(this.auditPlanId,this.auditLeaderId,false, `&q=${this.searchTerm}` + params).subscribe(res => {
        if(res.length==0){
  ...
  //   if (AuditPlanScheduleMasterStore.auditableItems.length > 0) {
  //     for (let i of AuditPlanScheduleMasterStore.auditableItems) {
    ...
    if (this.auditableItemArray.length == 0) {
      ...
      if (this.auditableItemArray.length > 0) {
        ...
        AppStore.enableLoading();
          this._auditPlanScheduleService.selectRequiredAuditableItem(this.auditableItemArray);
          this._utilityService.showSuccessMessage('Success!', 'Auditable Item Added To Your List');
          this.cancel();
      }
    }
    console.log(AuditPlanScheduleMasterStore.auditableItemToDisplay)
  }
  ...
  sortAuditableItems() {
    ...
    if (this.auditable_item_type_id) {
      if (params)
        params = params + `&auditable_item_type_ids=${this.auditable_item_type_id}&auditable_item=true`;
      else
        params = `&auditable_item_type_ids=${this.auditable_item_type_id}&auditable_item=true`;
    }

    if (this.auditable_item_category_id) {
      if (params)
        params = params + `&auditable_item_category_ids=${this.auditable_item_category_id}&auditable_item=true`;
      else
        params = `&auditable_item_category_ids=${this.auditable_item_category_id}&auditable_item=true`;
    }
    ...
    if (this.auditable_item_type_id == null && this.auditable_item_category_id == null && this.riskRatingId == null) {
      ...
      this._auditPlanScheduleService.getItemsForAuditProgam(this.auditPlanId,this.auditLeaderId,false, params).subscribe(res => {
        this._utilityService.detectChanges(this._cdr);
      });
    }
  }
