...
@Component({
  selector: 'app-non-conformity-overview',
  templateUrl: './non-conformity-overview.component.html',
  styleUrls: ['./non-conformity-overview.component.scss']
})
export class NonConformityOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(3500)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=3500&module_ids='+id).subscribe(res=>{
            this.loaded=true
