...
@Component({
  ...
  templateUrl: './isms-overview.component.html',
  styleUrls: ['./isms-overview.component.scss']
})
export class IsmsOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(3600)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=3600&module_ids='+id).subscribe(res=>{
            this.loaded=true
