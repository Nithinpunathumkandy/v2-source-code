...
@Component({
  selector: 'app-isms-count-type',
  templateUrl: './isms-count-type.component.html',
  styleUrls: ['./isms-count-type.component.scss']
})
export class IsmsCountTypeComponent implements OnInit {
  ...
  constructor(
	...
	this._route.params.subscribe(params => {
		this.reportType = params.ismscountType;
	});
  }
  ...
  ngOnInit(): void {
	...
	if (!IsmsReportStore.selectedReportObject)
			...
			IsmsReportStore.reportlistmakeEmpty();
			...
			this.reactionDisposer = autorun(() => {
				var subMenuItems = [
					...
					{ activityName: null, submenuItem: { type: 'close', path: '/isms/reports' } },
				]
				...
				if (SubMenuItemStore.clikedSubMenuItem) {
					switch (SubMenuItemStore.clikedSubMenuItem.type) {
						case "export_to_excel":
							...
							this._reportDetailsService.exportToExcel(this.IsmsReportStore.selectedReportObject, params);
							break;
						default:
							break;
					}
...
getReportList(dateObj) {
	...
	this._reportDetailsService.getItems(this.IsmsReportStore.selectedReportObject, params).subscribe(() => this._utilityService.detectChanges(this._cdr));
}
// for listing data of table dynamically

risktypeValue(row: any): string {
	const riskTypeValue = this.IsmsReportStore.selectedReportObject.riskTypeValue;
	if (this.IsmsReportStore.selectedReportObject.hasOwnProperty("riskTypeValue2")) {
		const riskTypeValue2 = this.IsmsReportStore.selectedReportObject.riskTypeValue2;
...
getRiskList(row: any) {
	IsmsReportStore.setRiskListingTableTitle(this.risktypeValue(row));
	if (this.IsmsReportStore.selectedReportObject.hasOwnProperty("analysisId") && ((this.IsmsReportStore.selectedReportObject.type === "isms-risks-by-risk-score-inherent") || (this.IsmsReportStore.selectedReportObject.type === "isms-risks-by-risk-score-residual"))) {
		const analysisId = this.IsmsReportStore.selectedReportObject.analysisId;
		this._router.navigateByUrl(`isms/reports/${this.IsmsReportStore.selectedReportObject.type}/${row[analysisId]}`);
	}
	else {
		this._router.navigateByUrl(`isms/reports/${this.IsmsReportStore.selectedReportObject.type}/${row.id}`);
	}
}
