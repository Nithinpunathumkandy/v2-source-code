...
@Component({
  ...
  templateUrl: './isms-impact.component.html',
  styleUrls: ['./isms-impact.component.scss']
})
export class IsmsImpactComponent implements OnInit {
  ...
  public Editor;
  public Config;
  constructor(private _impactService:IsmsImpactService,
  ...
  ngOnInit(): void {
    ...
    this.reactionDisposer = autorun(() => {
      var subMenuItems = [
        { activityName: 'CREATE_ISMS_RISK_MATRIX_IMPACT', submenuItem: { type: 'new_modal' } },
        { activityName: 'GENERATE_ISMS_RISK_MATRIX_IMPACT_TEMPLATE', submenuItem: { type: 'template' } },
        { activityName: 'EXPORT_ISMS_RISK_MATRIX_IMPACT', submenuItem: { type: 'export_to_excel' } },
      ...
      if (SubMenuItemStore.clikedSubMenuItem) {
        switch (SubMenuItemStore.clikedSubMenuItem.type) {
          ...
          case "export_to_excel":
            ...
            break;
            case "search":
              IsmsImpactStore.searchText = SubMenuItemStore.searchText;
  ...
  pageChange(newPage: number = null) {
    ...
    if (newPage) IsmsImpactStore.setCurrentPage(newPage);
...
createSaveData(){
  ...
  for(let i of formData){
    ...
    delete i.error;
    if(!IsmsImpactStore.individualImpactDetails)
      delete i.id;
  }
...
setEditValue(){
  if(IsmsImpactStore.individual_impact_loaded && IsmsImpactStore.individualImpactDetails.languages.length>0){
    for(let i of this.formNgModal){
...
getValuesForEdit(id: number){
  let languageValues = IsmsImpactStore.individualImpactDetails.languages.find(e=> e.id == id);
...
checkFormValid(){
  ...
  searchImpactList(){
    IsmsImpactStore.setCurrentPage(1);
 ...
 delete(status) {
  if (status && this.deleteObject.id) {
    ...
    this._impactService.delete(this.deleteObject.id).subscribe(resp => {
      setTimeout(() => {
        this._utilityService.detectChanges(this._cdr);
        if (IsmsImpactStore.currentPage > 1) {
          IsmsImpactStore.currentPage = Math.ceil(IsmsImpactStore.totalItems / 15);
          this.pageChange(IsmsImpactStore.currentPage);
        }
      }, 500);
...
closeFormModal(){
  ...
  IsmsImpactStore.unsetIndiviudalImpactDetails();
  setTimeout(() => {
...
editImpact(id){
  ...
  ngOnDestroy(){
    ...
    IsmsImpactStore.searchText=null;
