...
// import { IsmsIsmsRiskWorkflowStore } from 'src/app/stores/risk-management/risk-workflow/risk-workflow-store';

declare var $: any;
@Component({
  selector: 'app-isms-risk-workflow-details',
  templateUrl: './isms-risk-workflow-details.component.html',
  styleUrls: ['./isms-risk-workflow-details.component.scss']
})
export class IsmsRiskWorkflowDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      ...
      IsmsRiskWorkflowStore.workflowId = id;
      this.workflowId = id;
  ...
  deleteWorkflow(status: boolean) {
    if (status && this.deleteObject.id) {
      this._riskWorkflowService.deleteWorkflowSections(this.deleteObject.id,this.workflowId).subscribe(resp => {
        setTimeout(() => {
  ...
  openPopupSection(type:string,level:number){
    let levelId = level+1;
    IsmsRiskWorkflowStore.workflowPopupEnabled = true;
  ...
  workflowPopupsSections(type: string,level?) {
    ...
    this.workFlowSourceData.values = {
      workflowId: this.workflowId,
      level: IsmsRiskWorkflowStore.riskWorkflowDetails.workflow_items.length+1,
      module:'ISMS',
      moduleGroupId:3600
    }
    switch (type) {
  ...
  closeModal(popUp: number) {
    if (!IsmsRiskWorkflowStore.workflowPopupEnabled) {
  ...
  ngOnDestroy() {
    IsmsRiskWorkflowStore.workflowId=null;
    ...
    IsmsRiskWorkflowStore.unsetIndividualRiskTemplate();
  }
