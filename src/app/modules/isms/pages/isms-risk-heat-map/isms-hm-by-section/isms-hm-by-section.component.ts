...
@Component({
  selector: 'app-isms-hm-by-section',
  templateUrl: './isms-hm-by-section.component.html',
  styleUrls: ['./isms-hm-by-section.component.scss']
})
export class IsmsHmBySectionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.filterSubscription = this._eventEmitterService.sidebarFilterChanged.subscribe(filter => {
      this.IsmsRiskHeatMapStore.sectionLoaded = false;
    ...
    this._riskHeatMapService.getItemBySection().subscribe(res=>{
  ...
  pageChange(newPage: number = null,params?,num?,column?) {
    ...
    if(!params){
      params = 'inherent_risk_score='+IsmsRiskHeatMapStore.heatMapBySectionDetails[0]?.risk_score[0]?.score+'&is_inherent=true &section_ids='+IsmsRiskHeatMapStore.heatMapBySectionDetails[0]?.section_id;
      num=0;
      column=0;
    }
