...
@Component({
  selector: 'app-isms-hm-by-division',
  templateUrl: './isms-hm-by-division.component.html',
  styleUrls: ['./isms-hm-by-division.component.scss']
})
export class IsmsHmByDivisionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.filterSubscription = this._eventEmitterService.sidebarFilterChanged.subscribe(filter => {
      this.IsmsRiskHeatMapStore.divisionLoaded = false;
  ...
  pageChange(newPage: number = null,params?,num?,column?) {
    ...
    if(!params){
      params = 'inherent_risk_score='+IsmsRiskHeatMapStore.heatMapByDivisionDetails[0]?.risk_score[0]?.score+'&is_inherent=true &risk_source_ids='+IsmsRiskHeatMapStore.heatMapByDivisionDetails[0]?.risk_source_id;
      num=0;
      column=0;
    }
