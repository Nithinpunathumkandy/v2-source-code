...
@Component({
  selector: 'app-isms-risk-treatment',
  templateUrl: './isms-risk-treatment.component.html',
  styleUrls: ['./isms-risk-treatment.component.scss']
})
export class IsmsRiskTreatmentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!(IsmsRisksStore.individualRiskDetails) && IsmsRisksStore.riskId) {

      this._router.navigateByUrl('/isms/isms-risks/' + IsmsRisksStore.riskId);
    }

    //     if(IsmsRisksStore.individual_risk_loaded && IsmsRisksStore?.individualRiskDetails?.is_analysis_performed){
    ...
    this.reactionDisposer = autorun(() => {
      ...
      if (IsmsRisksStore.individualRiskDetails) {
        if (IsmsRisksStore.individualRiskDetails?.is_analysis_performed && IsmsRisksStore.individualRiskDetails?.risk_control_plan && IsmsRisksStore.individualRiskDetails?.risk_control_plan?.is_treatment == 1 && IsmsRisksStore.individualRiskDetails?.risk_status?.type != 'closed') {
          if (IsmsRisksStore.individualRiskDetails?.is_corporate) {
            ...
            if (IsmsRisksStore.isProperEditUser()) {
              ...
              subMenuItems = [
                { activityName: 'CREATE_ISMS_RISK_TREATMENT', submenuItem: { type: 'new_modal' } },
                // { activityName: 'GENERATE_ISMS_RISK_TREATMENT_TEMPLATE', submenuItem: { type: 'template' } },
                { activityName: 'EXPORT_ISMS_RISK_TREATMENT', submenuItem: { type: 'export_to_excel' } },
                { activityName: 'CLOSE_RISK', submenuItem: { type: 'close_risk' } },
        ...
        if (SubMenuItemStore.clikedSubMenuItem) {
          switch (SubMenuItemStore.clikedSubMenuItem.type) {
            ...
            case "template":
              ...
              this._ismsService.downloadFile('risk-treatment-template', null, null, fileDetails.title, null, fileDetails);
              break;
            case "export_to_excel":
              ...
              this._ismsService.downloadFile('risk-treatment-export', null, null, fileDetails.title, null, fileDetails);
              break;
    ...
    if (IsmsRisksStore.individualRiskDetails?.is_analysis_performed) {
      this._riskTreatmentService.getSummary('?risk_id=' + IsmsRisksStore.riskId).subscribe(res => {
        this._utilityService.detectChanges(this._cdr);
      })
    }
    if (IsmsRisksStore.individualRiskDetails?.is_analysis_performed) {
  ...
  setDiscussionComment() {
    if(IsmsRiskTreatmentStore.riskTreatmentDetails)
    DiscussionBotStore.setDiscussionAPI(IsmsRiskTreatmentStore.riskTreatmentDetails?.id + '/comments')
  }
  ...
  downloadDiscussionThumbnial() {
    if(IsmsRiskTreatmentStore.riskTreatmentDetails)
    DiscussionBotStore.setThumbnailDownloadAPI(IsmsRiskTreatmentStore.riskTreatmentDetails?.id + '/comments/')
  }
  showThumbnailImage() {
    if(IsmsRiskTreatmentStore.riskTreatmentDetails)
    DiscussionBotStore.setShowThumbnailAPI(IsmsRiskTreatmentStore.riskTreatmentDetails?.id + '/comments/')
  }
  getImagePrivew() {
  ...
  saveControlPlan(close: boolean = false) {
    ...
    this._riskTreatmentService.saveControlPlan(saveData).subscribe(res => {
      ...
      this._risksService.getItem(IsmsRisksStore.riskId).subscribe(response => {
  ...
  viewDocument(document, history_id) {
    this._ismsService.getFilePreview('risk-treatment', IsmsRiskTreatmentStore.riskTreatmentDetails.id, history_id, document.id).subscribe(res => {
  ...
  openPreviewModal(filePreview, itemDetails) {
    ...
    this.previewObject.uploaded_user = IsmsRiskTreatmentStore?.riskTreatmentDetails?.created_by;
    this.previewObject.uploaded_user['image_token'] = IsmsRiskTreatmentStore?.riskTreatmentDetails?.created_by?.image?.token;
    this.previewObject.created_at = IsmsRiskTreatmentStore?.riskTreatmentDetails?.created_at;
    setTimeout(() => {
  ...
  updateTreatment(close: boolean = false) {
    ...
    AppStore.enableLoading();
    IsmsRiskTreatmentStore.setHistoryCurrentPage(1);
    ...
    this._riskTreatmentService.updateTreatmentStatus(IsmsRiskTreatmentStore.riskTreatmentDetails.id, this.saveData()).subscribe(res => {
      // this._riskTreatmentService.getItems().subscribe();
      // this._riskTreatmentService.getItem(IsmsRiskTreatmentStore.riskTreatmentDetails.id, '?risk_id=' + IsmsRisksStore.riskId).subscribe();
      AppStore.disableLoading();
      this._risksService.getItem(IsmsRisksStore.riskId).subscribe(response => {
        this._utilityService.detectChanges(this._cdr);
      })
  ...
  getRiskTreatment(id) {
    if (this.activeTreatment != id) {
      ...
      this._riskTreatmentService.getItem(id, '?risk_id=' + IsmsRisksStore.riskId).subscribe(res => {
  ...
  openUpdateModal() {
    ...
    this.IsmsRiskTreatmentStore.clearDocumentDetails();
    this.getStatus();
    this.historyPageChange(1);
    // this._riskTreatmentService.getUpdateData(IsmsRiskTreatmentStore.riskTreatmentDetails.id).subscribe(res=>{
    //   if(res['data'].length>0){
    ...
    //         let docurl = this._ismsService.getThumbnailPreview('risk-treatment', i.token);
    //         let docDetails = {
    //           name: i.title,
    //           ext: i.ext,
    //           size: i.size,
    ...
    this.updateForm.patchValue({
      percentage: IsmsRiskTreatmentStore.riskTreatmentDetails.percentage ? IsmsRiskTreatmentStore.riskTreatmentDetails.percentage : 0,
      risk_treatment_status_id: IsmsRiskTreatmentStore.riskTreatmentDetails.risk_treatment_status.id,
      amount_used: IsmsRiskTreatmentStore.riskTreatmentDetails.amount_used ? IsmsRiskTreatmentStore.riskTreatmentDetails.amount_used : '0.00',
      comment: IsmsRiskTreatmentStore.riskTreatmentDetails.comment ? IsmsRiskTreatmentStore.riskTreatmentDetails.comment : ''
    })
    // if(res.length>0){
    this.sliderValue = IsmsRiskTreatmentStore.riskTreatmentDetails.amount_used;
    // this.setSlider();
    this._utilityService.detectChanges(this._cdr);
 ...
 */
  removeDocument(token) {
    IsmsRiskTreatmentStore.unsetProductImageDetails('support-file', token);
    this.checkForFileUploadsScrollbar();
    this._utilityService.detectChanges(this._cdr);
  }
  ...
  changeControlPlan() {
    ...
    // IsmsRisksStore.individualRiskDetails.risk_control_plan = null;
  }
 ...
 */
  ...
  downloadDocument(id, filename, file_id?, doc?) {
    ...
    if (file_id) {
      this._ismsService.downloadFile('risk-treatment-documents', IsmsRiskTreatmentStore.riskTreatmentDetails.id, id, filename, file_id, doc);
    }
  ...
  checkForFileUploadsScrollbar() {
    if (IsmsRiskTreatmentStore.getDocumentDetails.length >= 5 || this.fileUploadsArray.length > 5) {
  ...
  createImageUrl(type, token) {
    if (type == 'document') {
      return this._ismsService.getThumbnailPreview('risk-treatment', token);
    }
    ...
    if (selectedFiles.length > 0) {
      ...
      Array.prototype.forEach.call(temporaryFiles, elem => {
        ...
        if (this._imageService.validateFile(file, type)) {
          ...
          IsmsRiskTreatmentStore.document_preview_available = true;
            ...
            switch (uploadEvent.type) {
              ...
              case HttpEventType.Response:
                ...
                this._imageService.getPreviewUrl(temp.thumbnail_url).subscribe(prew => {
                  ...
                  IsmsRiskTreatmentStore.document_preview_available = false;
  ...
  */
        ...
        this._riskTreatmentService.delete(this.deleteObject.id).subscribe(resp => {
          setTimeout(() => {
            ...
            if (IsmsRiskTreatmentStore.currentPage > 1) {
              ...
              this.pageChange(IsmsRiskTreatmentStore.currentPage);
              this._utilityService.detectChanges(this._cdr);
            }
            else {
              this.pageChange(1);
  ...
  closeRisk() {
    this.deleteObject.id = IsmsRisksStore.riskId;
    this.deleteObject.type = 'Confirm';
  ...
  getCreatedPopupDetails(user) {
    ...
    if (user) {
      ...
      this.createdDetailObject.created_at = IsmsRiskTreatmentStore.riskTreatmentDetails.created_at;
  ...
  getRiskControlPlan(index?) {
    this._riskControlPlanService.getItems().subscribe(res => {
      if (index) {
        if (IsmsRisksStore.individualRiskDetails?.risk_control_plan) {
          this.controlPlan = IsmsRisksStore.individualRiskDetails?.risk_control_plan.id;
          this.controlData = IsmsRisksStore.individualRiskDetails?.risk_control_plan;
          this.strategy = IsmsRisksStore.individualRiskDetails?.risk_treatment_plan;
        }
        else {
  ...
  editTreatment() {
    ...
    this._router.navigateByUrl('/isms/isms-risks/' + IsmsRisksStore.riskId + '/edit-isms-risk-treatment');
  }
        ...
        if (IsmsRiskTreatmentStore.riskTreatmentDetails?.responsible_user?.id == id)
          ...
          if (pos2 != -1) {
            ...
            if (IsmsRiskTreatmentStore.riskTreatmentDetails?.created_by?.id == id)
              ...
              if (IsmsRisksStore.individualRiskDetails.created_by.id == id)
                return true;
              else
                return false;
            }
  ...
  openFormModal() {
    ...
    // }, 250);
    this._router.navigateByUrl('/isms/isms-risks/' + IsmsRisksStore.riskId + '/add-isms-risk-treatment');
  }
