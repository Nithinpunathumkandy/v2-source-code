...
// import { ResidualRiskService } from 'src/app/core/services/risk-management/risks/residual-risk/residual-risk.service';
// import { IsmsResidualRiskStore } from 'src/app/stores/risk-management/risks/residual-risk.store';
...
import { Risk } from 'src/app/core/models/internal-audit/auditable-item/auditable-item';
...
import { IsmsResidualRiskService } from 'src/app/core/services/isms/isms-risks/isms-residual-risk/isms-residual-risk.service';
...
declare var $: any;
@Component({
  selector: 'app-isms-residual-risk',
  templateUrl: './isms-residual-risk.component.html',
  styleUrls: ['./isms-residual-risk.component.scss']
})
export class IsmsResidualRiskComponent implements OnInit {
  ...
  IsmsRiskMatrixRatingLevelMasterStore = IsmsRiskMatrixRatingLevelMasterStore;
  reactionDisposer: IReactionDisposer;
  IsmsRisksStore = IsmsRisksStore;
  IsmsResidualRiskStore = IsmsResidualRiskStore;
  openAll=false;
  userDetailObject = {
    first_name: '',
    last_name: '',
  ...
  constructor(private _helperService: HelperServiceService,
    ...
    private _residualRiskService: IsmsResidualRiskService,
  ...
  ngOnInit(): void {
    IsmsImpactStore.orderBy = "desc";
    BreadCrumbMenuItemStore.displayBreadCrumbMenu = true;
    if (!IsmsRisksStore.individualRiskDetails) {
      this._router.navigateByUrl('/isms/isms-risks/' + IsmsRisksStore.riskId);
    }

    if(IsmsRisksStore.individual_risk_loaded && IsmsRisksStore.individualRiskDetails?.risk_status?.type=='closed'){
      NoDataItemStore.setNoDataItems({title: "", subtitle: "risk_closed"});
    
    }


    else if(IsmsRisksStore.individual_risk_loaded && !IsmsRisksStore.individualRiskDetails.is_analysis_performed){
      NoDataItemStore.setNoDataItems({title: "", subtitle: "assessment_empty_message"});
    
    }
    else if(IsmsRisksStore.individual_risk_loaded && IsmsRisksStore.individualRiskDetails?.is_analysis_performed && !IsmsRisksStore.individualRiskDetails?.is_residual_analysis_performed){
      ...
      this._riskTreatmentService.getItems(false,'',true).subscribe(res=>{
        if(res['data'].length>0){
          ...
          // || IsmsRisksStore.individualRiskDetails?.risk_control_plan?.id==1 || IsmsRisksStore.individualRiskDetails?.risk_control_plan?.id==4
            ...
            if(IsmsRisksStore.individualRiskDetails?.is_corporate){
              if(IsmsRisksStore.isProperEditUser){
                subMenuItems = [
                  ...
                  {activityName: null, submenuItem: {type: 'close',path:'/isms/corporate-isms-risks'}},
                  {activityName: 'CREATE_ISMS_RISK_RESIDUAL_ANALYSIS', submenuItem: {type: 'edit_modal'}},
    ...
    this._ismsRiskSettingsService.getItems().subscribe(res=>{
      ...
      IsmsImpactStore.orderBy = "desc";
    IsmsLikelihoodStore.orderBy = "desc";
    ...
    if(IsmsRisksStore.individual_risk_loaded && IsmsRisksStore.individualRiskDetails.is_residual_analysis_performed){
    ...
    this._controlEfficiencyMeasureService.getItems().subscribe(res => {
      if(!(IsmsRisksStore?.individualRiskDetails?.is_residual_analysis_performed == 1)){
        for(let i of IsmsResidualRiskStore.residualRiskDetails?.risk_processes){
        ...
        for(let j of IsmsResidualRiskStore.residualRiskDetails?.risk_controls){
          // if (i.controls?.length > 0) {
            // for (let j of i.controls) {
...
downloadPdf(file){
  ...
  getCompletedPercentageChart(){
    ...
    setInterval(function() {
      let value = IsmsResidualRiskStore?.chartDetails?.mitigation_cycle_time;// set store values
...
chart.data = [{
  "country": "",
  "value1": 0,
  "value2": IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis.score>IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score?IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis.score?IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual.risk_analysis.score:0:IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score,
  "value3": IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score<IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis.score?IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual.residual_analysis.score?IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual.residual_analysis.score:0:IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis.score?IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis.score:0
}];
...
series2.tooltipText =  IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis.score>IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score?"{valueY.value}(Risk Analysis)":"{valueY.value}(Residual Analysis)";
series2.columns.template.strokeOpacity = 0;
series2.clustered = false;
series2.fill = am4core.color(IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis.score>IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score?IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis?.color_code:IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.color_code);
...
series3.tooltipText = IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score<IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis.score?"{valueY.value}(Residual analysis)":"{valueY.value}(Risk analysis)";
series3.columns.template.strokeOpacity = 0;
series3.clustered = false;
series3.fill = am4core.color(IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.score<IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis.score?IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.residual_analysis?.color_code:IsmsResidualRiskStore?.chartDetails?.risk_analysis_vs_residual?.risk_analysis?.color_code);
...
chart.data = [
    {
        category: 'Place #1',
        first: IsmsResidualRiskStore?.chartDetails?.control_efficiency.old,
        second: IsmsResidualRiskStore?.chartDetails?.control_efficiency.new,
        third:0
...
function arrangeColumns() {
  ...
  getAnalysisScoreClass(ratingScore) {
    for (let i of IsmsRiskRatingMasterStore.allItems) {
  ...
  getRiskControls() {
    let riskData = []
    // if(IsmsRiskAssessmentStore?.riskAssessmentDetails?.risk_processes){
      for (let i of IsmsResidualRiskStore.residualRiskDetails?.risk_processes) {
        if(!i.controls || i.controls?.length==0){
  ...
  getControlArray(){
    let controlData=[];
    for (let i of IsmsResidualRiskStore.residualRiskDetails?.risk_controls) {
  ...
  submitResidualRisk() {
    ...
    this._residualRiskService.updateItem(saveData).subscribe(res => {
      ...
      // setTimeout(() => {
      // this._router.navigateByUrl('risk-management/risks/' + IsmsRisksStore.riskId + '/risk-assessment');
      //}, 100);
  ...
  setActiveControlEfficiency(measure, processIndex, controlIndex?,control?) {
    if(control){
      if (IsmsResidualRiskStore.residualRiskDetails.risk_controls[controlIndex]) {
        IsmsResidualRiskStore.residualRiskDetails.risk_controls[controlIndex].pivot.updated_control_efficiency_measure_id = measure.id;
        IsmsResidualRiskStore.residualRiskDetails.risk_controls[controlIndex].pivot.updated_control_efficiency_measure_title = measure.control_efficiency_measure_title;
        IsmsResidualRiskStore.residualRiskDetails.risk_controls[controlIndex].pivot.updated_control_efficiency_measure_score = measure.score;
      }
    }
    else{
      if (IsmsResidualRiskStore.residualRiskDetails.risk_processes[processIndex].controls[controlIndex]) {
        IsmsResidualRiskStore.residualRiskDetails.risk_processes[processIndex].controls[controlIndex].efficiency = measure;
      
    }
      }
    // else {
      // IsmsResidualRiskStore.residualRiskDetails.risk_processes[processIndex].controls[controlIndex]['activeControlEfficiency'] = measure;
  ...
  getRiskClass(type) {
    // let totalScore = IsmsRisksStore.individualRiskDetails.asset_criticality_score?.score * (this.activeImpact?.score*this.activeLikelihood?.score)
    // for (let rating of IsmsRiskRatingMasterStore.allItems) {
    //     if(rating?.score_from<=totalScore && rating?.score_to>=totalScore){
    //     if(type)
    //     return rating.isms_risk_rating_language_title;
    //     else
    //     return rating.label;
    //     }
    //   }
    ...
    let totalScore=0;
    if(IsmsRisksStore.calculationMethod?.is_addition)
    totalScore = IsmsRisksStore.individualRiskDetails?.asset_criticality_score?.score+(this.activeImpact?.score + this.activeLikelihood?.score);
    else if(IsmsRisksStore.calculationMethod?.is_multiplication)
    totalScore = IsmsRisksStore.individualRiskDetails?.asset_criticality_score?.score*(this.activeImpact?.score * this.activeLikelihood?.score);
    else if(IsmsRisksStore.calculationMethod?.is_division){
      if(this.activeImpact?.score > this.activeLikelihood?.score)
      totalScore = IsmsRisksStore.individualRiskDetails?.asset_criticality_score?.score/(this.activeImpact?.score / this.activeLikelihood?.score);
      else
      totalScore =IsmsRisksStore.individualRiskDetails?.asset_criticality_score?.score/(this.activeLikelihood?.score/this.activeImpact?.score);
    }
    else{
      if(this.activeImpact?.score > this.activeLikelihood?.score)
      totalScore = IsmsRisksStore.individualRiskDetails?.asset_criticality_score?.score-(this.activeImpact?.score - this.activeLikelihood?.score);
  ...
  getControlEfficiencyAverage() {
    let scoreTotal = 0;
    let count = 0
    // if (IsmsResidualRiskStore.residualRiskDetails?.risk_processes) {
      for (let i of IsmsResidualRiskStore.residualRiskDetails?.risk_processes) {
        if(i.controls?.length>0){
          for (let j of i.controls) {
