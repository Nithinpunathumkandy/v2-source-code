...
@Component({
  selector: 'app-isms-risk-assessment',
  templateUrl: './isms-risk-assessment.component.html',
  styleUrls: ['./isms-risk-assessment.component.scss']
})
export class IsmsRiskAssessmentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.reactionDisposer = autorun(() => {
      if (IsmsRisksStore.individualRiskDetails?.is_corporate) {
        if (IsmsRisksStore.individualRiskDetails?.is_residual_analysis_performed || IsmsRisksStore.individualRiskDetails?.risk_status?.type == 'closed' || !IsmsRisksStore.isProperEditUser()) {
          var subMenuItems = [
            { activityName: null, submenuItem: { type: 'export_to_excel' } },
            { activityName: null, submenuItem: { type: 'close', path: '/isms/corporate-risks' } },
          ]
        }
        else {
          subMenuItems = [
            { activityName: null, submenuItem: { type: 'export_to_excel' } },
            { activityName: null, submenuItem: { type: 'close', path: '/isms/corporate-risks' } },
            { activityName: 'CREATE_ISMS_RISK_ANALYSIS', submenuItem: { type: 'edit_modal', path: null } },
    ...
    this._controlEfficiencyMeasureService.getItems().subscribe(res => {
      if (!(IsmsRisksStore?.individualRiskDetails?.is_analysis_performed == 1)) {
        for (let i of IsmsRisksStore.individualRiskDetails?.processes) {
        ...
        for (let j of IsmsRisksStore.individualRiskDetails?.controls) {
          // if (i.controls?.length > 0) {
          // for (let j of i.controls) {
          j['activeControlEfficiency'] = res['data'][0];
          // }
...
downloadPdf(file){
  ...
  setSelectedControl(pIndex,cIndex,event){
      ...
      IsmsRisksStore.individualRiskDetails.processes[pIndex].controls[cIndex]['active']=event.target.checked;
    
  }

  selectedControl(pIndex,cIndex){
    
      if(IsmsRisksStore.individualRiskDetails.processes[pIndex].controls[cIndex]['active'] &&IsmsRisksStore.individualRiskDetails.processes[pIndex].controls[cIndex]['active']==true){
  ...
  // setControls() {
  //   for (let j of IsmsRisksStore.individualRiskDetails.processes) {
  ...
  setActiveControlEfficiency(measure, processIndex, controlIndex?, control?) {
    if (control) {
      if (IsmsRisksStore.individualRiskDetails?.is_analysis_performed) {
        if (IsmsRiskAssessmentStore?.riskAssessmentDetails?.risk_controls[controlIndex]?.pivot?.old_control_efficiency_measure_id) {
          IsmsRiskAssessmentStore.riskAssessmentDetails.risk_controls[controlIndex].pivot.old_control_efficiency_measure_id = measure.id;
          IsmsRiskAssessmentStore.riskAssessmentDetails.risk_controls[controlIndex].pivot.old_control_efficiency_measure_title = measure.control_efficiency_measure_title;
          IsmsRiskAssessmentStore.riskAssessmentDetails.risk_controls[controlIndex].pivot.old_control_efficiency_measure_score = measure.score;
        ...
        IsmsRisksStore.individualRiskDetails.controls[controlIndex]['activeControlEfficiency'] = measure;
      }
    }
    else {
      if (IsmsRisksStore.individualRiskDetails?.is_analysis_performed) {
        if (IsmsRiskAssessmentStore?.riskAssessmentDetails?.risk_processes[processIndex]?.controls[controlIndex]?.efficiency) {
          IsmsRiskAssessmentStore.riskAssessmentDetails.risk_processes[processIndex].controls[controlIndex].efficiency = measure;
        }

      }
      else {

        IsmsRisksStore.individualRiskDetails.processes[processIndex].controls[controlIndex]['activeControlEfficiency'] = measure;
    ...
    if (control) {
      ...
      if (data.controls?.length > 0) {
        for (let i of data.controls) {
          if((i.active && i.active==true) || IsmsRisksStore.individualRiskDetails?.is_analysis_performed ){
            count++;
  ...
  submitAssessment() {
    ...
    this._riskAssessmentService.updateItem(saveData).subscribe(res => {
      ...
      IsmsRisksStore.individualRiskDetails.is_analysis_performed = 1;
  ...
  getControlArray() {
    let controlData = [];
    if (IsmsRisksStore.individualRiskDetails?.is_analysis_performed) {
      if (IsmsRiskAssessmentStore.loaded) {
        for (let i of IsmsRiskAssessmentStore.riskAssessmentDetails?.risk_controls) {
      ...
      if (IsmsRisksStore.individualRiskDetails.controls?.length > 0) {
        for (let i of IsmsRisksStore.individualRiskDetails.controls) {
  ...
  getRiskControls() {
    ...
    if (IsmsRisksStore.individualRiskDetails?.is_analysis_performed) {
      ...
      if (IsmsRisksStore.individualRiskDetails.processes.length > 0) {
        for (let i of IsmsRisksStore.individualRiskDetails.processes) {
          if (!i.controls || i.controls?.length == 0) {
  ...
  getControlEfficiencyAverage() {
    ...
    if (IsmsRisksStore.individualRiskDetails?.is_analysis_performed) {
      ...
      // if(control){
      if (IsmsRisksStore.individualRiskDetails?.controls) {
        for (let i of IsmsRisksStore.individualRiskDetails?.controls) {
      ...
      // }
      // else{
      if (IsmsRisksStore.individualRiskDetails?.processes) {
        for (let i of IsmsRisksStore.individualRiskDetails?.processes) {
          if (i.controls?.length > 0) {
            for (let j of i.controls) {
              if((j['active'] && j['active']==true)){
