...
declare var $: any;
@Component({
  selector: 'app-kh-overview',
  templateUrl: './kh-overview.component.html',
  styleUrls: ['./kh-overview.component.scss']
})
export class KhOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(700)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=700&module_ids='+id).subscribe(res=>{
            this.loaded=true
