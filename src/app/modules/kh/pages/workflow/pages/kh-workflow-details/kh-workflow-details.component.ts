...
// import { WorkFlowStore } from 'src/app/stores/internal-audit/audit-workflow/audit-workflow-store';
import { WorkFlowStore } from 'src/app/stores/knowledge-hub/work-flow/workFlow.store';

declare var $: any;

@Component({
  selector: 'app-kh-workflow-details',
  templateUrl: './kh-workflow-details.component.html',
  styleUrls: ['./kh-workflow-details.component.scss']
})
export class KhWorkflowDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      ...
      WorkFlowStore.workflowId = this.workflowId = id
  ...
  openPopupSection(type:string,level:number){
    let levelId = level+1;
    WorkFlowStore.workflowPopupEnabled = true;
  ...
  deleteWorkflow(status: boolean) {
    if (status && this.popupObject.id) {
      this._workFlowService.deleteWorkflowSections(this.popupObject.id,this.workflowId).subscribe(resp => {
        setTimeout(() => {
  ...
  closeModal(popUp: number) {
    if (!WorkFlowStore.workflowPopupEnabled) {
