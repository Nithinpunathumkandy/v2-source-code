...
@Component({
  selector: 'app-cr-notes',
  templateUrl: './cr-notes.component.html',
  styleUrls: ['./cr-notes.component.scss']
})
export class CrNotesComponent implements OnInit {
    ...
    save(close: boolean = false) {
      ...
      if (this.notesForm.value) {
        ...
        if (this.notesForm.value.id) {
                ...
                save = this._changeRequestNoteService.updateItem(this.notesForm.value.id, this.setFormData('Update', this.NotesParams.type));
           
          
        } else {                    
              save = this._changeRequestNoteService.saveItem(this.setFormData('Save',this.NotesParams.type));
  ...
  setFormData(type, modalType?) {
    ...
    if (type == 'Update') {
      ...
      data.document_change_request_id = changeRequestStore.documentId
        return data
    }
