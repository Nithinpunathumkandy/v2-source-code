...
declare var $: any;
@Component({
  selector: 'app-kh-template-external',
  templateUrl: './kh-template-external.component.html',
  styleUrls: ['./kh-template-external.component.scss']
})
export class KhTemplateExternalComponent implements OnInit {
  ...
  submitPopupSubscription: Subscription
  ...
  createImageUrl(type, token, h?, w?) {    
    return this._templateFileService.getThumbnailPreview(type,token,h,w);
  }
  ...
  setRequestContentAccordion(type, index: number, id, initial: boolean = false) {
    ...
    this._templateContentService.getItemById(id).subscribe()
  ...
  getChangeRequestDetails() {
    this._khTemplatesService.getItemById(changeRequestStore.documentId).subscribe(res => {
      this.addSubmenu();
      res['type'] = 'Request'
      this.dataArray = [res]
      this._utilityService.detectChanges(this._cdr);
