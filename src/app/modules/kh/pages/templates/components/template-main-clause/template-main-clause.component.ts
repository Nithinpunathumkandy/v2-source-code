...
@Component({
  selector: 'app-template-main-clause',
  templateUrl: './template-main-clause.component.html',
  styleUrls: ['./template-main-clause.component.scss']
})
export class TemplateMainClauseComponent implements OnInit {
  ...
  constructor(
    ...
    private _templateContent: TemplateContentService,
  ) {
    ...
    if (this.contentForm.value) {
      if (!this.clauseError) {
        ...
        if (this.contentForm.value.id) {
          ...
          save = this._templateContent.updateItem(this.contentForm.value.id, this.setPostParams(this.sourceParams.modalType, 'Update'));


        } else {
          this.contentForm.removeControl('id')
          save = this._templateContent.saveItem(this.setPostParams(this.sourceParams.modalType, 'Save'));
  ...
  setPostParams(modalType, type) {
    ...
    if (type == 'Save') {
      postParams = {
        ...
        order: this.order,
        document_template_content_id: this.templateContentId,
  ...
  setDataforEdit() {
    ...
    this.templateContentId = data.templateContentId
    this.contentForm.patchValue({
      id: data.id,
      title: data.title,
  ...
  setParams() {
    ...
    if (sourceData.templateContentId == null) {
      let data = ContentStore.ContentList
      this.templateContentId = sourceData.templateContentId;
      ...
      if (sourceData.children) {
        ...
        this._templateContent.getItemById(this.templateContentId);
        let data: any = ContentStore.ContentIndividualList;
  ...
  verifyParentContent(type, contentId) {
    this._templateContent.getItemById(contentId).subscribe(res => {
