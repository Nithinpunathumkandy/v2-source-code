...
@Component({
  selector: 'app-score-review-submit',
  templateUrl: './score-review-submit.component.html',
  styleUrls: ['./score-review-submit.component.scss']
})
export class ScoreReviewSubmitComponent implements OnInit {
  ...
  closeFormModal(close:boolean=false) {
    this.resetForm();
    this._eventEmitterService.dismissKpiScoreRivewSubmitModal(close);
  }
  ...
  save(close: boolean = false) {
    if (this.form.value) {
      ...
      if (this.form.value) {
        save = this._kpiScoreService.review(this.source.id, this.saveData);
      }
      save.subscribe(
        (res: any) => {
          this.resetForm();
