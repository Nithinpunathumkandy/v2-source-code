...
@Component({
  selector: 'app-score-revert',
  templateUrl: './score-revert.component.html',
  styleUrls: ['./score-revert.component.scss']
})
export class ScoreRevertComponent implements OnInit {
  ...
  closeFormModal(close:boolean=false) {
    this.resetForm();
    this._eventEmitterService.dismisskpiScoreRevertModal(close);
  }
  ...
  save(close: boolean = false) {
    ...
    if (this.form.value) {
      ...
      if (this.form.value) {
        save = this._kpiScoreService.sentBack(this.source.id, this.saveData);
      }
      save.subscribe(
        (res: any) => {
          this.resetForm();
