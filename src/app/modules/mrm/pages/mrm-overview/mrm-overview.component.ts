...
@Component({
  selector: 'app-mrm-overview',
  templateUrl: './mrm-overview.component.html',
  styleUrls: ['./mrm-overview.component.scss']
})
export class MrmOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(1600)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=1600&module_ids='+id).subscribe(res=>{
            this.loaded=true
