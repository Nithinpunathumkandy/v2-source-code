...
declare var $: any;
@Component({
  selector: 'app-mrm-workflow-details',
  templateUrl: './mrm-workflow-details.component.html',
  styleUrls: ['./mrm-workflow-details.component.scss']
})
export class MrmWorkflowDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      ...
      MrmWorkflowStore.workflowId = this.workflowId = id
  ...
  deleteWorkflow(status: boolean) {
    if (status && this.deleteObject.id) {
      this._mrmWorkflowService.deleteWorkflowSections(this.deleteObject.id,this.workflowId).subscribe(resp => {
        setTimeout(() => {
  ...
  openPopupSection(type:string,level:number){
    let levelId = level+1;
    MrmWorkflowStore.workflowPopupEnabled = true;
  ...
  closeModal(popUp: number) {
    if (!MrmWorkflowStore.workflowPopupEnabled) {
  ...
  ngOnDestroy() {
    ...
    this.showDiv=false;
    MrmWorkflowStore.unSetIndividualMrmTemplate();
  }
}
