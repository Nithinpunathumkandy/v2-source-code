...
declare var $: any;
@Component({
  selector: 'app-compliance-register-action-plans',
  templateUrl: './compliance-register-action-plans.component.html',
  styleUrls: ['./compliance-register-action-plans.component.scss']
})
export class ComplianceRegisterActionPlansComponent implements OnInit {
  ...
  pageChange(newPage: number = null) {
    ...
    this._compianceRegisterActionPlanService.getItems(false, `compliance_register_ids=${ComplianceRegisterStore.complianceRegisterId}`).subscribe((res) => {
      if (res?.data[0]) {
 ...
 // Clear previous data from store
    this._compianceRegisterActionPlanService.getItem(id).subscribe(res => {
      this.actionPlanId = res.id;
      this._utilityService.detectChanges(this._cdr);
    })
    this._compianceRegisterActionPlanService.setSelected(id);
  }

  getDaysRemaining() {

    let startDate = new Date(ComplianceRegisterActionPlanStore.complianceRegisterActionPlanDetails?.target_date);
  ...
  editActionPlan() {
    if (ComplianceRegisterActionPlanStore.complianceRegisterActionPlanDetails?.id) {
      this.actionPlanData.values = {
        ...
        // checklistId:ComplianceRegisterActionPlanStore.complianceRegisterActionPlanDetails.business_assessment_document_content_checklist.id
      }
      this._utilityService.detectChanges(this._cdr);
      this.openActionPlanForm();
    }
  ...
  // }
  openActionPlanForm(){
    ComplianceRegisterActionPlanStore.actionPlanUpdateModal = true;
  ...
  closeActionPlanForm(){
    ...
    ComplianceRegisterActionPlanStore.actionPlanUpdateModal = false;
  ...
  deleteActionPlans(status: boolean) {
    ...
    if (status && this.popupObject.id) {
      this._compianceRegisterActionPlanService.deleteSingleActionPlan(this.popupObject.id, `compliance_register_ids=${ComplianceRegisterStore.complianceRegisterId}`).subscribe(resp => {
        setTimeout(() => {
          this._utilityService.detectChanges(this._cdr);
        }, 500);
    ...
    switch (type) {
      case "compliance-status-document":
        this._complianceRegisterService
          .getFilePreview(type, ComplianceRegisterActionPlanStore.complianceRegisterActionPlanDetails.id, documentFile.id, documents.meeting_action_plan_update_id)
          .subscribe((res) => {
  ...
  ngOnDestroy() {
    ...
    ComplianceRegisterActionPlanStore.searchText = null;
    ...
    RightSidebarLayoutStore.showFilter = false;
    ComplianceRegisterActionPlanStore.clearComplianceRegisterActionPlans();//action plan list
