...
export class ActionPlanStatusUpdateModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form = this._formBuilder.group({
      ...
      document_action_plan_status_id: [null, Validators.required],
      comment: [''],
      documents: [[], '']
    });
  ...
  setPercentage(percentage) {
    if (percentage == '100%')
      this.form.controls['document_action_plan_status_id'].setValue(3);
    else if (percentage == '0%')
      this.form.controls['document_action_plan_status_id'].setValue(1);
    else
      this.form.controls['document_action_plan_status_id'].setValue(2);
  }

  setFormData(data) {
    this.form.patchValue({
      percentage: data.completion ? data.completion + '%' : '0%',
      document_action_plan_status_id: data?.document_action_plan_status ? data.document_action_plan_status.id : 1,
    })
  }
  ...
  getStatus() {
    if (!(this.statuses.length > 0))
      ...
      this._complianceActionPlanStatusService.getAllItems().subscribe(res => {
        if (res) {
          this.loader = true;
  ...
  getSaveData() {
    ...
    this.saveData = {
      ...
      document_action_plan_status_id: this.form.value.document_action_plan_status_id ? this.form.value.document_action_plan_status_id : null,
  ...
  save(close: boolean = false) {
    if (this.form.value) {
      ...
      if (this.form.value) {
        save = this._complianceRegisterActionPlanService.updateActionPlanStatus(this.source.id, this.saveData);
      }
      save.subscribe(
        (res: any) => {
          this.resetForm();
