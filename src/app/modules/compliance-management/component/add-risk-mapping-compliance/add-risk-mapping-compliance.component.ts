...
@Component({
  selector: 'app-add-risk-mapping-compliance',
  templateUrl: './add-risk-mapping-compliance.component.html',
  styleUrls: ['./add-risk-mapping-compliance.component.scss']
})
export class AddRiskMappingComplianceComponent implements OnInit {
  ...
  pageChange(newPage: number = null) {
    ...
    this._riskService.getItems(false, this.generateSortParams()).subscribe((res) => {
      setTimeout(() => {
  ...
  save(close: boolean = false) {
    ...
    let title = this.riskModalTitle?.component ? this.riskModalTitle?.component : 'item'
    if (this.selectedRisk.length > 0) this._utilityService.showSuccessMessage('risk_selected', 'Selected risks are mapped with the ' +this._helperService.translateToUserLanguage(title)+ ' successfully!');
  ...
  generateSortParams() {
    ...
    if (this.risk_type_id) {
      params = (params == '') ? params + `risk_type_ids=${this.risk_type_id}` : params + `&risk_type_ids=${this.risk_type_id}`;
    }
    if (this.risk_category_id) {
      params = (params == '') ? params + `risk_category_ids=${this.risk_category_id}` : `&risk_category_ids=${this.risk_category_id}`;
    }
    if (this.searchTerm)
  ...
  checkedAll(){
    if(this.selectedRisk.length > 0){
       if(this.selectedRisk.length == RisksStore.totalItems){
         return true
        }else{
         return false
        }
