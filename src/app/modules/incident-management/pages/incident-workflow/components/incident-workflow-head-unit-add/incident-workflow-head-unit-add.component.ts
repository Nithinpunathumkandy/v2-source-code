...
@Component({
  selector: 'app-incident-workflow-head-unit-add',
  templateUrl: './incident-workflow-head-unit-add.component.html',
  styleUrls: ['./incident-workflow-head-unit-add.component.scss']
})
export class IncidentWorkflowHeadUnitAddComponent implements OnInit {
  ...
  save(close: boolean = false) {
    ...
    if (this.form.value) {
      ...
      if (this.form.value.id) {
        ...
        save = this._incidentWorkflowService.saveHeadOfUnitAdd(saveData,this.WorkFlowSource.values ? this.WorkFlowSource.values.workflowId : '');
      }
  ...
  close() {
    this.resetForm();
    this._eventEmitterService.dismissIncidentWorkflowHeadUnitAddModal()
  }
  ...
  searchDepartment(event) {
    ...
    if (this.form.get('organization_ids').value) {
      ...
      this._departmentService.getItems(false, '&q=' + event.term).subscribe((res) => {
        this._utilityService.detectChanges(this._cdr);
      });
    }
  }
