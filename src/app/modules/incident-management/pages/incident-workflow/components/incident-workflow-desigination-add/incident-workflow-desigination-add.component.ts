...
@Component({
  selector: 'app-incident-workflow-desigination-add',
  templateUrl: './incident-workflow-desigination-add.component.html',
  styleUrls: ['./incident-workflow-desigination-add.component.scss']
})
export class IncidentWorkflowDesiginationAddComponent implements OnInit {
  ...
  searchDesignation(e) {
    ...
    this._designationService.getItems(false, '&q=' + e.term + params).subscribe((res) => {
      this._utilityService.detectChanges(this._cdr);
    });
  }
  }
  ...
  save(close: boolean = false) {
    ...
    if (this.form.value) {
      ...
      if (this.form.value.id) {
        ...
        save = this._incidentWorkflowService.saveDesignationAdd(saveData,this.WorkFlowSource.values ? this.WorkFlowSource.values.workflowId : '');
      }
  ...
  close() {
    this.resetForm();
    this._eventEmitterService.dismissIncidentWorkflowDesignationAddModal()
  }
