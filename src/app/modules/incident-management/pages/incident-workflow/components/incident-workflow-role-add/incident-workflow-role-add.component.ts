...
@Component({
  selector: 'app-incident-workflow-role-add',
  templateUrl: './incident-workflow-role-add.component.html',
  styleUrls: ['./incident-workflow-role-add.component.scss']
})
export class IncidentWorkflowRoleAddComponent implements OnInit {
  ...
  close() {
    this.resetForm();
    this._eventEmitterService.dismissIncidentWorkflowRoleAddModal()
  }
  ...
  searchUserType(e) {
    this._auditWorkflowService.getUserTypes('?q=' + e.term).subscribe(res => {
      this._utilityService.detectChanges(this._cdr);
    })
  }
