...
@Component({
  selector: 'app-incident-management-overview',
  templateUrl: './incident-management-overview.component.html',
  styleUrls: ['./incident-management-overview.component.scss']
})
export class IncidentManagementOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(1900)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=1900&module_ids='+id).subscribe(res=>{
            this.loaded=true
