...
@Component({
  selector: 'app-ms-audit-finding-ca-types',
  templateUrl: './ms-audit-finding-ca-types.component.html',
  styleUrls: ['./ms-audit-finding-ca-types.component.scss']
})
export class MsAuditFindingCaTypesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.reactionDisposer = autorun(() => {
      ...
      var subMenuItems = [
        {activityName: 'MS_AUDIT_FINDING_CORRECTIVE_ACTION_TYPES_LIST', submenuItem: { type: 'search' }},
        {activityName: 'EXPORT_MS_AUDIT_FINDING_CORRECTIVE_ACTION_TYPES', submenuItem: {type: 'export_to_excel'}},
          ...
          if (SubMenuItemStore.clikedSubMenuItem) {
            switch (SubMenuItemStore.clikedSubMenuItem.type) {             
              case "export_to_excel":
                this._msAuditFindingCaTypesService.exportToExcel();
                break;
                case "search":
                  MSAuditFindingCATypesMasterStore.searchText = SubMenuItemStore.searchText;
                  this.pageChange(1);
                  break;
              default:
                break;
  ...
  pageChange(newPage: number = null) {
    if (newPage) MSAuditFindingCATypesMasterStore.setCurrentPage(newPage);
    this._msAuditFindingCaTypesService.getItems(false,null,true).subscribe(() => 
  ...
  //     this._msAuditFindingCaTypesService.activate(this.popupObject.id)
  ...
  //   if (status && this.popupObject.id) {
  //     this._msAuditFindingCaTypesService.deactivate(this.popupObject.id)
  //     .subscribe(resp => { setTimeout(() => {
  //         this._utilityService.detectChanges(this._cdr);
  //       }, 500);
  //       this.clearPopupObject();
  ...
  // }
    ...
    sortTitle(type: string) {
      this._msAuditFindingCaTypesService.sortList(type, null);
      this.pageChange();
    }
    ...
    ngOnDestroy() {
      ...
      MSAuditFindingCATypesMasterStore.searchText = '';
      MSAuditFindingCATypesMasterStore.currentPage = 1 ;
    }
}