...
@Component({
  selector: 'app-ms-audit-schedule-statuses',
  templateUrl: './ms-audit-schedule-statuses.component.html',
  styleUrls: ['./ms-audit-schedule-statuses.component.scss']
})
export class MsAuditScheduleStatusesComponent implements OnInit,OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.reactionDisposer = autorun(() => {
      ...
      var subMenuItems = [
        {activityName: 'MS_AUDIT_SCHEDULE_STATUS_LIST', submenuItem: { type: 'search' }},
      ...
      if (SubMenuItemStore.clikedSubMenuItem) {
        switch (SubMenuItemStore.clikedSubMenuItem.type) {
          case "search":
            MsAuditScheduleStatusMasterStore.searchText = SubMenuItemStore.searchText;
            this.pageChange(1);
            break;
          case "share":
  ...
  pageChange(newPage: number = null) {
    if (newPage) MsAuditScheduleStatusMasterStore.setCurrentPage(newPage);
    this._msAuditScheduleStatusesService.getItems(false, null, true).subscribe(() => setTimeout(() => this._utilityService.detectChanges(this._cdr), 100));
  }

  sortTitle(type: string) {
    this._msAuditScheduleStatusesService.sortScheduleStatus(type, null);
    this.pageChange();
  }
...
modalControl(status: boolean) {
  ...
  activateCorrectiveActionStatus(status: boolean) {
    if (status && this.popupObject.id) {
      ...
      this._msAuditScheduleStatusesService.activate(this.popupObject.id).subscribe(resp => {
        setTimeout(() => {
  ...
  deactivateCorrectiveActionStatus(status: boolean) {
    if (status && this.popupObject.id) {
      ...
      this._msAuditScheduleStatusesService.deactivate(this.popupObject.id).subscribe(resp => {
        setTimeout(() => {
          this._utilityService.detectChanges(this._cdr);
        }, 500);
        this.clearPopupObject();
  ...
  ngOnDestroy() {
    ...
    MsAuditScheduleStatusMasterStore.searchText = '';
    MsAuditScheduleStatusMasterStore.currentPage = 1 ;
