...
export class BusinessApplicationsComponent implements OnInit, OnDestroy {
	...
	controlBusinessApplicationsSubscriptionEvent: any = null;
	popupControlBusinessApplicationsEventSubscription: any;
	idleTimeoutSubscription: any;
	networkFailureSubscription: any;

	constructor(
		private _businessApplicationsServiceice: BusinessApplicationsService,
	...
	ngOnInit(): void {
		...
		this.reactionDisposer = autorun(() => {
			...
			if (SubMenuItemStore.clikedSubMenuItem) {
				...
				switch (SubMenuItemStore.clikedSubMenuItem.type) {
					case "new_modal":
						...
						break;
					case "template":
						this._businessApplicationsServiceice.generateTemplate();
						break;
					case "export_to_excel":
						this._businessApplicationsServiceice.exportToExcel();
						break;
					case "search":
			...
			if (ShareItemStore.shareData) {
				this._businessApplicationsServiceice.shareData(ShareItemStore.shareData).subscribe(res => {
			...
			if (ImportItemStore.importClicked) {
				...
				this._businessApplicationsServiceice.importData(ImportItemStore.getFileDetails).subscribe(res => {
	...
	pageChange(newPage: number = null) {
		...
		this._businessApplicationsServiceice.getItems(false, null, true).subscribe(() => setTimeout(() => this._utilityService.detectChanges(this._cdr), 100));
	}
	...
	getBusinessApplications(id: number) {
		...
		this._businessApplicationsServiceice.getItem(id).subscribe(res => {
			this.businessApplicationsObject.values = {
				...
				supplier_title: res.supplier_title,
	...
	deleteBusinessApplication(status: boolean) {
		if (status && this.popupObject.id) {
			this._businessApplicationsServiceice.delete(this.popupObject.id).subscribe(resp => {
				setTimeout(() => {
					this._utilityService.detectChanges(this._cdr);
				}, 500);
	...
	activateBusinessApplication(status: boolean) {
		if (status && this.popupObject.id) {
			...
			this._businessApplicationsServiceice.activate(this.popupObject.id).subscribe(resp => {
				setTimeout(() => {
					this._utilityService.detectChanges(this._cdr);
				}, 500);
				this.clearPopupObject();
	...
	deactivateBusinessApplication(status: boolean) {
		if (status && this.popupObject.id) {
			...
			this._businessApplicationsServiceice.deactivate(this.popupObject.id).subscribe(resp => {
				setTimeout(() => {
					this._utilityService.detectChanges(this._cdr);
				}, 500);
				this.clearPopupObject();
	...
	sortTitle(type: string) {
		...
		this._businessApplicationsServiceice.sortBusinessApplicationList(type, null);
		this.pageChange();
	}

	// Sub-Menu Search 
	searchBusinessApplication(term: string) {
		this._businessApplicationsServiceice.getItems(false, `&q=${term}`).subscribe(res => {
			this._utilityService.detectChanges(this._cdr);
		});
	}
	...
	ngOnDestroy() {
		...
		this.popupControlBusinessApplicationsEventSubscription.unsubscribe();
		BusinessApplicationsMasterStore.searchText = '';
		BusinessApplicationsMasterStore.currentPage = 1 ;
