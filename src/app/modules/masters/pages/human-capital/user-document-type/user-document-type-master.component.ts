...
import { UserDocumentType } from 'src/app/core/models/masters/human-capital/user-document-type';
...
@Component({
  ...
  selector: 'app-user-document-type-master',
  templateUrl: './user-document-type-master.component.html',
  styleUrls: ['./user-document-type-master.component.scss']
})
export class UserDocumentTypeMasterComponent implements OnInit,OnDestroy {
 ...
 AuthStore = AuthStore;
  AppStore = AppStore;
  mailConfirmationData = 'share_user_doc_type_message';
  ...
  ngOnInit() {
    ...
    this.reactionDisposer = autorun(() => {
      ...
      if(!AuthStore.getActivityPermission(1100,'CREATE_USER_DOCUMENT_TYPE')){
      ...
      if (SubMenuItemStore.clikedSubMenuItem) {
        ...
        switch (SubMenuItemStore.clikedSubMenuItem.type) {
          case "new_modal":
            ...
            break;
          case "template":
            this._userDocumentTypeService.generateTemplate();
            break;
          case "export_to_excel":
            this._userDocumentTypeService.exportToExcel();
            break;
          case "search":
            UserDocumentTypeMasterStore.searchText  = SubMenuItemStore.searchText;
            this.pageChange(1);
            break;
          case "share":
            ShareItemStore.setTitle('share_user_doc_type_title');
            ShareItemStore.formErrors = {};
            break;
          case "import":
            ImportItemStore.setTitle('import_user_document_type');
      ...
      if(ShareItemStore.shareData){
        this._userDocumentTypeService.shareData(ShareItemStore.shareData).subscribe(res=>{
      ...
      if(ImportItemStore.importClicked){
        ...
        this._userDocumentTypeService.importData(ImportItemStore.getFileDetails).subscribe(res=>{
    ...
    this.userDocumentSubscriptionEvent = this._eventEmitterService.userDocument.subscribe(res => {
      this.closeFormModal();
    })
  ...
  pageChange(newPage: number = null) {
    if (newPage) UserDocumentTypeMasterStore.setCurrentPage(newPage);
    this._userDocumentTypeService.getItems(false,null,true).subscribe(() => setTimeout(() => this._utilityService.detectChanges(this._cdr), 100));
  }
   ...
   */
  getUserDocumentType(id: number) {
    const userDocumentType: UserDocumentType = UserDocumentTypeMasterStore.getDocumentTypeById(id);
     //set form value
  this.userDocumentObject.values = {
    id: userDocumentType.id,
    title: userDocumentType.title
  }
  ...
  deleteUserDocumetType(status: boolean) {
    if (status && this.popupObject.id) {
      this._userDocumentTypeService.delete(this.popupObject.id).subscribe(resp => {
        setTimeout(() => {
          this._utilityService.detectChanges(this._cdr);
        }, 500);
        ...
        if(error.status == 405 && UserDocumentTypeMasterStore.getDocumentTypeById(this.popupObject.id).status_id == AppStore.activeStatusId){
  ...
  closeConfirmationPopUp(){
    ...
    activateUserDocumetType(status: boolean) {
      if (status && this.popupObject.id) {
        ...
        this._userDocumentTypeService.activate(this.popupObject.id).subscribe(resp => {
          setTimeout(() => {
            this._utilityService.detectChanges(this._cdr);
          }, 500);
          this.clearPopupObject();
    ...
    deactivateUserDocumetType(status: boolean) {
      if (status && this.popupObject.id) {
        ...
        this._userDocumentTypeService.deactivate(this.popupObject.id).subscribe(resp => {
          setTimeout(() => {
            this._utilityService.detectChanges(this._cdr);
          }, 500);
          this.clearPopupObject();
    ...
    activate(id: number) {
      ...
      this.popupObject.title = 'Activate User Document Type?';
    ...
    deactivate(id: number) {
      ...
      this.popupObject.id = id;
      this.popupObject.title = 'Deactivate User Document Type?';
    ...
    delete(id: number) {
      ...
      this.popupObject.type = '';
      this.popupObject.id = id;
      this.popupObject.title = 'Delete  User Document Type?';
   ...
   this._userDocumentTypeService.sortUserDocumentList(type, null);
   this.pageChange();
  }
  ...
  ngOnDestroy() {
    ...
    UserDocumentTypeMasterStore.searchText = '';
    UserDocumentTypeMasterStore.currentPage = 1 ;
