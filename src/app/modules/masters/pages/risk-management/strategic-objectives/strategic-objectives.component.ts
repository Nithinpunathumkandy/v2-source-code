...
export class StrategicObjectivesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.reactionDisposer = autorun(() => {
      ...
      if(!AuthStore.getActivityPermission(100,'CREATE_STRATEGIC_OBJECTIVE')){
  ...
  pageChange(newPage: number = null) {
    ...
    this._strategicObjectivesService.getItems(false,null,true).subscribe(() => setTimeout(() => this._utilityService.detectChanges(this._cdr), 100));
  }
  ...
  deleteStrategicObjectives(status: boolean) {
    if (status && this.popupObject.id) {
      this._strategicObjectivesService.delete(this.popupObject.id).subscribe(resp => {
        setTimeout(() => {
          this._utilityService.detectChanges(this._cdr);
        }, 500);
  ...
  ngOnDestroy() {
    ...
    StrategicObjectivesMasterStore.currentPage = 1 ;
