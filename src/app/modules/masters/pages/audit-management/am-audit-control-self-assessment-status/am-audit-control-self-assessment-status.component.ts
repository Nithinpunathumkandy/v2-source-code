...
import { AmAuditControlSelfAssessmentStatusService } from 'src/app/core/services/masters/audit-management/am-audit-control-self-assessment-status/am-audit-control-self-assessment-status.service';
...
@Component({
  selector: 'app-am-audit-control-self-assessment-status',
  templateUrl: './am-audit-control-self-assessment-status.component.html',
  styleUrls: ['./am-audit-control-self-assessment-status.component.scss']
})
export class AmAuditControlSelfAssessmentStatusComponent implements OnInit {
  ...
  AuditManagementControlSelfAssessmentStatusStore = AuditManagementControlSelfAssessmentStatusStore;
  ...
  constructor(
    ...
    private _auditManagementSelfAssessmentStatusService: AmAuditControlSelfAssessmentStatusService,
  ...
  ngOnInit(): void {
    ...
    this.reactionDisposer = autorun(() => {
      ...
      var subMenuItems = [
        { activityName: 'AM_AUDIT_CONTROL_SELF_ASSESSMENT_STATUS_LIST', submenuItem: { type: 'search' } },
        ...
        { activityName: 'EXPORT_AM_AUDIT_CONTROL_SELF_ASSESSMENT_STATUS', submenuItem: { type: 'export_to_excel' } },
      ...
      if (SubMenuItemStore.clikedSubMenuItem) {
        switch (SubMenuItemStore.clikedSubMenuItem.type) {
          ...
          //   break;
          // case "template":
          //   this._auditManagementSelfAssessmentStatusService.generateTemplate();
          //   break;
          case "export_to_excel":
            this._auditManagementSelfAssessmentStatusService.exportToExcel();
            break;
          case "search":
            AuditManagementControlSelfAssessmentStatusStore.searchText = SubMenuItemStore.searchText;
            this.pageChange(1);
      ...
      //   this._auditManagementSelfAssessmentStatusService.shareData(ShareItemStore.shareData).subscribe(res => {
      ...
      //   this._auditManagementSelfAssessmentStatusService.importData(ImportItemStore.getFileDetails).subscribe(res => {
  ...
  pageChange(newPage: number = null) {
    if (newPage) AuditManagementControlSelfAssessmentStatusStore.setCurrentPage(newPage);
    this._auditManagementSelfAssessmentStatusService.getItems(false, null, true).subscribe(() => setTimeout(() => this._utilityService.detectChanges(this._cdr), 100));
  }
  ...
  //     this._auditManagementSelfAssessmentStatusService.delete(this.popupObject.id).subscribe(resp => {
  //       setTimeout(() => {
  //         this._utilityService.detectChanges(this._cdr);
  //       }, 500);
  ...
  //     }, (error => {
  //       if (error.status == 405 && AuditManagementControlSelfAssessmentStatusStore.getAuditManagementLikelihoodById(this.popupObject.id).status_id == AppStore.activeStatusId) {
  ...
  //     this._auditManagementSelfAssessmentStatusService.activate(this.popupObject.id).subscribe(resp => {
  //       setTimeout(() => {
  //         this._utilityService.detectChanges(this._cdr);
  //       }, 500);
  //       this.clearPopupObject();
  ...
  //     this._auditManagementSelfAssessmentStatusService.deactivate(this.popupObject.id).subscribe(resp => {
  //       setTimeout(() => {
  //         this._utilityService.detectChanges(this._cdr);
  //       }, 500);
  //       this.clearPopupObject();
  ...
  sortTitle(type: string) {
    //
    this._auditManagementSelfAssessmentStatusService.sortAuditManagementLikelihoodList(type, null);
    this.pageChange();
  }
  ...
  searchAuditManagementLikelihoodMasterStore(term: string) {
    this._auditManagementSelfAssessmentStatusService.getItems(false, `&q=${term}`).subscribe(res => {
      this._utilityService.detectChanges(this._cdr);
    });
  }
  ...
  ngOnDestroy() {
    ...
    AuditManagementControlSelfAssessmentStatusStore.searchText = '';
    AuditManagementControlSelfAssessmentStatusStore.currentPage = 1;
