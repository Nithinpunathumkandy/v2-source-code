...
@Component({
  selector: 'app-customer-engagement-overview',
  templateUrl: './customer-engagement-overview.component.html',
  styleUrls: ['./customer-engagement-overview.component.scss']
})
export class CustomerEngagementOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(300)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=300&module_ids='+id).subscribe(res=>{
            this.loaded=true
