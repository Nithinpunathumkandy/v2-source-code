...
declare var $: any;
@Component({
  selector: 'app-event-risk-assessment-model',
  templateUrl: './event-risk-assessment-model.component.html',
  styleUrls: ['./event-risk-assessment-model.component.scss']
})
export class EventRiskAssessmentModelComponent implements OnInit {

  @Input('source') eventSource: any;
  ...
  ngOnInit(): void {
    ...
    if (EventRiskAssessmentStore?.eventRiskDetails?.is_analysis_performed == 1) {
      this._eventRiskAssessmentService.getRiskAssessment(EventRiskAssessmentStore.selectedId).subscribe(res => {
  ...
  getScoreClass(rating) {
    ...
    let totalScore=0;
    if(EventRiskAssessmentStore.calculationMethod?.is_addition)
    totalScore = this.activeImpact?.score + this.activeLikelihood?.score;
    else if(EventRiskAssessmentStore.calculationMethod?.is_multiplication)
    totalScore = this.activeImpact?.score * this.activeLikelihood?.score;
    else if(EventRiskAssessmentStore.calculationMethod?.is_division){
  ...
  // callLabelFn(){
  //   if(EventRiskAssessmentStore.individualRiskDetails?.risk_classification?.is_risk==1){
  ...
  setCategoryValues(){
    this.categoryItems = []
    if(this.eventSource.component=='risk_treatment'){
      if(EventResidualRiskStore.residualRiskDetails.risk_residual_analysis){
        this.comments = EventResidualRiskStore.residualRiskDetails.risk_residual_analysis.comments
        EventResidualRiskStore.residualRiskDetails.risk_residual_analysis.risk_impact_category_analyses.forEach(res=>{
          var obj = new Object
      ...
      if(EventRiskAssessmentStore.eventRiskAnalysis.risk_analysis){
        this.comments = EventRiskAssessmentStore.eventRiskAnalysis.risk_analysis.comments
        EventRiskAssessmentStore.eventRiskAnalysis.risk_analysis.risk_impact_category_analyses.forEach(res=>{
          var obj = new Object
          obj['bia_impact_category_id'] = res.bia_impact_category_id;
          obj['risk_matrix_impact_id'] = res.risk_matrix_impact_id
          obj['risk_matrix_likelihood_id'] = res.risk_matrix_likelihood_id
  ...
  savePerformAssessment(){
    ...
    if(this.eventSource.component=='risk_treatment'){
      ...
      this._eventRiskAssessmentService.saveRiskAssessment(EventRiskAssessmentStore.selectedId,saveData).subscribe(res=>{
        AppStore.disableLoading();
        this.cancel();
        this._utilityService.detectChanges(this._cdr);
      }, 
  ...
  cancel(){
    this.currentTab = 0;
    this.categoryItems = [];
    this._eventEmitterService.dismisseventRiskAssessmentModal();
  }
