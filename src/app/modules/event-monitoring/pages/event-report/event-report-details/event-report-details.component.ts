...
declare var $: any;
@Component({
  selector: 'app-event-report-details',
  templateUrl: './event-report-details.component.html',
  styleUrls: ['./event-report-details.component.scss']
})
export class EventReportDetailsComponent implements OnInit {
  ...
  constructor(
    private _router: Router,
		...
		this._route.params.subscribe(params => {
			this.id = params.id;
			this.reportType = params.eventcountType;
		}); }
				...
				// this.processDateFilterSelected(SubMenuItemStore.DatefilterValue);
					...
					this.reactionDisposer = autorun(() => {
						var subMenuItems = [
							...
							{ activityName: this.EventReportStore.selectedReportObject.listPermission, submenuItem: { type: 'export_to_excel' } },
							{ activityName: null, submenuItem: { type: 'close', path: `/event-monitoring/reports/${this.riskcountType}` } },
						]
							...
							switch (SubMenuItemStore.clikedSubMenuItem.type) {
								case "export_to_excel":
									...
									if (this.filterDateObject.startDate) {
										params = `?${this.EventReportStore.selectedReportObject.eventItemId}=${this.id}&from=${this.filterDateObject.startDate}&to=${this.filterDateObject.endDate}`;
									}
									this._eventReportService.exportToExcelList(this.EventReportStore.selectedReportObject, params);
									break;
								default:
									break;
							}
...
processDateFilterSelected(dateType: any): void {
	if (dateType === "custom") {
		...
		EventReportStore.eventtlistmakeEmpty();
		this.pageChange(1);
	}
}
...
getRisk(id: any): void {
	...
	if (this.EventReportStore.selectedReportObject.reportType === 'eventRegister') {
		this._router.navigateByUrl(`event-monitoring/events/${id}`);
	}
	else if (this.EventReportStore.selectedReportObject.reportType === 'eventTreatment') {
		this._router.navigateByUrl(`event-monitoring/events/${id}/change-request/${id}`);
	}
else if (this.EventReportStore.selectedReportObject.reportType === 'eventClosure') {
		this._router.navigateByUrl(`event-monitoring/events/${id}/closure/${id}`);
	}
}
...
pageChange(newPage: number = null) {
	...
	if (newPage) EventReportStore.setCurrentPage(newPage);
	this._eventReportService.getProjectItemsDetails(this.id, this.EventReportStore.selectedReportObject, params).subscribe(() => setTimeout(() => this._utilityService.detectChanges(this._cdr), 100));
}
