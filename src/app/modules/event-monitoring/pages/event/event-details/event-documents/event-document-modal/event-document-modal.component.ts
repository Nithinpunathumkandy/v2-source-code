...
@Component({
  selector: 'app-event-document-modal',
  templateUrl: './event-document-modal.component.html',
  styleUrls: ['./event-document-modal.component.scss']
})
export class EventDocumentModalComponent implements OnInit {

  @ViewChild('fileUploadModal', { static: false }) fileUploadModal: ElementRef;
  @ViewChild('uploadArea', { static: false }) uploadArea: ElementRef;
  @Input ('source') eventDocumentSource: any;
  ...
  ngOnInit(): void {
    ...
    if (this.eventDocumentSource.type == 'Edit') {
      this.setFormValues();
    }
  ...
  setFormValues(){
      this.form.patchValue({
        id: this.eventDocumentSource.value?.id,
        title: this.eventDocumentSource.value?.title,
        description: this.eventDocumentSource.value?.description,
      })
  }
  ...
  closeFormModal(){
    this.resetForm();
    this._eventEmitterService.dismissEventDocumentModal();
  }
  ...
  save(close: boolean = false) {
    ...
    if (this.form.value) {
      ...
      if (this.form.value.id) {
        ...
        save = this._eventDocumentService.updateItem(EventsStore.selectedEventId,this.form.value.id, updateParam);
      } else {
        let saveParam = {
          ...this.form.value,
          document: {...this._helperService.sortFileuploadData(fileUploadPopupStore.displayFiles, 'save')[0]}
        }
        save = this._eventDocumentService.saveProjectDocument(EventsStore.selectedEventId, saveParam);
      }
