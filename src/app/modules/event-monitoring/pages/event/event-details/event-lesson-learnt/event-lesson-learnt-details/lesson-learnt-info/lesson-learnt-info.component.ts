...
@Component({
  selector: 'app-lesson-learnt-info',
  templateUrl: './lesson-learnt-info.component.html',
  styleUrls: ['./lesson-learnt-info.component.scss']
})
export class LessonLearntInfoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.reactionDisposer = autorun(() => {
      var subMenuItems = [
        { activityName: 'UPDATE_EVENT_LESSON_LEARNED', submenuItem: { type: 'edit_modal' } },
    ...
    this.taskSubscription = this._eventEmitterService.eventLessonLearnedModal.subscribe(item => {
      this.closeFormModal();
      this.getTaskDetails(this.taskId)
    })
  ...
  delete() {    
    ...
    this.popupObject.id = this.taskId;
  ...
  deleteEventLesson(status: boolean) {
    if (status && this.popupObject.id) {
      this._lessonLearnedService.delete(this.popupObject.id).subscribe(resp => {
        if (resp) {
          setTimeout(() => {
            this._utilityService.detectChanges(this._cdr);
            this._router.navigateByUrl(`event-monitoring/events/${EventsStore.selectedEventId}/lesson-learned`);
          }, 500);
          this._utilityService.detectChanges(this._cdr);
