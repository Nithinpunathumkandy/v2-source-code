...
import { EventRiskJourneyService } from 'src/app/core/services/event-monitoring/event-risk-journey/event-risk-journey.service';
...
@Component({
  selector: 'app-event-risk-journey',
  templateUrl: './event-risk-journey.component.html',
  styleUrls: ['./event-risk-journey.component.scss']
})
export class EventRiskJourneyComponent implements OnInit {
  ...
  AppStore = AppStore;
  ResidualRiskStore = EventResidualRiskStore;
  EventRiskJourneyStore = EventRiskJourneyStore
  ...
  constructor(
    private bcmRiskJourneyService: EventRiskJourneyService,
  ...
  getRiskJourney(){
    this.bcmRiskJourneyService.getItem(EventRiskAssessmentStore.selectedId).subscribe(res=>{
      this.setValues(res);
    })
  }
  ...
  setValues(RiskMapping) {
    ...
    EventMappingStore.projects = [];
    EventMappingStore.locations = [];
    EventMappingStore.controls = [];
    EventMappingStore.customers = [];
    EventMappingStore.products = [];
    EventMappingStore.objectives = [];
    this.issues = [];
    this.processes = [];
    // if(EventMappingStore.loaded){
    let processItem = RiskMapping.processes;
    let issueItem = RiskMapping.organization_issues;
    let projectItem = RiskMapping.projects;
    EventMappingStore.locations = RiskMapping.locations;
    EventMappingStore.products = RiskMapping.products;
    EventMappingStore.customers = RiskMapping.customers;
    EventMappingStore.objectives = RiskMapping.strategic_objectives;
    EventMappingStore.controls = RiskMapping.controls;
    ...
    for (let i of projectItem) {
      ...
      EventMappingStore.projects.push(i);
    }
