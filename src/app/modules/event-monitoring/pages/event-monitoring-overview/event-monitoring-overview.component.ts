...
@Component({
  ...
  templateUrl: './event-monitoring-overview.component.html',
  styleUrls: ['./event-monitoring-overview.component.scss']
})
export class EventMonitoringOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if(OrganizationModulesStore.organizationModules.length>0){
        this.getModules(toJS(OrganizationModulesStore.getOrganziationModulesByModuleGroup(4000)));
  ...
  getModules(data){
    ...
    setIndex(id,index?){
      ...
      this.mainModuleIndex=index
          this._overViewService.getOverviewDetails('?module_group_ids=4000&module_ids='+id).subscribe(res=>{
            this.loaded=true
