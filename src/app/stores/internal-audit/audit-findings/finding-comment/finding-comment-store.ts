...
class Store {
    ...
    @observable
    private _findingComment: FindingComment[] = [];
    ...
    @action
    setAllFindingsComments(response: FindingCommentPaginationResponse) {
       
        this._findingComment = response.data;
    ...
    @computed
    get allItems(): FindingComment[] {
        
        return this._findingComment;
    }
    ...
    @action
    getCorrectiveActionCommentById(id: number): FindingComment {
        return this._findingComment.slice().find(e => e.id == id);
    }
...
export const FindingCommentsStore = new Store();
