...
@Component({
  selector: 'app-incident-investigation-workflow',
  templateUrl: './incident-investigation-workflow.component.html',
  styleUrls: ['./incident-investigation-workflow.component.scss']
})
export class IncidentInvestigationWorkflowComponent implements OnInit {
  ...
  isUser(){
    for(let i of IncidentInvestigationWorkflowStore?.workflowDetails){
      if(i.level==IncidentInvestigationStore.individualInvestigationItem.next_review_user_level){
       var pos=i.incident_investigation_workflow_item_users.findIndex(e=>e.id==AuthStore.user.id)
       if(pos!=-1)
         return true;
       else
        return false
