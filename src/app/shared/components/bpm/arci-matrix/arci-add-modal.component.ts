...
@Component({
  selector: 'app-arci-add-modal',
  templateUrl: './arci-add-modal.component.html',
  styleUrls: ['./arci-add-modal.component.scss']
})
export class ArciAddModalComponent implements OnInit {
  ...
  constructor(private _utilityService: UtilityService, 
    ...
    private _formBuilder: FormBuilder, public _arciService: ArciService,private _processService:ProcessService,
    ...
    ngOnInit(): void {
      ...
      if (this.arciMatrixSource.hasOwnProperty('values') && this.arciMatrixSource.values) {
        ...
        this.arciMatrixForm.controls["process_id"].disable()
        
        this.arciMatrixForm.setValue({
          process_id: this.arciMatrixSource.values.process_id,
   ...
   this.searchProcess({term: this.arciMatrixSource.values.process_id});
      }
  ...
  searchProcess(e){
    this._processService.getAllItems(false, '&q=' + e.term).subscribe(res => {
        this._utilityService.detectChanges(this._cdr);
      });
    // }
  }
  ...
  searchUers(e) {
    // var params = '';
    // params = '&organization_ids='+this.processForm.value.organization_id
    // +'&division_ids='+this.processForm.value.division_id
    // +'&department_ids='+this.processForm.value.department_id
    // +'&section_ids='+this.processForm.value.section_id
    // +'&sub_section_ids='+this.processForm.value.sub_section_id
  ...
  processChange(id) {
    ...
    this._processService.getItemById(id).subscribe(res => {
      ...
      this.arciMatrixForm.patchValue({
        ...
        // accountable_user_ids:this.getEditValue(ARCIDetails.process_accountable_users),
        // responsible_user_ids:this.getEditValue(ARCIDetails.process_responsible_users),
        // consulted_user_ids: this.getEditValue(ARCIDetails.process_consulted_users),
        // informed_user_ids: this.getEditValue(ARCIDetails.process_informed_users),
