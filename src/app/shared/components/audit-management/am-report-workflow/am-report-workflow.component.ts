...
// import { AmPreliminaryReportWorkflowStore } from 'src/app/stores/audit-management/am-audit-plan/am-annual-audit-plan-workflow.store';
declare var $: any;

@Component({
  selector: 'app-am-report-workflow',
  templateUrl: './am-report-workflow.component.html',
  styleUrls: ['./am-report-workflow.component.scss']
})
export class AmReportWorkflowComponent implements OnInit {
   ...
   WorkflowEmptyMessage = "empty_workflow_title"
     ...
     isUser(){
       for(let i of AmPreliminaryReportWorkflowStore?.workflowDetails){
      ...
      checkStatus(workflow) {
       ...
       if (!workflow.workflow_status) {
         ...
         if (workflow.level == AmPreliminaryReportStore.reportDetails?.next_review_user_level)
     ...
     closeWorkflowModal(){
       this._eventEmitterService.dismissAmPreliminaryWorkflowModal();
     }
