...
@Component({
  selector: 'app-finding-ca-update-modal',
  templateUrl: './finding-ca-update-modal.component.html',
  styleUrls: ['./finding-ca-update-modal.component.scss']
})
export class FindingCaUpdateModalComponent implements OnInit {
  ...
  getStatus() {
    this.statuses = [];
    this._findingCorrectiveActionStatusesService.getItems(false,'&is_active=true').subscribe(res => {
  ...
  searchStatus(event) {
    this.statuses = [];
    this._findingCorrectiveActionStatusesService.getItems(false, '&q=' + event.term+'&is_active=true').subscribe(res => {
      for (let i of res['data']) {
        if (i.type == 'new' || i.type == 'wip' || i.type == 'resolved') {
          this.statuses.push(i);
        }
  ...
  updateCA(close: boolean = false) {
    ...
    this._correctiveActionService.updateCorrectiveAction(AmFindingCAStore?.correctiveActionDetails.id, this.saveData()).subscribe(res => {
      // this._findingCorrectiveActionService.getItem(this.updateObject?.id).subscribe();
      AppStore.disableLoading();
      this._correctiveActionService.getItem(AmFindingCAStore?.correctiveActionDetails.finding_id, AmFindingCAStore?.correctiveActionDetails.id).subscribe(response => {
        this._utilityService.detectChanges(this._cdr);
      })
  ...
  closeUpdateModal() {
    ...
    this.AmFindingCAStore.clearUpdateDocumentDetails();
    this.updateForm.reset();
    this._eventEmitterService.dismissAmAuditFindingCaUpdateModal();
  }
    ...
    if (selectedFiles.length > 0) {
      ...
      Array.prototype.forEach.call(temporaryFiles, elem => {
        ...
        if (this._imageService.validateFile(file, type)) {
          ...
          AmFindingCAStore.update_document_preview_available = true;
            ...
            switch (uploadEvent.type) {
              ...
              case HttpEventType.Response:
                ...
                this._imageService.getPreviewUrl(temp.thumbnail_url).subscribe(prew => {
                  AmFindingCAStore.update_document_preview_available = false;
  ...
  removeDocument(token) {
    AmFindingCAStore.unsetProductImageDetails('support-file', token);
    // this.checkForFileUploadsScrollbar();
    this._utilityService.detectChanges(this._cdr);
  }
  ...
  getUpdateModalDetails() {
    ...
    this._correctiveActionService.getItem(AmFindingCAStore.correctiveActionDetails.finding_id, this.updateObject.values.ca_id).subscribe(() => setTimeout(() => this._utilityService.detectChanges(this._cdr), 100));
    this.updateForm.patchValue({
      percentage: AmFindingCAStore?.correctiveActionDetails?.finding_corrective_action_status_updates?.slice(-1)[0]?.percentage ? AmFindingCAStore?.correctiveActionDetails?.finding_corrective_action_status_updates?.slice(-1)[0]?.percentage : 0,
      finding_corrective_action_status_id: AmFindingCAStore?.correctiveActionDetails?.finding_corrective_action_status_updates?.slice(-1)[0]?.finding_corrective_action_status_id ? Number(AmFindingCAStore?.correctiveActionDetails?.finding_corrective_action_status_updates?.slice(-1)[0]?.finding_corrective_action_status_id) : null,
    })
    this._utilityService.detectChanges(this._cdr);
  }
