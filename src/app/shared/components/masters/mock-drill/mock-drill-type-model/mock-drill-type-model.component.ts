...
@Component({
  selector: 'app-mock-drill-type-model',
  templateUrl: './mock-drill-type-model.component.html',
  styleUrls: ['./mock-drill-type-model.component.scss']
})
export class MockDrillTypeModelComponent implements OnInit {

  @Input('source') MockDrillTypeSource: any;
  ...
  constructor(
    ...
    private _mockDrillType: MockDrillTypesService,
  ...
  ngOnInit(): void {
    ...
    if (this.MockDrillTypeSource) {
      if (this.MockDrillTypeSource.hasOwnProperty('values') && this.MockDrillTypeSource.values) {
        this.setEditValue();
        this.mockDrillTypeId = this.MockDrillTypeSource.values.id;
      }
    }
  }
  ngDoCheck() {
  ...
  setEditValue() {
    if (this.MockDrillTypeSource.values.languages && this.MockDrillTypeSource.values.languages.length > 0) {
      for (let i of this.formNgModal) {
        i.title = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.title : '';
        i.id = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.mock_drill_type_id : '';
      }
      this._utilityService.detectChanges(this._cdr);
    }
  }
  getValuesForEdit(id: number) {
    let languageValues = this.MockDrillTypeSource.values.languages.find(e => e.id == id);
  ...
  closeFormModal() {
    ...
    this._eventEmitterService.dismissMockDrillTypeModel();
  }
  ...
  save(close: boolean = false) {
    ...
    if (saveData.languages.length) {
      ...
      if (this.mockDrillTypeId) {
        save = this._mockDrillType.updateItem(this.mockDrillTypeId, saveData);
      } else {
        save = this._mockDrillType.saveItem(saveData);
      }
  ...
  createSaveData() {
    ...
    for (let i of formData) {
      ...
      delete i.error;
      if (!this.MockDrillTypeSource.values)
        delete i.id;
    }
