...
export class SuppliersModalComponent implements OnInit {
  ...
  //   this.SuppliersMasterStore.addOrEditFlag = false;
  ...
  resetForm() {
    // SuppliersMasterStore.unsetFile();
    this.form.reset();
    this.form.pristine;
  ...
  save(close: boolean = false) {
    ...
    if (this.form.value) {
      ...
      save.subscribe((res: any) => {
        this.SuppliersMasterStore.lastInsertedId = res.id
        if (!this.form.value.id) {
          this.resetForm();
        }
        AppStore.disableLoading();
  ...
  onFileChange(event, type: string) {
    ...
    if (event.target.files.length > 0) {
      ...
      if (this._imageService.validateFile(file, type)) {
        ...
        if (type == 'logo') SuppliersMasterStore.logo_preview_available = true;
              ...
              case HttpEventType.Response:
                ...
                this._imageService.getPreviewUrl(temp.thumbnail_url).subscribe(prew => {
                  if (type == 'logo') {
                    ...
                    SuppliersMasterStore.logo_preview_available = false;
                  }
  ...
  createImageFromBlob(image: Blob, imageDetails, type) {
    ...
    reader.addEventListener("load", () => {
      ...
      this._suppliersService.setFileDetails(imageDetails, logo_url, type);
      this._utilityService.detectChanges(this._cdr);
    }, false);
