...
@Component({
  selector: 'app-event-risk-impact-area-model',
  templateUrl: './event-risk-impact-area-model.component.html',
  styleUrls: ['./event-risk-impact-area-model.component.scss']
})
export class EventRiskImpactAreaModelComponent implements OnInit {

  @Input('source') riskImpactAreaSource: any;
  AppStore = AppStore;
  formErrors: any;
  ...
  ngOnInit(): void {
    ...
    if (this.riskImpactAreaSource) {

      if (this.riskImpactAreaSource.hasOwnProperty('values') && this.riskImpactAreaSource.values) {
        this.setEditValue();
        this.checklistId = this.riskImpactAreaSource.values.id;
  ...
  closeFormModal() {
    ...
    this._eventEmitterService.dismissriskImpactAreaModal();
  }
  ...
  save(close: boolean = false) {
    ...
    if (saveData.languages.length) {
      ...
      if (this.checklistId) {
        save = this._eventRiskImpactAreaService.updateItem(this.checklistId, saveData);
      } else {
        save = this._eventRiskImpactAreaService.saveItem(saveData);
      }
    ...
    createSaveData(){
      ...
      for(let i of formData){
        ...
        delete i.error;
        if(!this.riskImpactAreaSource.values)
          delete i.id;
      }
    ...
    setEditValue(){
      if(this.riskImpactAreaSource.values.languages.length > 0){
        for(let i of this.formNgModal){
          i.title = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.title : '';
          i.id = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.checklist_id : '';
        }
        this._utilityService.detectChanges(this._cdr);
      }
    }
    ...
    getValuesForEdit(id: number){
      let languageValues = this.riskImpactAreaSource.values.languages.find(e=> e.id == id);
