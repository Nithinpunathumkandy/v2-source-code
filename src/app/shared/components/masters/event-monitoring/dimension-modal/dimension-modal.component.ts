...
@Component({
  ...
  templateUrl: './dimension-modal.component.html',
  styleUrls: ['./dimension-modal.component.scss']
})
export class DimensionModalComponent implements OnInit {

  @Input('source') DimensionSource: any;
  AppStore = AppStore;
  formErrors: any;
  ...
  ngOnInit(): void {
    ...
    if (this.DimensionSource) {
      ...
      if (this.DimensionSource.hasOwnProperty('values') && this.DimensionSource.values) {
        this.setEditValue();
        this.dimensionId = this.DimensionSource.values.id;
  ...
  closeFormModal() {
    ...
    this._eventEmitterService.dismissDimensionModal();
  }
  ...
  save(close: boolean = false) {
    ...
    if (saveData.languages.length) {
      ...
      if (this.dimensionId) {
        save = this._dimensionService.updateItem(this.dimensionId, saveData);
      } else {
        save = this._dimensionService.saveItem(saveData);
      }
    ...
    createSaveData(){
      ...
      for(let i of formData){
        ...
        delete i.error;
        if(!this.DimensionSource.values)
          delete i.id;
      }
    ...
    setEditValue(){
      if(this.DimensionSource.values.languages.length > 0){
        for(let i of this.formNgModal){
          i.title = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.title : '';
          i.id = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.dimension_id : '';
        }
        this._utilityService.detectChanges(this._cdr);
      }
    }
    ...
    getValuesForEdit(id: number){
      let languageValues = this.DimensionSource.values.languages.find(e=> e.id == id);
