...
export class StatusModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.StatusSource) {
      ...
      if (this.StatusSource.hasOwnProperty('values') && this.StatusSource.values) {
        this.setEditValue();
        this.statusId = this.StatusSource.values.id;
  ...
  closeFormModal() {
    ...
    this._eventEmitterService.dismissStatusModal();
  }
  ...
  save(close: boolean = false) {
    ...
    if (saveData.languages.length) {
      ...
      if (this.statusId) {
        save = this._statusService.updateItem(this.statusId, saveData);
      } else {
        save = this._statusService.saveItem(saveData);
      }
    ...
    setEditValue(){
      if(this.StatusSource.values.languages.length > 0){
        for(let i of this.formNgModal){
          i.title = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.title : '';
          i.id = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.status_id : '';
        }
        this._utilityService.detectChanges(this._cdr);
      }
    }
    ...
    getValuesForEdit(id: number){
      let languageValues = this.StatusSource.values.languages.find(e=> e.id == id);
