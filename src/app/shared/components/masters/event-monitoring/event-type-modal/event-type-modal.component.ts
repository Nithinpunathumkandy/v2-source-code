...
@Component({
  ...
  styleUrls: ['./event-type-modal.component.scss']
})
export class EventTypeModalComponent implements OnInit {

  @Input('source') EventTypeSource: any;
  AppStore = AppStore;
  formErrors: any;
  ...
  ngOnInit(): void {
    ...
    if (this.EventTypeSource) {

      if (this.EventTypeSource.hasOwnProperty('values') && this.EventTypeSource.values) {
        this.setEditValue();
        this.eventTypeId = this.EventTypeSource.values.id;
  ...
  closeFormModal() {
    ...
    this._eventEmitterService.dismissEventTypeModal();
  }
  ...
  save(close: boolean = false) {
    ...
    if (saveData.languages.length) {
      ...
      if (this.eventTypeId) {
        save = this._eventTypeService.updateItem(this.eventTypeId, saveData);
      } else {
        save = this._eventTypeService.saveItem(saveData);
      }
    ...
    createSaveData(){
      ...
      for(let i of formData){
        ...
        delete i.error;
        if(!this.EventTypeSource.values)
          delete i.id;
      }
    ...
    setEditValue(){
      if(this.EventTypeSource.values.languages && this.EventTypeSource.values.languages.length > 0){
        for(let i of this.formNgModal){
    ...
    getValuesForEdit(id: number){
      let languageValues = this.EventTypeSource.values.languages.find(e=> e.id == id);
