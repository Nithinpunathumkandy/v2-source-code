...
@Component({
  selector: 'app-periodicity-modal',
  templateUrl: './periodicity-modal.component.html',
  styleUrls: ['./periodicity-modal.component.scss']
})
export class PeriodicityModalComponent implements OnInit {

  @Input('source') PeriodicitySource: any;
  AppStore = AppStore;
  formErrors: any;
  ...
  ngOnInit(): void {
    ...
    if (this.PeriodicitySource) {

      if (this.PeriodicitySource.hasOwnProperty('values') && this.PeriodicitySource.values) {
        this.setEditValue();
        this.periodicityId = this.PeriodicitySource.values.id;
  ...
  closeFormModal() {
    ...
    this._eventEmitterService.dismissPeriodicityModal();
  }
  ...
  save(close: boolean = false) {
    ...
    if (saveData.languages.length) {
      ...
      if (this.periodicityId) {
        save = this._periodicityService.updateItem(this.periodicityId, saveData);
      } else {
        save = this._periodicityService.saveItem(saveData);
      }
    ...
    createSaveData(){
      ...
      for(let i of formData){
        ...
        delete i.error;
        if(!this.PeriodicitySource.values)
          delete i.id;
      }
    ...
    setEditValue(){
      if(this.PeriodicitySource.values.languages && this.PeriodicitySource.values.languages.length > 0){
        for(let i of this.formNgModal){
          i.title = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.title : '';
          i.id = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.periodicity_id : '';
        }
        this._utilityService.detectChanges(this._cdr);
      }
    }
    ...
    getValuesForEdit(id: number){
      let languageValues = this.PeriodicitySource.values.languages.find(e=> e.id == id);
