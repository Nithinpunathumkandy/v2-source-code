...
@Component({
  selector: 'app-target-audience-modal',
  templateUrl: './target-audience-modal.component.html',
  styleUrls: ['./target-audience-modal.component.scss']
})
export class TargetAudienceModalComponent implements OnInit {

  @Input('source') TargetAudienceSource: any;
  AppStore = AppStore;
  formErrors: any;
  ...
  ngOnInit(): void {
    ...
    if (this.TargetAudienceSource) {

      if (this.TargetAudienceSource.hasOwnProperty('values') && this.TargetAudienceSource.values) {
        this.setEditValue();
        this.targetAudienceId = this.TargetAudienceSource.values.id;
  ...
  closeFormModal() {
    ...
    this._eventEmitterService.dismissTargetAudienceModal();
  }
  ...
  save(close: boolean = false) {
    ...
    if (saveData.languages.length) {
      ...
      if (this.targetAudienceId) {
        save = this._targetAudienceService.updateItem(this.targetAudienceId, saveData);
      } else {
        save = this._targetAudienceService.saveItem(saveData);
      }
    ...
    createSaveData(){
      ...
      for(let i of formData){
        ...
        delete i.error;
        if(!this.TargetAudienceSource.values)
          delete i.id;
      }
    ...
    setEditValue(){
      if(this.TargetAudienceSource.values.languages && this.TargetAudienceSource.values.languages.length > 0){
        for(let i of this.formNgModal){
          i.title = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.title : '';
          i.id = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.target_audience_id : '';
        }
        this._utilityService.detectChanges(this._cdr);
      }
    }
    ...
    getValuesForEdit(id: number){
      let languageValues = this.TargetAudienceSource.values.languages.find(e=> e.id == id);
