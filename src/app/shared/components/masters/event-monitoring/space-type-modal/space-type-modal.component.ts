...
@Component({
  selector: 'app-space-type-modal',
  templateUrl: './space-type-modal.component.html',
  styleUrls: ['./space-type-modal.component.scss']
})
export class SpaceTypeModalComponent implements OnInit {

  @Input('source') SpaceTypeSource: any;
  AppStore = AppStore;
  formErrors: any;
  ...
  ngOnInit(): void {
    ...
    if (this.SpaceTypeSource) {

      if (this.SpaceTypeSource.hasOwnProperty('values') && this.SpaceTypeSource.values) {
        this.setEditValue();
        this.spaceTypeId = this.SpaceTypeSource.values.id;
  ...
  closeFormModal() {
    this.resetForm();
    this.spaceTypeId= null;
    this.selectedLanguageId = null;
    this._eventEmitterService.dismissSpaceTypeModal();
  }
  ...
  save(close: boolean = false) {
    ...
    if (saveData.languages.length) {
      ...
      if (this.spaceTypeId) {
        save = this._spaceTypeService.updateItem(this.spaceTypeId, saveData);
      } else {
        save = this._spaceTypeService.saveItem(saveData);
      }
    ...
    createSaveData(){
      ...
      for(let i of formData){
        ...
        delete i.error;
        if(!this.SpaceTypeSource.values)
          delete i.id;
      }
    ...
    setEditValue(){
      if(this.SpaceTypeSource.values.languages.length > 0){
        for(let i of this.formNgModal){
          i.title = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.title : '';
          i.id = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.space_type_id : '';
        }
        this._utilityService.detectChanges(this._cdr);
      }
    }
    ...
    getValuesForEdit(id: number){
      let languageValues = this.SpaceTypeSource.values.languages.find(e=> e.id == id);
