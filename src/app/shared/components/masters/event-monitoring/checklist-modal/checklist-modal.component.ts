...
export class ChecklistModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.ChecklistSource) {
      ...
      if (this.ChecklistSource.hasOwnProperty('values') && this.ChecklistSource.values) {
        this.setEditValue();
        this.checklistId = this.ChecklistSource.values.id;
  ...
  closeFormModal() {
    ...
    this._eventEmitterService.dismissChecklistModal();
  }
  ...
  save(close: boolean = false) {
    ...
    if (saveData.languages.length) {
      ...
      if (this.checklistId) {
        save = this._checklistService.updateItem(this.checklistId, saveData);
      } else {
        save = this._checklistService.saveItem(saveData);
      }
    ...
    createSaveData(){
      ...
      for(let i of formData){
        ...
        delete i.error;
        if(!this.ChecklistSource.values)
          delete i.id;
      }
    ...
    setEditValue(){
      if(this.ChecklistSource.values.languages.length > 0){
        for(let i of this.formNgModal){
    ...
    getValuesForEdit(id: number){
      let languageValues = this.ChecklistSource.values.languages.find(e=> e.id == id);
