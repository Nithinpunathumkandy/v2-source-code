...
@Component({
  selector: 'app-event-engagement-strategy-modal',
  templateUrl: './event-engagement-strategy-modal.component.html',
  styleUrls: ['./event-engagement-strategy-modal.component.scss']
})
export class EventEngagementStrategyModalComponent implements OnInit {

  @Input('source') EventEngagementStrategySource: any;
  AppStore = AppStore;
  formErrors: any;
  ...
  eventEngagementStrategyId = null;
  ...
  ngOnInit(): void {
    ...
    if (this.EventEngagementStrategySource) {

      if (this.EventEngagementStrategySource.hasOwnProperty('values') && this.EventEngagementStrategySource.values) {
        this.setEditValue();
        this.eventEngagementStrategyId = this.EventEngagementStrategySource.values.id;
  ...
  closeFormModal() {
    this.resetForm();
    this.eventEngagementStrategyId= null;
    this.selectedLanguageId = null;
    this._eventEmitterService.dismissEventEngagementStratergyModal();
  }
  ...
  save(close: boolean = false) {
    ...
    if (saveData.languages.length) {
      ...
      if (this.eventEngagementStrategyId) {
        save = this._eventEngagementStrategyService.updateItem(this.eventEngagementStrategyId, saveData);
      } else {
        save = this._eventEngagementStrategyService.saveItem(saveData);
      }
    ...
    createSaveData(){
      ...
      for(let i of formData){
        ...
        delete i.error;
        if(!this.EventEngagementStrategySource.values)
          delete i.id;
      }
    ...
    setEditValue(){
      if(this.EventEngagementStrategySource.values.languages && this.EventEngagementStrategySource.values.languages.length > 0){
        for(let i of this.formNgModal){
    ...
    getValuesForEdit(id: number){
      let languageValues = this.EventEngagementStrategySource.values.languages.find(e=> e.id == id);
