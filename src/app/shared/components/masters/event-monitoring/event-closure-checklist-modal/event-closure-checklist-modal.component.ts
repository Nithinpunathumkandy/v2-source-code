...
@Component({
  selector: 'app-event-closure-checklist-modal',
  templateUrl: './event-closure-checklist-modal.component.html',
  styleUrls: ['./event-closure-checklist-modal.component.scss']
})
export class EventClosureChecklistModalComponent implements OnInit {

  @Input('source') EventClosureChecklistSource: any;
  AppStore = AppStore;
  formErrors: any;
  ...
  ngOnInit(): void {
    ...
    if (this.EventClosureChecklistSource) {

      if (this.EventClosureChecklistSource.hasOwnProperty('values') && this.EventClosureChecklistSource.values) {
        this.setEditValue();
        this.eventClosureChecklistId = this.EventClosureChecklistSource.values.id;
  ...
  closeFormModal() {
    this.resetForm();
    this.eventClosureChecklistId= null;
    this.selectedLanguageId = null;
    this._eventEmitterService.dismissEventClosureChecklistModal();
  }
  ...
  save(close: boolean = false) {
    ...
    if (saveData.languages.length) {
      ...
      if (this.eventClosureChecklistId) {
        save = this._eventClosureChecklistService.updateItem(this.eventClosureChecklistId, saveData);
      } else {
        save = this._eventClosureChecklistService.saveItem(saveData);
      }
    ...
    createSaveData(){
      ...
      for(let i of formData){
        ...
        delete i.error;
        if(!this.EventClosureChecklistSource.values)
          delete i.id;
      }
    ...
    setEditValue(){
      if(this.EventClosureChecklistSource.values.languages && this.EventClosureChecklistSource.values.languages.length > 0){
        for(let i of this.formNgModal){
          i.title = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.title : '';
          i.id = this.getValuesForEdit(i.language_id) ? this.getValuesForEdit(i.language_id).pivot.event_closure_checklist_id : '';
        }
        this._utilityService.detectChanges(this._cdr);
      }
    }
    ...
    getValuesForEdit(id: number){
      let languageValues = this.EventClosureChecklistSource.values.languages.find(e=> e.id == id);
