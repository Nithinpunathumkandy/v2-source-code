...
@Component({
  selector: 'app-kh-check-list-modal',
  templateUrl: './kh-check-list-modal.component.html',
  styleUrls: ['./kh-check-list-modal.component.scss']
})
export class KhCheckListModalComponent implements OnInit {
  ...
  constructor( 
    ...
    private _auditCheckListService: CheckListService,
  ...
  setFormValues(){
    if (this.CheckListSource.hasOwnProperty('values') && this.CheckListSource.values) {
      let { id, title, ms_type_organization_id} = this.CheckListSource.values
      this.form.setValue({
        id: id,
        title: title,
