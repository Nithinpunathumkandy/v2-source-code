...
@Component({
  selector: 'app-isms-workflow-comment-popup',
  templateUrl: './isms-workflow-comment-popup.component.html',
  styleUrls: ['./isms-workflow-comment-popup.component.scss']
})
export class IsmsWorkflowCommentPopupComponent implements OnInit {
  ...
  setTitle() {
    ...
    this.title = IsmsRiskInfoWorkflowStore.type;
  }

  getTitle(){
    if(IsmsRiskInfoWorkflowStore.type=='approve'){
      return 'approve?';
    }else if(IsmsRiskInfoWorkflowStore.type=='revert'){
    return 'revert'
    }else if(IsmsRiskInfoWorkflowStore.type=='submit'){
      return 'submit'
    }
  ...
  setLevel(){
    ...
    this.levelArray.push(0);
      for(let i of IsmsRisksStore.individualRiskDetails?.workflow_items){
  ...
  save(close: boolean = false) {
    ...
    switch (IsmsRiskInfoWorkflowStore.type) {
      ...
      case 'approve':
        ...
        save = this._riskInfoWorkflowService.approveRisk(IsmsRisksStore.riskId,comment);
        break;
        case 'submit':
          let submitComment = {
            comment: this.comments
          }
          save = this._riskInfoWorkflowService.approveRisk(IsmsRisksStore.riskId,submitComment);
          break;
      ...
      case 'revert':
        ...
        save = this._riskInfoWorkflowService.revertRisk(IsmsRisksStore.riskId,data);
        break;
  ...
  closeFormModal() {
    ...
    this.level=null;
    this.comments = null;
    this._eventEmitterService.dismissIsmsRiskWorkflowCommentModal()
  }
