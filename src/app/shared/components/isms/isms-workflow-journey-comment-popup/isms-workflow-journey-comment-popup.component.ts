...
@Component({
  selector: 'app-isms-workflow-journey-comment-popup',
  templateUrl: './isms-workflow-journey-comment-popup.component.html',
  styleUrls: ['./isms-workflow-journey-comment-popup.component.scss']
})
export class IsmsWorkflowJourneyCommentPopupComponent implements OnInit {
  ...
  setTitle() {
    ...
    this.title = IsmsRiskJourneyWorkflowStore.type;
  }

  setLevel(){
    this.levelArray=[];
    // for(let i=0;i<IsmsRiskJourneyStore.individualRiskJourney.journey_next_review_user_level;i++){
    //   this.levelArray.push(i);
    // }
    this.levelArray.push(0)
    for(let i of IsmsRiskJourneyStore.individualRiskJourney?.journey_workflow_items){
  ...
  save(close: boolean = false) {
    ...
    switch (IsmsRiskJourneyWorkflowStore.type) {
      ...
      case 'approve':
        ...
        save = this._riskJourneyWorkflowService.approveRisk(IsmsRisksStore.riskId,comment);
        break;
        case 'submit':
          let submitComment = {
            comment: this.comments
          }
          save = this._riskJourneyWorkflowService.approveRisk(IsmsRisksStore.riskId, submitComment);
          break;  
      ...
      case 'revert':
        ...
        save = this._riskJourneyWorkflowService.revertRisk(IsmsRisksStore.riskId,data);
        break;
  ...
  closeFormModal() {
    ...
    this.level=null;
    this.comments = null;
    this._eventEmitterService.dismissIsmsRiskJourneyWorkflowCommentModal()
  }
