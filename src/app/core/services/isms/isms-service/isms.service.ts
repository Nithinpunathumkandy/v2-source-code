...
// import { IsmsRiskStore } from 'src/app/stores/isms/risks/risks.store';
...
export class IsmsService {
  ...
  getThumbnailPreview(type, token, h?: number, w?: number) {
    ...
    switch (type) {
      case 'risk-treatment': return environment.apiBasePath + '/isms/files/isms-risk-treatment-update-document/thumbnail?token=' + token;
        break;
  ...
  getFilePreview(type, id, file_id?, doc_id?) {
    var previewURL = "";
    switch (type) {
      case 'risk-treatment': previewURL = environment.apiBasePath + '/isms-risk-treatments/' + `${id}` + '/updates/' + `${file_id}` + '/files/'+`${doc_id}`+'/preview'
        break;
  ...
  downloadFile(type, id, file_id?, file_name?, individual_file_id?, fileDetails?) {
    var downloadURL = "";
    switch (type) {
      case 'risk-treatment-documents': downloadURL = environment.apiBasePath + '/isms-risk-treatments/' + `${id}` + '/updates/' + `${file_id}` + '/files/'+`${individual_file_id}`+'/download';
        break;
        case 'risk-treatment-all-documents': downloadURL = environment.apiBasePath + '/isms-risk-treatments/' + `${id}` + '/updates/' + `${file_id}` + '/download';
        break;
     
      case 'risk-treatment-template': downloadURL = environment.apiBasePath + '/isms-risk-treatments/template?risk_ids=' + IsmsRisksStore.riskId;
        break;

      case 'risk-treatment-export': downloadURL = environment.apiBasePath + '/isms-risk-treatments/export?risk_ids=' + IsmsRisksStore.riskId+'&fields=risk_treatment_id,risk_number,risk_title,risk_description,reference_code,title,treatment_dependency,risk_treatment_action_plan,start_date,agreed_date,frequency_of_review,risk_treatment_owner,risk_owner,inherent_risk_rating,budget,status,divisions,departments';
        break;
