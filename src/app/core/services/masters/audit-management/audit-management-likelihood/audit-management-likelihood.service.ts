...
export class AuditManagementLikelihoodService {
  ...
  getItems(getAll: boolean = false, additionalParams?: string, status: boolean = false): Observable<AuditManagementLikelihoodPaginationResponse> {
		let params = '';
		if (!getAll) {
			params = `?page=${AuditManagementLikelihoodMasterStore.currentPage}`;
			if (AuditManagementLikelihoodMasterStore.orderBy) params += `&order_by=${AuditManagementLikelihoodMasterStore.orderItem}&order=${AuditManagementLikelihoodMasterStore.orderBy}`;
		}
		if (AuditManagementLikelihoodMasterStore.searchText) params += (params ? '&q=' : '?q=') + AuditManagementLikelihoodMasterStore.searchText;
		if (additionalParams) params += additionalParams;
		if (status) params += (params ? '&' : '?') + 'status=all';
		return this._http.get<AuditManagementLikelihoodPaginationResponse>('/am-likelihoods' + (params ? params : '')).pipe(
			map((res: AuditManagementLikelihoodPaginationResponse) => {
				AuditManagementLikelihoodMasterStore.setAuditManagementLikelihood(res);
				return res;
			})
		);
	}
  ...
  getItem(id: number): Observable<AuditManagementLikelihood> {
		return this._http.get<AuditManagementLikelihood>('/am-likelihoods/' + id).pipe(
			map((res: AuditManagementLikelihood) => {
				AuditManagementLikelihoodMasterStore.updateAuditManagementLikelihood(res)
				return res;
			})
		);
	}
  ...
  saveItem(item: AuditManagementLikelihood) {
	...
	exportToExcel() {
		this._http.get('/am-likelihoods/export', { responseType: 'blob' as 'json' }).subscribe(
			(response: any) => {
				this._utilityService.downloadFile(response, this._helperService.translateToUserLanguage('audit_management_likelihoods') + ".xlsx");
			}
		)
	}
  ...
  activate(id: number) {
		...
		return this._http.delete('/am-likelihoods/' + id).pipe(
			map(res => {
				...
				this.getItems(false, null, true).subscribe(resp => {
					if (resp.from == null) {
						AuditManagementLikelihoodMasterStore.setCurrentPage(resp.current_page - 1);
						this.getItems(false, null, true).subscribe();
					}
				});
				return res;
	...
	sortAuditManagementLikelihoodList(type: string, text: string) {
		if (!AuditManagementLikelihoodMasterStore.orderBy) {
			AuditManagementLikelihoodMasterStore.orderBy = 'asc';
			AuditManagementLikelihoodMasterStore.orderItem = type;
		}
		else {
			if (AuditManagementLikelihoodMasterStore.orderItem == type) {
				if (AuditManagementLikelihoodMasterStore.orderBy == 'asc') AuditManagementLikelihoodMasterStore.orderBy = 'desc';
				else AuditManagementLikelihoodMasterStore.orderBy = 'asc'
			}
			else {
				AuditManagementLikelihoodMasterStore.orderBy = 'asc';
				AuditManagementLikelihoodMasterStore.orderItem = type;
