...
export class ProjectDetailsDocumentsService  {
  ...
  getItems(getAll: boolean = false,additionalParams?:string,is_all:boolean = false): Observable<ProjectDocumentPaginationResponse> {
    ...
    return this._http.get<ProjectDocumentPaginationResponse>(`/projects/${ProjectsStore.selectedProjectID}/documents` + (params ? params : '')).pipe(
   ...
   */
    getItem(projectId: number,documentId: number) : Observable<IndividualProjectDocument>{
      return this._http.get<IndividualProjectDocument>(`/projects/${ProjectsStore.selectedProjectID}/documents/${documentId}`).pipe(
  ...
  saveProjectDocument(projectId: number,item: any){
    return this._http.post(`/projects/${ProjectsStore.selectedProjectID}/documents`, item).pipe(
      map((res:any )=> {
  ...
  updateItem(projectId: number, documentId: number, item: any): Observable<any> {
    return this._http.put(`/projects/${ProjectsStore.selectedProjectID}/documents/${documentId}`, item).pipe(
      map(res => {
  ...
  delete(projectId: number, documentId: number){
    return this._http.delete(`/projects/${ProjectsStore.selectedProjectID}/documents/${documentId}`).pipe(
      map(res => {
  ...
  getThumbnailPreview(type,token, h?: number, w?: number) {
    ...
    return environment.apiBasePath+ 'ing/files/project-document/thumbnail?token='+token;
  }
  ...
  getFilePreview(type, id, file_id?, doc_id?) {
    var previewURL = "";
    switch(type){ 
      case 'project-documents' : previewURL = environment.apiBasePath+'/projects/'+`${ProjectsStore.selectedProjectID}`+'/documents/'+`${file_id}`+'/file-preview'
        break; 
    }
    if(previewURL){
      try{
  ...
  downloadFile(projectId,type,file_id?, doc_id?, file_name?, fileDetails?) {
    var downloadURL = "";
   switch(type){
     case 'project-documents' : downloadURL = environment.apiBasePath+'/projects/'+`${ProjectsStore.selectedProjectID}`+'/documents/'+`${file_id}`+'/file-download';
       break;
