...
export class ArciService {
  ...
  getItems(getAll: boolean = false, resparams: string = ''): Observable<ARCIPaginationResponse> {
    ...
    return this._http
      .get<ARCIPaginationResponse>('/process-arci'+(resparams ? resparams : '') + (params ? params : ''))
      .pipe(
  ...
  getItemById(id:number):Observable<ARCIDetails>{
    return this._http.get<ARCIDetails>('/process-arci/' + id).pipe(map((res: ARCIDetails) => {
  ...
  saveItem(item: ARCI,type) {
    return this._http.post('/process-arci', item).pipe(
      map(res => {
  ...
  generateTemplate() {
    this._http.get('/process-arci/template', { responseType: 'blob' as 'json' }).subscribe(
      (response: any) => {
  ...
  exportToExcel() {
    let params = '';
    if (ArciStore.orderBy) params += `?order=${ArciStore.orderBy}`;
    if (ArciStore.orderItem) params += `&order_by=${ArciStore.orderItem}`;
    if(RightSidebarLayoutStore.filtersAsQueryString)
    params = (params == '') ? ('?' + RightSidebarLayoutStore.filtersAsQueryString) : (params + '&' + RightSidebarLayoutStore.filtersAsQueryString);
    this._http.get('/process-arci/export'+params, { responseType: 'blob' as 'json' }).subscribe(
      (response: any) => {
  ...
  importData(data){
    ...
    return this._http.post('/process-arci/import',data).pipe(
      map((res:any )=> {
  ...
  delete(id: number) {
    return this._http.delete('/process-arci/' + id).pipe(
      map(res => {
