...
export class BpmFileService {
  ...
  getThumbnailPreview(type, token, h?: number, w?: number) {
    ...
    // +(h && w)?'&h='+h+'&w='+w:''
    switch(type){
      case 'process-document': return environment.apiBasePath+ '/bpm/files/process-document/thumbnail?token='+token;
          break;
      case 'process-flow-document': return environment.apiBasePath+ '/bpm/files/process-flow-document/thumbnail?token='+token;
        break;
      case 'process-activities': return environment.apiBasePath + '/bpm/files/process-activity-document/thumbnail?token=' + token;
        break;
  ...
  getFilePreview(type,id,file_id?,doc_id?){
    ...
    switch(type){
      ...
      case 'process-document' : previewURL = environment.apiBasePath+'/processes/'+`${id}`+'/files/'+`${file_id}`+'/preview'
        break;
      case 'flow-document' : previewURL = environment.apiBasePath+'/processes/'+`${id}`+'/files/'+`${file_id}`+'/process-flow-document'+'/preview';
        break;
      case 'process-activities' : previewURL = environment.apiBasePath+'/processes/'+`${id}`+'/activities/'+`${file_id}`+'/files/'+`${doc_id}`+'/preview';
      break;
      case 'control-document' : previewURL = environment.apiBasePath+'/controls/'+`${id}`+'/files/'+`${file_id}`+'/preview';
      break;
