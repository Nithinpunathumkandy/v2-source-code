...
export class MeetingPlanFileService {
  ...
  getThumbnailPreview(type, token, h?: number, w?: number) {
    ...
    switch(type){
        ...
        case 'report-cover-page': return environment.apiBasePath+ '/mrm/files/meeting-report-page-field-document/thumbnail?token='+token;
          break; 
        case 'meeting-action-plan-update-document': return environment.apiBasePath+ '/mrm/files/meeting-action-plan-update-document/thumbnail?token='+token;
          break; 
  ...
  //Get File Preview 
    ...
    switch(type){
      ...
      case 'meeting-plan-document' : previewURL = environment.apiBasePath+'/meeting-plans/'+`${id}`+'/files/'+`${file_id}`+'/preview'
        break;
      case 'findings-document' : previewURL = environment.apiBasePath+'/mrm/findings/'+`${id}`+'/files/'+`${file_id}`+'/preview'
        break;
      case 'findings-ca-document': previewURL = environment.apiBasePath+ '/mrm/finding-corrective-actions/'+`${id}`+'/files/'+`${file_id}`+'/preview'
        break;
      case 'meetings-document': previewURL = environment.apiBasePath+ '/meetings/'+`${id}`+'/files/'+`${file_id}`+'/preview'
        break;     
      case 'meeting-action-plan-update-document': previewURL = environment.apiBasePath+ '/meeting-action-plans/'+`${id}`+'/updates/'+`${doc_id}`+'/files/'+`${file_id}`+'/preview'
        break;  
      case 'action-plan': previewURL = environment.apiBasePath+ '/meeting-action-plans/'+`${id}`+'/files/'+`${file_id}`+'/preview'
        break;    
  ...
  downloadFile(type, id, file_id?, doc_id?, file_name?, fileDetails?) {
   ...
   switch(type){
    ...
    case 'meeting-plan-attachemt-all' : downloadURL = environment.apiBasePath+'/meeting-plans/'+`${id}`+'/download';
      break;  
    case 'meeting-action-plan-update-document' : downloadURL = environment.apiBasePath+`/meeting-action-plans/${id}/updates/${doc_id}/files/${file_id}/download`;
      break;  
