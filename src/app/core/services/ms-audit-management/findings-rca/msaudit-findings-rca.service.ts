...
export class MsauditFindingsRcaService {
  ...
  constructor(private _http: HttpClient,
    ...
    getItems(id:number,getAll: boolean = false,additionalParams?:string): Observable<RootCauseAnalysisPaginationResponse> {
      let params = '';
      if (!getAll) {
        params = `?page=${FindingRCAStore.currentPage}&status=all`;
        if (FindingRCAStore.orderBy) params += `&order_by=findings.title&order=${FindingRCAStore.orderBy}`;

      }
     
      if(additionalParams) params += additionalParams;
      if(FindingRCAStore.searchText) params += (params ? '&q=' : '?q=')+FindingRCAStore.searchText;

      return this._http.get<RootCauseAnalysisPaginationResponse>('/ms-audit/findings/'+id+'/root-cause-analyses' + (params ? params : '')).pipe(
        map((res: RootCauseAnalysisPaginationResponse) => {
          FindingRCAStore.setRCA(res);
          return res;
        })
      );
    ...
    saveItem(id,item: RootCauseAnalysis,whyValue) {
      return this._http.post('/ms-audit/findings/'+id+'/root-cause-analyses', item).pipe(
        map((res:any )=> {
    ...
    updateItem(findingid: number,id,item: RootCauseAnalysis,whyValue) {      
      return this._http.put('/ms-audit/findings/'+findingid+'/root-cause-analyses/'+id, item).pipe(
        map((res:any )=> {
    ...
    generateTemplate(id:number) {
      this._http.get('/ms-audit/findings/'+id+'/root-cause-analyses/template', { responseType: 'blob' as 'json' }).subscribe(
        (response: any) => {
    ...
    delete(findings_id:number, id: number) {
      return this._http.delete('/ms-audit/findings/'+findings_id+'/root-cause-analyses/' + id).pipe(
        map(res => {
    ...
    exportToExcel(id) {
      this._http.get('/ms-audit/findings/'+id+'/root-cause-analyses/export', { responseType: 'blob' as 'json' }).subscribe(
        (response: any) => {
    ...
    getFindingsRootCauseAnalysis(findingId: number, params?: string){
      return this._http.get<any>('/ms-audit/findings/'+findingId+'/root-cause-analyses' + (params ? params : '')).pipe(
        map((res: any) => {
    ...
    saveFindingsRootCauseAnalysis(id,item: RootCauseAnalysis){
      return this._http.post<any>('/ms-audit/findings/'+id+'/root-cause-analyses',item).pipe(
        map((res: any) => {
